 ...
 * Macros
 */
#define IS_BUILDER(ch, Area) ((ch->pcdata->security >= Area->security || strstr(Area->builders, ch->name) || strstr(Area->builders, "All")) && !IS_SWITCHED(ch))

#define IS_PPLBUILDER(ch, Area) (((ch)->clan ? (strstr(Area->builders, ch->clan->name) && !IS_SWITCHED(ch)) : FALSE))
