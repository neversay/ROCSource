...
char *const dir_name[] = {"北", "東",   "南",   "西",   "上",
                          "下", "東北", "西北", "東南", "西南"};

const int rev_dir[] = {2, 3, 0, 1, 5, 4};

const int movement_loss[SECT_MAX] = {1, 2, 2, 3, 4, 6, 4, 1, 5, 10, 6};
...
void aggr_check(CHAR_DATA *ch, ROOM_INDEX_DATA *to_room) {
    ...
    if (to_room->people) {
        for (inp = to_room->people; inp; inp = inp->next_in_room) {
            ...
            if (IS_SET(inp->in_room->room_flags, ROOM_SAFE))
                return;
            if (inp == ch || !IS_AWAKE(inp) || !can_see(inp, ch) ||
                inp->deleted
                //|| IS_IMMORTAL(ch))  修改為判定 wizinv 2022/07/15
                || (!IS_NPC(ch) && IS_SET(ch->act, PLR_WIZINVIS)))
                continue;
            if (is_same_group(ch, inp) && !IS_AFFECTED(inp, AFF_CRAZY))
                continue;

            if ((IS_AFFECTED(inp, AFF_CHARM) ||
                 IS_AFFECTED(inp, AFF_SUMMONED)) &&
                !IS_AFFECTED(inp, AFF_CRAZY))
                continue;

            if (IS_NPC(ch) && IS_NPC(inp) && !IS_AFFECTED(inp, AFF_CRAZY))
                continue;

            if (IS_AFFECTED(inp, AFF_CRAZY) ||
                (IS_NPC(inp) && IS_SET(inp->act, ACT_AGGRESSIVE))) {
                if (IS_NPC(inp) && IS_NPC(ch) && number_bits(4) == 1 &&
                    !inp->fighting) {
    ...
    if (!ch->fighting && (IS_AFFECTED(ch, AFF_CRAZY) ||
                          (IS_NPC(ch) && IS_SET(ch->act, ACT_AGGRESSIVE)))) {
        for (inp = to_room->people; inp; inp = inp->next_in_room) {
            // safe room 中不會攻擊 2022/07/11
            if (IS_SET(inp->in_room->room_flags, ROOM_SAFE))
                return;
            if (inp == ch || !can_see(ch, inp) ||
                inp->deleted
            ...
            if ((IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED)) &&
                !IS_AFFECTED(ch, AFF_CRAZY))
                continue;
            if (IS_NPC(ch) && IS_NPC(inp) && !IS_AFFECTED(ch, AFF_CRAZY))
                continue;
...
void move_char(CHAR_DATA *ch, int door) {
    ...
    if (pexit->exit_size > 0 &&
        (get_curr_size(ch) > pexit->exit_size)) // 替換體型 size 公式 2022/12/23
    {
    ...
    if ((IS_NPC(ch) &&
        ...
        !IS_SET(ch->act, ACT_IGNORE_NOMOB && !IS_SET(ch->act, ACT_PET)) &&
    ...
    if (!IS_IMMORTAL(ch)) {
        ...
        /*if( to_room->area && IS_SET(to_room->area->area_flags, AREA_CLAN) &&
        !IS_IMMORTAL(ch) && (ch->clan && (!IS_PPLBUILDER(ch, to_room->area) ||
        room_clanlv(to_room) > ch->pcdata->clanlevel )))
        {
        ...
        /*
                ...
                {
                        if ( iClass != ch->class && to_room->vnum ==
           class_table[iClass].guild )
                        {
                                send_to_char(
           "[33m你不被允許\到那裡去。[m\n\r", ch ); return;
                        }
        ...
        */
        ...
        if (in_room->roomprogs && (in_room->progtypes & ROOM_LEAVE) &&
    ...
    // 修改 wizinv 必須為 pc 才判定 2022/10/11
    // if ( !IS_AFFECTED( ch, AFF_SNEAK ) && ( IS_NPC( ch ) || !IS_SET( ch->act,
    // PLR_WIZINVIS ) ) )
    if (!IS_AFFECTED(ch, AFF_SNEAK) &&
        ...
        if (((in_room->sector_type == SECT_WATER_SWIM) ||
            ...
            for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
                if (rch == ch || !can_see(rch, ch) || !IS_AWAKE(rch) ||
                    (IS_SET(rch->act, PLR_BRIEF) && is_same_group(ch, rch) &&
                     rch->master != ch))
                    continue;
                sprintf(buf, "" GRN "%s" GRN "往%s游去。" NOR "\n\r",
                        PERS(ch, rch), dir_name[door]);
            ...
            if (is_flying(ch))
                for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
                    if (rch == ch || !can_see(rch, ch) || !IS_AWAKE(rch) ||
                        (IS_SET(rch->act, PLR_BRIEF) &&
                         is_same_group(ch, rch) && rch->master != ch))
                        continue;
                    sprintf(buf,
                            "" GRN "%s" GRN "騎著%s" GRN "往%s飛過去。" NOR
                            "\n\r",
                            PERS(ch->pcdata->horse, rch),
                            can_see(rch, ch) ? ch->short_descr : "不明人物",
                            dir_name[door]);
                    send_to_char(buf, rch);
                }
            else
            ...
            for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
                ...
                if ((horse = get_eq_char(ch, WEAR_RIDE)))
                    sprintf(buf,
                            "" GRN "%s" GRN "騎著%s" GRN
                            "踢答踢答地往%s走去。" NOR "\n\r",
                            ch->short_descr, horse->description,
                            dir_name[door]);
                else
    ...
    } else if (!IS_IMMORTAL(ch) && (to_room->sector_type == SECT_UNDERWATER &&
                                    !IS_AFFECTED(ch, AFF_GILLS) &&
    ...
    for (fch = in_room->people; fch; fch = fch_next) {
        ...
        if ((fch->master == ch && fch->in_room != ch->in_room) ||
            (!IS_NPC(fch) && fch->tracking == ch &&
             fch->position == POS_STANDING)) {
            if (fch->position == POS_STANDING &&
                !IS_AFFECTED(fch, AFF_PARALYZED)) {
    ...
    // !IS_SET( ch->act, PLR_WIZINVIS ) && ch->position > POS_DEAD )
    if (to_room->roomprogs && (to_room->progtypes & ROOM_ENTER) &&
        (IS_NPC(ch) || (!IS_NPC(ch) && !IS_SET(ch->act, PLR_WIZINVIS))) &&
...
int find_door(CHAR_DATA *ch, char *arg) {
    ...
    /*
            2021/11/23
            跑到這邊必然不是門, 此判定沒有任何意義, 故直接改為 reutrn -1
       丟回各判定式
            ...
            else
            {
                    ...
                    //act( "[33m我在這裡沒看到什麼$T。[m", ch, NULL, arg,
       TO_CHAR ); return -1;
            }
    */
...
void do_open(CHAR_DATA *ch, char *argument) {
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        switch (door) {
        default:
            act("$n" NOR "把$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_ROOM);
            act("你把$d" NOR "給打開了。", ch, NULL, pexit->keyword, TO_CHAR);
            break;
        case 0:
            act("$n" NOR "把北方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把北方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        case 1:
            act("$n" NOR "把東方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把東方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        case 2:
            act("$n" NOR "把南方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把南方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        case 3:
            act("$n" NOR "把西方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把西方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        case 4:
            act("$n" NOR "把上方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把上方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        case 5:
            act("$n" NOR "把下方的$d" NOR "給打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            act("你把下方的$d" NOR "給打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            break;
        }
        ...
        if ((to_room = pexit->to_room) &&
            ...
            for (rch = to_room->people; rch; rch = rch->next_in_room) {
                ...
                switch (door) {
                default:
                    act("$d" NOR "打開了。", rch, NULL, pexit_rev->keyword,
                        TO_CHAR);
                    break;
                case 0:
                    act("南方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 1:
                    act("西方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 2:
                    act("北方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 3:
                    act("東方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 4:
                    act("下方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 5:
                    act("上方的$d" NOR "打開了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                }
            }
...
void do_close(CHAR_DATA *ch, char *argument) {
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        if (IS_SET(pexit->exit_info, EX_CLOSED)) {
            ...
            // );
            act("這方向的$d" NOR "已經是關著的了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            return;
        }
        if (IS_SET(pexit->exit_info, EX_BASHED)) {
            act("這個$d" NOR "是被撞開的所以關不起來。", ch, NULL,
        ...
        default:
            ...
            act("$n" NOR "把$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_ROOM);
            break;
        case 0:
            act("你把北方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把北方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 1:
            act("你把東方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把東方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 2:
            act("你把南方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把南方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 3:
            act("你把西方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把西方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 4:
            act("你把上方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把上方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 5:
            act("你把下方的$d" NOR "關起來了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把下方的$d" NOR "關起來了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        }
        ...
        if ((to_room = pexit->to_room) &&
            ...
            for (rch = to_room->people; rch; rch = rch->next_in_room) {
                if (rch->deleted)
                    continue;
                // act( "$d"NOR"被關上了。", rch, NULL, pexit_rev->keyword,
                // TO_CHAR );
                switch (door) {
                default:
                    act("$d" NOR "被關上了。", rch, NULL, pexit_rev->keyword,
                    ...
                    break;
                case 0:
                    act("南方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 1:
                    act("西方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 2:
                    act("北方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 3:
                    act("東方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 4:
                    act("下方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                case 5:
                    act("上方的$d" NOR "被關上了。", rch, NULL,
                        pexit_rev->keyword, TO_CHAR);
                    break;
                }
            }
...
void do_lock(CHAR_DATA *ch, char *argument) {
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        default:
            ...
            act("$n" NOR "把$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_ROOM);
            break;
        case 0:
            act("你把北方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把北方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 1:
            act("你把東方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把東方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 2:
            act("你把南方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把南方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 3:
            act("你把西方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把西方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 4:
            act("你把上方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把上方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 5:
            act("你把下方的$d" NOR "給鎖上了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把下方的$d" NOR "給鎖上了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        }
...
void do_unlock(CHAR_DATA *ch, char *argument) {
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        // 追加方向判定 2021/11/23
        // act( "$n"NOR"把$d"NOR"的鎖打開了。", ch, NULL, pexit->keyword,
        // TO_ROOM );
        send_to_char("[36m※喀啦※[m\n\r", ch);
        switch (door) {
        default:
            ...
            break;
        case 0:
            act("你把北方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把北方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 1:
            act("你把東方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把東方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 2:
            act("你把南方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把南方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 3:
            act("你把西方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把西方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 4:
            act("你把上方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把上方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 5:
            act("你把下方的$d" NOR "的鎖打開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把下方的$d" NOR "的鎖打開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        }
...
void do_pick(CHAR_DATA *ch, char *argument) {
    ...
    /*
            ...
            if( (  !IS_NPC( ch ) && number_percent() > get_skill_percent(ch,
       gsn_pick_lock) )
    ...
    */
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
            ...
            if (gch->deleted)
                continue;
            if (IS_SET(gch->act, ACT_PET) || IS_AFFECTED(gch, AFF_CHARM) ||
                IS_AFFECTED(gch, AFF_SUMMONED))
                continue;
            ...
            if (gch->group && !IS_NPC(gch->group->leader))
                continue;
            // if ( IS_NPC( gch ) && IS_AWAKE( gch )&& ch->level + 5 <
            // gch->level )
            if (IS_AWAKE(gch) && can_see(gch, ch) &&
                IS_SET(gch->act, ACT_SENTINEL) &&
                IS_SET(gch->act, ACT_STAY_AREA)) {
                act("$N[33m站得離這門鎖實在太近了。[m", ch, NULL,
                    gch, TO_CHAR);
                return;
            }
        }
        if (IS_SET(pexit->exit_info, EX_PICKPROOF)) {
            act("$n" NOR "試著撬開$d" NOR "的鎖，但失敗了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            send_to_char("[33m你沒能成功。[m\n\r", ch);
            return;
        }
        ...
        REMOVE_BIT(pexit->exit_info, EX_LOCKED);
        // 追加方向判定 2021/11/23
        // act( "$n"NOR"把$d"NOR"的鎖給撬開了。", ch, NULL, pexit->keyword,
        // TO_ROOM );
        send_to_char("[36m※喀啦※[m\n\r", ch);
        switch (door) {
        default:
            act("你把$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_ROOM);
            break;
        case 0:
            act("你把北方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把北方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 1:
            act("你把東方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把東方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 2:
            act("你把南方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把南方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 3:
            act("你把西方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把西方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 4:
            act("你把上方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把上方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        case 5:
            act("你把下方的$d" NOR "的鎖給撬開了。", ch, NULL, pexit->keyword,
                TO_CHAR);
            act("$n" NOR "把下方的$d" NOR "的鎖給撬開了。", ch, NULL,
                pexit->keyword, TO_ROOM);
            break;
        }
    ...
    if ((obj = get_obj_here(ch, arg))) {
        ...
        {
                // 判定為 ch 身上穿戴的物品
                if ( ( obj = get_obj_carry( ch, arg ) ) || ( obj = get_obj_wear(
        ch, arg ) ) ) break; if( !gch ) continue; if( !IS_NPC( gch ) ) continue;
                if( gch == ch)
                        continue;
                if( gch->deleted )
                        continue;
                if( IS_SET( gch->act, ACT_PET ) || IS_AFFECTED( gch, AFF_CHARM )
        || IS_AFFECTED( gch, AFF_SUMMONED )) continue; if( is_same_group( ch,
        gch ) ) continue;
                // npc 在 pc leader group 時
                if( gch->group && !IS_NPC( gch->group->leader ) )
                        continue;
                //if ( IS_NPC( gch ) && IS_AWAKE( gch )&& ch->level + 5 <
        gch->level ) if( IS_AWAKE( gch ) && can_see(gch , ch) && IS_SET(
        gch->act, ACT_SENTINEL ) && IS_SET( gch->act, ACT_STAY_AREA ) )
                {
                        act( "$N[33m站得離這物品的鎖實在太近了。[m", ch, NULL,
        gch, TO_CHAR ); return;
                }
        }
        */
        ...
        if (!IS_NPC(ch)) {
            if (number_percent() > get_skill_percent(ch, gsn_pick_lock)) {
                act("$n" NOR "試著撬開$p" NOR "的鎖，但失敗了。", ch, obj, NULL,
                    TO_ROOM);
                send_to_char("[33m你沒能成功。[m\n\r", ch);
                return;
            }
...
void do_wake(CHAR_DATA *ch, char *argument) {
    ...
    if (IS_AWAKE(victim)) {
        act("[33m$N本來就是醒著的。[m", ch, NULL, victim, TO_CHAR);
        return;
    }

    if (IS_AFFECTED(victim, AFF_SLEEP)) {
        act("[33m你叫不醒$M！[m", ch, NULL, victim, TO_CHAR);
        return;
    }
...
void do_sneak(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM)) ||
        (!IS_NPC(ch) && get_skill_percent(ch, gsn_sneak) == 0)) {
        send_to_char("[33m啥？[m\n\r", ch);
        return;
    }
    ...
    if (IS_NPC(ch) ||
        number_percent() <
            get_skill_percent(ch,
                              gsn_sneak)) // ch->pcdata->learned[gsn_sneak] )
    {
        af.type = gsn_sneak;
    ...
    if (ch->position == POS_FIGHTING) {
        ...
        for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
            if (is_same_group(ch, rch)) {
                ...
                if (!rch->fighting && can_see(rch, ch) &&
                    !IS_AFFECTED(rch, AFF_CHARM) &&
                    !IS_AFFECTED(rch, AFF_SLEEP) &&
                    !IS_AFFECTED(rch, AFF_CRAZY)) {
                    if (rch->level <= ch->level ||
                        (rch->level > ch->level &&
                         rch->level - ch->level <= nNightSklv)) {
                        if (IS_GOOD(rch)) {
                            if (IS_GOOD(ch))
                                nSameGroupLv += rch->level / (20 - nNightSklv);
                            if (IS_EVIL(ch))
                                nDiffGroupLv += rch->level / (1 + nNightSklv);
                        }
                        if (IS_NEUTRAL(rch)) {
                            if (IS_NEUTRAL(ch))
                                nSameGroupLv += rch->level / (20 - nNightSklv);
        ...
        // nSuccess = 100 * (nSameGroupLv + nNightSklv * nNightSklv * 2.5) /
        // (nDiffGroupLv + 100 - nNightSklv * nNightSklv);
        nSuccess =
            100 * (nSameGroupLv + nNightSklv * 16) / (nDiffGroupLv + 100);

        // 地型基本成功率 2020/09/16
        if (ch->in_room->sector_type == SECT_CITY)
            nSuccess += 2 * nNightSklv;
...
void do_hide(CHAR_DATA *ch, char *argument) {
    ...
    if (IS_NPC(ch) ||
        number_percent() <
            get_skill_percent(ch, gsn_hide)) // ch->pcdata->learned[gsn_hide] )
    {
...
void do_disperse(CHAR_DATA *ch, char *argument) {
    ...
    if (arg[0] == '\0' || !str_cmp(arg, "all")) {
        if (is_affected(ch, gsn_mana_shield))
            WAIT_MAGIC_STATE(ch, 0);
        if (is_affected(ch, gsn_mystic_mirror))
            WAIT_MAGIC_STATE(ch, 0);
        if (is_affected(ch, gsn_mystic_armor))
            WAIT_MAGIC_STATE(ch, 0);
        if (is_affected(ch, gsn_flaming_armor))
            WAIT_MAGIC_STATE(ch, 0);
        if (is_affected(ch, gsn_powerup))
            WAIT_MAGIC_STATE(ch, 0);
        if (is_affected(ch, gsn_hasten))
            WAIT_MAGIC_STATE(ch, 0);
        ...
        affect_strip(ch, gsn_hasten);
        WAIT_MAGIC_STATE(ch, 0);
    } else if (!str_cmp(arg, "day of power") || !str_cmp(arg, "day")) {
        affect_strip(ch, skill_lookup("day of power"));
        WAIT_MAGIC_STATE(ch, 0);
    } else if (!str_cmp(arg, "holy crown") || !str_cmp(arg, "holy") ||
               !str_cmp(arg, "crown")) {
...
void do_train(CHAR_DATA *ch, char *argument) {
    ...
    nHpFee = (ch->class > 4) ? 10000 * ((ch->max_hit / oldbie / 2) + 1)
                             : 8000 * ((ch->max_hit / newbie / 2) + 1);
    nMnFee = (ch->class > 4) ? 15000 * ((ch->max_mana / oldbie) + 1)
                             : 12000 * ((ch->max_mana / newbie) + 1);
    nMvFee = (ch->class > 4) ? 10000 * ((ch->max_move / oldbie) + 1)
                             : 8000 * ((ch->max_move / newbie) + 1);
    ...
    // 新增連續 train 2022/02/09
        ...
        if (ch->exp >
            EXP_PER_LEVEL(ch->level - 1, ch->class) * (ch->level) + 5000)
            strcat(buf, " practice");
        if (ch->max_hit < (ch->level * hp_f))
            strcat(buf, " hp");
        if (ch->max_mana < (ch->level * mp_f))
    ...
    if (bone_flag == 0) {
        ...
        if (!ok) {
            act("[33m你的$T已經到了上限了。[m", ch, NULL, pOutput,
                TO_CHAR);
            return;
        }
    }
    ...
    if (cost > ch->practice && str_cmp(argument, "practice") &&
        str_cmp(argument, "practice-r")) { // if demote level, ppl's prac point
                                           // may become minus
    ...
    if (bone_flag == 0) {
        ...
        if (times > 2) {
            act("[36m你的$T大量提升了！[m", ch, NULL, pOutput,
                TO_CHAR);
            act("[36m$n的$T大量提升了！[m", ch, NULL, pOutput,
                TO_ROOM);
        } else {
...
void do_bash(CHAR_DATA *ch, char *argument) {
    ...
    if ((door = find_door(ch, arg)) >= 0) {
        ...
        if (IS_SET(pexit->exit_info, EX_BASHPROOF)) {
            act("[33m砰！！你用力地往$d上撞，但是它動也不動。["
                "m",
                ch, NULL, pexit->keyword, TO_CHAR);
            act("[36m砰！！！$n用力地往$d上撞，但是它依然屹立不搖。["
                "m",
                ch, NULL, pexit->keyword, TO_ROOM);
            ////damage( ch, ch, ( ch->max_hit /  3 ), gsn_bash, WEAR_NONE );
            damage(ch, ch, (ch->max_hit / 3), gsn_bash, WEAR_NONE, MSG_NORMAL,
                   DMG_NORMAL);
            ...
            if ((get_curr_str(ch) + sklv * 4 >= 20) &&
                number_percent() <
                    (chance + 4 * (get_curr_str(ch) + sklv * 4 - 20))) {
                /* Success */
                ...
                // 追加方向判定 2021/11/23
                // act( "[36m劈哩啪啦！你把$d撞開了！[m", ch, NULL,
                // pexit->keyword, TO_CHAR );
                // act( "[36m$n把$d撞開了！[m", ch, NULL, pexit->keyword,
                // TO_ROOM );
                switch (door) {
                default:
                    act("劈哩啪啦！你把$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_ROOM);
                    break;
                case 0:
                    act("劈哩啪啦！你把北方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把北方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                case 1:
                    act("劈哩啪啦！你把東方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把東方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                case 2:
                    act("劈哩啪啦！你把南方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把南方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                case 3:
                    act("劈哩啪啦！你把西方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把西方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                case 4:
                    act("劈哩啪啦！你把上方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把上方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                case 5:
                    act("劈哩啪啦！你把下方的$d" NOR "撞開了。", ch, NULL,
                        pexit->keyword, TO_CHAR);
                    act("劈哩啪啦！$n" NOR "把下方的$d" NOR "撞開了。", ch,
                        NULL, pexit->keyword, TO_ROOM);
                    break;
                }
                ////damage( ch, ch, ( ch->max_hit / 20 ), gsn_bash, WEAR_NONE );
                damage(ch, ch, (ch->max_hit / 20), gsn_bash, WEAR_NONE,
                       MSG_NORMAL, DMG_NORMAL);
                ...
                if ((to_room = pexit->to_room) &&
                    ...
                    for (rch = to_room->people; rch; rch = rch->next_in_room) {
                        if (rch->deleted)
                            continue;
                        // 追加方向判定 2021/11/23
                        // act( "[36m$d被撞開了！[m", rch, NULL,
                        // pexit_rev->keyword, TO_CHAR );
                        switch (door) {
                        default:
                            act("劈哩啪啦！$d" NOR "被撞開了。", rch, NULL,
                                pexit_rev->keyword, TO_CHAR);
                            break;
                        case 0:
                            act("劈哩啪啦！南方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        case 1:
                            act("劈哩啪啦！西方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        case 2:
                            act("劈哩啪啦！北方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        case 3:
                            act("劈哩啪啦！東方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        case 4:
                            act("劈哩啪啦！下方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        case 5:
                            act("劈哩啪啦！上方的$d" NOR "被撞開了。", rch,
                                NULL, pexit_rev->keyword, TO_CHAR);
                            break;
                        }
                    }
                ...
                act("[33m噢！你用力地往$d上撞，但是它動也不動。["
                    "m",
                    ch, NULL, pexit->keyword, TO_CHAR);
                act("[36m$n用力地往$d上撞，但是它依然屹立不搖。["
                    "m",
                    ch, NULL, pexit->keyword, TO_ROOM);
    ...
    /* 自撞牆判定 2021/11/23  */
    if (door == -2) {
        act("[33m噢！！你發瘋似的用力地往堅硬的表面撞去。["
            "m",
            ch, NULL, NULL, TO_CHAR);
        act("[36m噢！！$n發瘋似的用力地往堅硬的表面撞去。["
            "m",
            ch, NULL, NULL, TO_ROOM);
        damage(ch, ch, (ch->max_hit / 5), gsn_bash, WEAR_NONE, MSG_NORMAL,
               DMG_NORMAL);
        return;
    }
    ...
    for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
        ...
        */
        ...
        if (IS_AWAKE(gch) && can_see(gch, ch) &&
            IS_SET(gch->act, ACT_SENTINEL) && IS_SET(gch->act, ACT_STAY_AREA) &&
            number_bits(2) >= 2) {
            multi_hit(gch, ch, TYPE_UNDEFINED);
        }
    }
...
void do_snare(CHAR_DATA *ch, char *argument) {
    ...
    if (arg[0] == '\0') {
        ...
        // if ( sklv + get_curr_int( ch ) + get_curr_dex( ch ) + number_bits( 4
        // ) < race_table[victim->race].size * 6 )
        if (sklv + get_curr_int(ch) + get_curr_dex(ch) + number_bits(4) <
            get_curr_size(victim) * 6) // 替換體型 size 公式 2022/12/23
        {
        ...
        if ((IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM) &&
             number_percent() < 50) ||
            (!IS_NPC(ch) &&
             number_percent() * 100 / weakness_percent(victim, gsn_snare) <
                 sklv * 3 + get_skill_percent(ch, gsn_snare) * 4 / 5)) {
            ...
            act("[36m你把$N困在陷阱裡了！[m", ch, NULL, victim,
                TO_CHAR);
            act("[36m$n把你困在陷阱裡了！[m", ch, NULL, victim,
                TO_VICT);
            act("[36m$n把$N困在陷阱裡了。[m", ch, NULL, victim,
                TO_NOTVICT);
        } else {
            act("[33m噢喔！你沒能把$N用陷阱困住。" NOR, ch, NULL,
                victim, TO_CHAR);
            act("[33m$n要把你困在陷阱裡！抓住$m" NOR, ch, NULL,
                victim, TO_VICT);
            act("[36m$n想用陷阱困住$N，但是沒能成功！" NOR, ch,
    ...
    } else /* argument supplied */
    {
        ...
        {                           /* if person other than victim      */
            ...
            if ((IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM) &&
                ...
                (!IS_NPC(ch) &&
                 number_percent() <
                     sklv + get_skill_percent(ch, gsn_snare) * 4 / 5)) {
                affect_strip(victim, gsn_snare);
                ...
                act("[36m你把$M困在陷阱裡了！[m", ch, NULL, victim,
                    TO_CHAR);
                act("[36m$n把你困在陷阱裡了！[m", ch, NULL, victim,
                    TO_VICT);
                act("[36m$n把$N困在陷阱裡了。[m", ch, NULL, victim,
                ...
                act("[33m噢喔！你沒能把$M用陷阱困住。[m", ch,
                    NULL, victim, TO_CHAR);
                act("[33m$n要把你困在陷阱裡！抓住$m！[m", ch,
                    NULL, victim, TO_VICT);
                act("[36m$n想用陷阱困住$N，但是沒能成功。[m",
...
void do_untangle(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM)) ||
        (!IS_NPC(ch) &&
         number_percent() < get_skill_percent(ch, gsn_untangle))) {
        affect_strip(victim, gsn_snare);
        ...
        if (victim != ch) {
            ...
            act("[36m$n替$m自己解開陷阱。[m", ch, NULL, NULL,
                TO_ROOM);
        }
...
void do_wager(CHAR_DATA *ch, char *argument) {
    ...
    for (croupier = ch->in_room->people; croupier;
        ...
        if (IS_NPC(croupier) && IS_SET(croupier->act, ACT_GAMBLE) &&
...
void game_u_l_t(CHAR_DATA *ch, CHAR_DATA *croupier, char *argument) {
    ...
    if (!str_cmp(choice, "lower"))
        ...
        send_to_char(
            "[33m你要賭什麼：Upper、Lower還是Triple？[m\n\r", ch);
        return;
    }
    /*
    ...
    // );
    act("[36m你把$t枚金幣放在桌上，賭「$T」。[m", ch, wager,
        choice, TO_CHAR);
    ch->gold -= amount;
    ...
    sprintf(msg, "[36m$n擲了骰子，結果是%d點、%d點、%d點", die1,
            die2, die3);
    ...
    if (die1 == die2 && die2 == die3) {
        ...
        if (ichoice == 3) {
            ...
            act("[36m是TRIPLE！你贏了$t枚金幣！[m", ch, haul, NULL,
                TO_CHAR);
            ch->gold += amount;
        } else
...
void check_asshunt(CHAR_DATA *ch) {
    ...
    // if ( !IS_NPC(ch) && ch->pcdata->autocombat != AUTOC_NORMAL &&
    // ch->pcdata->autocombat != AUTOC_CRITICAL ) return;
    ...
    for (victim = ch->in_room->people; victim; victim = victim->next_in_room) {
        if (!(victim->hunting == ch || ch->hunting == victim ||
              victim->assassinating == ch || ch->assassinating == victim))
            continue;
        ...
        /*if ch and victim assassinate/hunt each other, the one who reach the
        room earlier(victim) should assassinate/hunt firstly*/
        if (!victim->deleted && IS_AWAKE(victim) && (!victim->fighting) &&
            can_see(victim, ch) && !IS_AFFECTED(ch, AFF_SNEAK)) {
            // autocombat 是否成立移置前方判定 2022/07/14
            // if ( victim->hunting == ch && ( IS_NPC(victim) ||
            // victim->pcdata->autocombat == AUTOC_NORMAL ||
            // victim->pcdata->autocombat == AUTOC_CRITICAL) )
            ...
            if (victim->assassinating == ch) {
                ...
                if (IS_NPC(victim) ||
                    number_percent() <
                        get_skill_percent(victim, gsn_backstab)) {
                    multi_hit(victim, ch, gsn_backstab);
                    WAIT_STATE(ch, assd);
                } else {
                    damage(victim, ch, 0, gsn_backstab, WEAR_WIELD, MSG_NORMAL,
                           DMG_NORMAL);
        ...
        if (can_see(ch, victim) && IS_AWAKE(ch)) {
            // autocombat 是否成立移置前方判定 2022/07/14
            // if ( ch->hunting == victim && (IS_NPC(ch) ||
            // ch->pcdata->autocombat == AUTOC_NORMAL || ch->pcdata->autocombat
            // == AUTOC_CRITICAL || ch->pcdata->autocombat == AUTOC_CHARGE))
            if (ch->hunting == victim) {
                // 加入 hunt charge stun 判定及進入戰鬥第一回合觸發 mob
                // fight_prog 2022/12/07
                int charge_stun = 0;
                int vch_prog = 0;
                ...
                if (!IS_NPC(ch)) {
                    if (ch->pcdata->autocombat == AUTOC_CHARGE) {
                        ...
                        charge_stun = UMAX(
                            3, charge_stun / 2); // hunt charge 有較低的 stun
                    }
                }
                multi_hit(ch, victim, TYPE_UNDEFINED);
                ch->wait += charge_stun; // multi hit 完成後加回 stun
                if (vch_prog == 1)
                    mprog_fight_trigger(victim,
                                        ch); // npc 被開戰則觸發 fight_prog
                // WAIT_STATE( victim, huntd );   1060107 modify by jye
                // 移除STUN效果
                continue;
            }
            if (ch->assassinating == victim) {
                ...
                if (IS_NPC(ch) || !ch->pcdata ||
                    (number_percent() <
                     get_skill_percent(
                         ch,
                         gsn_backstab))) // ch->pcdata->learned[gsn_backstab] ))
                {
                    multi_hit(ch, victim, gsn_backstab);
                    WAIT_STATE(victim, assd);
                } else {
                    damage(ch, victim, 0, gsn_backstab, WEAR_WIELD, MSG_NORMAL,
                           DMG_NORMAL);
                    WAIT_STATE(ch, faild);
                }
                continue;
...
void do_ride(CHAR_DATA *ch, char *argument) {
    ...
    if (pObj) {
        ...
        if (!pObj->pIndexData->horse) {
            bug("Ride_horse: Horse ride no horse vnum %d",
    ...
    if (arg[0] != '\0') {
        ...
        if (IS_SET(ch->act, PLR_HORSE) || IS_NPC(ch) ||
        ...
        if (!horse->pIndexData->replica) {
            bug("Ride_Horse: Horse ride no replica vnum %d",
                horse->pIndexData->vnum);
            return;
        }
        ...
        if (!(qObj->value[1] & 4096) &&
            (sklv == 0 || !(qObj->value[1] & temp))) {
        ...
        for (pObj = horse->carrying; pObj; pObj = obj_next) {
            ...
            if (get_obj_weight(pObj) + get_obj_weight(qObj) - qObj->weight >
                    qObj->value[0] ||
                get_obj_number(qObj) >= qObj->value[1])
        ...
        for (horse = ch->in_room->people; horse; horse = horse->next_in_room) {
            if (IS_NPC(horse) && IS_SET(horse->act, ACT_HORSE) &&
                horse->rembch && horse->rembch == ch)
                break;
        }
...
void do_autocombat(CHAR_DATA *ch, char *argument) {
    ...
    if (!str_cmp(arg, "normal")) {
        ...
        if (!get_eq_char(ch, WEAR_RIDE) &&
            !IS_SET(race_table[ch->race].race_abilities, RACE_CENTAUR)) {
...
void do_hsummon(CHAR_DATA *ch, char *argument) {
    ...
    if (!ch->in_room)
        return;
    if ((/*ch->pcdata->learned[gsn_ride]*/ get_skill_percent(ch, gsn_ride) ==
         0) ||
    ...
    if (sklv < 6 && horse->in_room->area != ch->in_room->area) {
        ...
        return;
               ...
               horse->in_room->area->recall != ch->in_room->area->recall) {
        send_to_char("距離太遠，牠聽不到你的召喚。\n\r", ch);
        return;
    }
