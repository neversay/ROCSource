...
*/
    ...
    if ( !victim->desc )
    {
    	//Modified by Razgriz	20050704
    	fPC ?
    		act( "$N 已經被 disconnect 囉."     , ch, NULL, victim, TO_CHAR )
...
//	return;
  //  }
	...
	act( "$n $T", ch, NULL,
	    ...
	    ? ch->pcdata->bamfout : "的身影突然從你面前消失",  TO_ROOM );
    }
	...
	act( "$n $T", ch, NULL,
	    ...
	    ? ch->pcdata->bamfin : "突然出現在這裡", TO_ROOM );
    }
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	for ( door = 0; door <= 5; door++ )
	{
		...
		{
			sprintf( buf,
				""HIB"%s"NOR" - 通往: %d. Key: %d.  Exit flags: %d. Exit size: %d %s\n\r",
				door == 0 ? "北" : door == 1 ? "東" : door == 2 ? "南" : door == 3 ? "西" : door == 4 ? "上" : door == 5 ? "下" : "bug" ,
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Master: %s.	Leader: %s.  \n\rAffected by: %s.\n\r",
	    ...
	    (victim->group && victim->group->leader)	? victim->group->leader->name	 : "<無>",
	...
	if( is_name( arg, "-a" ) )
	{
		for ( d = descriptor_list; d; d = d->next )
		{
			...
			if( to->desc )
			{
				...
				{
					case POS_DEAD:
						strcat( buf1, RED"它已死了"NOR          ); break;
					case POS_MORTAL:
						strcat( buf1, HIR"受致命傷"NOR          ); break;
					case POS_INCAP:
						strcat( buf1, HIB"失去行動"NOR          ); break;
					case POS_STUNNED:
						strcat( buf1, HIB"不省人事"NOR          ); break;
					case POS_SLEEPING:
						strcat( buf1, HIU"它睡著了"NOR          ); break;
					case POS_RESTING:
						strcat( buf1, HIG"它在休息"NOR          ); break;
					case POS_STANDING:
						strcat( buf1, HIB"它在站著"NOR          ); break;
					case POS_FIGHTING:
						strcat( buf1, HIY"在戰鬥中"NOR          ); break;
					default:break;
				}
