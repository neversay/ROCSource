                ...
                || archer->clan )))
{
    continue;
        ...
        */
        ...
        if (!IS_NPC(archer) && ((get_skill_percent(archer, gsn_meteoric_arrow) >> 1) + (sklv << 3) < number_percent())) {
...
void do_lead_assault(CHAR_DATA *ch, char *argument) {
    ...
    for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
        if (!is_same_group(rch, ch) || !check_blind(rch) || !can_see(rch, ch->fighting) || rch->fighting == ch->fighting || (IS_NPC(rch) && (IS_AFFECTED(rch, AFF_CHARM) || IS_AFFECTED(rch, AFF_SUMMONED) || rch->clan)) || (!IS_NPC(rch) && (ch->pcdata->autocombat & (AUTOC_CONCENTRATION | AUTOC_DODGE | AUTOC_PARRY | AUTOC_STOP))))
            continue;
        // Changing percentage from 20%~100%
        if ((UMAX(get_skill_percent(ch, gsn_change_fighting), get_skill_percent(rch, gsn_change_fighting)) >> 2) + get_curr_int(rch) + get_curr_dex(rch) + sklv * 5 < number_percent()) {
