...
void do_morphing(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && get_skill_percent(ch, gsn_morphing) < number_percent())) {
        send_to_char(
            "你失敗了。\n\r", ch);
        return;
    }
    ...
    if (arg[0] == '\0') {
        ...
        dm_effect(ch);
        return;
    } else if (!str_cmp(arg, "drow") || !str_cmp(arg, "gnome") || !str_cmp(arg, "hobbit") || !str_cmp(arg, "elf") || !str_cmp(arg, "fairy") || !str_cmp(arg, "human") || !str_cmp(arg, "dwarf")) {
        affect_strip(ch, gsn_morphing);
...
void do_ammo_casting(CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj) {
    ...
    if (victim && !victim->deleted) {
        if (skill_table[obj->value[3]].target == TAR_CHAR_SELF && victim != ch) {
            ...
            if (obj->value[3] == 314 || obj->value[3] == 315 || obj->value[3] == 316 || obj->value[3] == 317 || obj->value[3] == 318 || obj->value[3] == 319 || obj->value[3] == 320 || obj->value[3] == 321)
                obj_cast_spell(obj->value[3], (obj->value[2] * 2) / 3, ch, ch, obj);
            else
                obj_cast_spell(obj->value[3], (obj->value[2] * 2) / 3, ch, ch, obj);
...
void do_coin_dart(CHAR_DATA *ch, char *argument) {
    ...
    if (ch->apdr > 0)
        nDamage += ch->apdr * ((nCurrStr - 10) * 2 + nCoinSklv * (nCoinSklv + 4)) / 12;
...
void do_trueshot(CHAR_DATA *ch, char *argument) {
    ...
    if (is_affected(ch, gsn_trueshot))
        return;
    if (ch->move < 100) {
...
void do_perform(CHAR_DATA *ch, char *argument) {
    ...
    if (!str_prefix(arg, "empslash")) {
        ...
        if (arg[0] == '\0')
            victim = ch->fighting;
        else if (!(victim = get_char_room(ch, arg)) || !can_see(ch, victim)) {
            send_to_char("這裡沒有這個人。\n\r", ch);
            return;
        }
        ...
        if (ch->class != RANGER || get_skill_percent(ch, gsn_shot_arrow) < 1 || get_skill_level(ch, gsn_shot_arrow) < 5) {
        ...
        if (ch->class != RANGER || get_skill_percent(ch, gsn_shot_arrow) < 1 || get_skill_level(ch, gsn_shot_arrow) < 3) {
            send_to_char("你在箭術上的造詣還不夠。\n\r", ch);
            return;
        }
        do_powerbolt(ch, argument);
        return;
    } else if (!str_prefix(arg, "demon tremor") && (sn = skill_lookup("demon tremor")) > 0) {
        if (psionic_check(ch, sn))
            spell_demontremor(sn, ch->level, ch, NULL);

        WAIT_STATE(ch, skill_table[sn].beats * 2);
        return;
    } else if (!str_prefix(arg, "glacier strike") && (sn = skill_lookup("glacier strike")) > 0) {
        if (psionic_check(ch, sn))
            spell_glacier_strike(sn, ch->level, ch, NULL);

        WAIT_STATE(ch, skill_table[sn].beats * 2);
        return;
    } else if (!str_prefix(arg, "green breath") && (sn = skill_lookup("green breath")) > 0) {
        if (psionic_check(ch, sn))
            spell_green_breath(sn, ch->level, ch, NULL);

        WAIT_STATE(ch, skill_table[sn].beats * 2);
        return;
    } else if (!str_prefix(arg, "sand tide") && (sn = skill_lookup("sand tide")) > 0) {
        if (psionic_check(ch, sn))
...
bool isBloodThirsty(CHAR_DATA *ch) {
    ...
    if (get_skill_percent(ch, gsn_blood_thirsty) <= 0)
        return FALSE;
    if (is_affected(ch, gsn_blood_thirsty) && affected_value(ch, gsn_blood_thirsty, APPLY_SAVING_LIFE) > 0)
        return TRUE;
...
void LizardmanSkill(CHAR_DATA *ch, CHAR_DATA *victim, int multi) {
    ...
    if (!IS_SET(race_table[ch->race].race_abilities, RACE_LIZARDMAN))
        return;
    ...
    //  噴酸
    if (ch->pcdata->condition[COND_DRUNK] > 10 && number_percent() < UMAX(40, ch->pcdata->condition[COND_DRUNK]) / 4 + nSklv * nSklv / 3) {
        AFFECT_DATA af;
        ...
        } else if (number_percent() < nBlindRate && !IS_AFFECTED(victim, AFF_BLIND) && !IS_SET(victim->special, NO_BLIND) && !(is_affected(victim, gsn_invoke) && get_skill_level(victim, gsn_invoke) > 2)) {
...
void do_backstab(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch) && !ch->fighting && IS_NPC(victim) && victim->fighting) {
        if (can_bs_fighting_mob && (ch->class == ASSASSIN || ch->class == NINJA || ch->class == RANGER)) {
            send_to_char("你偷偷摸摸地從暗處背刺！\n\r", ch);
        } else {
    ...
    if (!IS_AWAKE(victim) || IS_NPC(ch) || IS_AFFECTED(ch, AFF_DARK_RAID) || number_percent() < get_skill_percent(ch, gsn_backstab) + bs_sklv * 3) {
        ...
        if (!IS_NPC(ch) && !victim->deleted && can_see(ch, victim) &&
            ...
            if (ch->pcdata->autocombat == AUTOC_CRITICAL)
                chance += get_skill_level(ch, gsn_deadly_strike) * 10;
            ...
            if (ch->pcdata->snd_attr == APPLY_STR)
                chance += 8;
            if (ch->pcdata->snd_attr == APPLY_DEX)
                chance += 10;
            if (ch->pcdata->snd_attr == APPLY_INT)
                chance += 8;
            // 加入 dual 判定 2020/09/17
            if (isdual)
                chance += 5 * get_skill_level(ch, gsn_dual) + get_skill_percent(ch, gsn_dual) / 10; // 30 + 10
            chance += get_skill_level(ch, gsn_backstab) * 6;                                        // 36
            if (fighting_bs)
                chance /= 2;
            if (ch->move < 10 + bs_sklv * 5)
                chance = 0;
...
return;
    ...
    */
    // 加入 weapon type 命中設定
    if (IS_NPC(ch) || ((ch->pcdata->prime_attr == APPLY_DEX) && ((number_percent() < (100 + TrickySklv * 3) * weakness_percent(victim, gsn_circle) * weakness_percent(victim, gsn_pierce_attack) / 10000))) || (number_percent() < (ch->hitroll / 3 + sklv / 2 * get_curr_dex(ch) + TrickySklv * 3) * weakness_percent(victim, gsn_circle) * weakness_percent(victim, gsn_pierce_attack) / 10000)) {
        /* 移除設定  2020/09/16
...
void do_berserk(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && ch->pcdata->learned[gsn_berserk] == 0)) {
...
void do_change(CHAR_DATA *ch, char *argument) {
    ...
    /* Don't allow charmed mobs to do this, check player's level */
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && number_percent() > get_skill_percent(ch, gsn_change_fighting))) {
        send_to_char("什麼？\n\r", ch);
        return;
    }
...
void do_kick(CHAR_DATA *ch, char *argument) {
    ...
    /* Don't allow charmed mobs to do this, check player's level */
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && ch->pcdata->learned[gsn_kick] == 0)) {
        send_to_char("什麼？\n\r", ch);
        return;
    }
    ...
    */
    ...
    if (IS_SET(race_table[ch->race].race_abilities, RACE_BEHOLDER)) {
        if (nKickHit * weakness_percent(victim, gsn_kick) / 100 > number_percent()) {
            ...
            if (nDamBonus > number_percent()) {
                if (nMonkBonus > 0) {
                    door = number_door(); // generate a random number from 0-5
                    if ((pexit = ch->in_room->exit[door]) != 0 && pexit->to_room && (!IS_SET(pexit->exit_info, EX_CLOSED))) {
                        ringout = TRUE;
                    }
                }
                nDamage *= 2;
            ...
            if (feet && (feet->hp || feet->material->hp)) {
                ...
                for (; ohp; ohp = ohp->next) {
                    if (IS_SET(ohp->event, HP_FIGHT)) {
                        if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_ARTIFACT) && (artifact(ch) * ohp->percent * get_curr_wis(ch) / 2500 + (get_skill_level(ch, gsn_artifact) == 6 ? 40 : 30)) > number_percent()) || (IS_NPC(ch) && ohp->percent * get_curr_wis(ch) / 25 > number_percent())) {
    ...
    while (times-- && !victim->deleted && victim->in_room == ch->in_room) {
        ...
        if (IS_NPC(ch) && number_percent() > times * nCurrDex + ch->level / 4) {
            break;
        } else if (IS_SET(ch->act, PLR_FIST_FIGHT) && number_percent() > times * nCurrDex * (1 + nMonkBonus) + nKickSklv * 4) {
            break;
    ...
    if (ringout && !victim->deleted && victim->in_room == ch->in_room) {
        ...
        if (!IS_NPC(victim)) {
            if (victim->pcdata->autocombat == AUTOC_PARRY && ((get_skill_percent(victim, gsn_autoparry) / 11 + get_skill_level(victim, gsn_autoparry) * 15) > number_percent())) {
                return;
            }
        }
        // 體型能抵擋踢飛 2022/12/24
...
void do_hunt(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch) && (ch->pcdata->autocombat == AUTOC_STOP || ch->pcdata->autocombat == AUTOC_DODGE || ch->pcdata->autocombat == AUTOC_PARRY || ch->pcdata->autocombat == AUTOC_CONCENTRATION || ch->pcdata->autocombat == AUTOC_INSTANT)) {
            ...
            if (IS_NPC(ch) || number_percent() < get_skill_percent(ch, gsn_ass)) {
                ...
                if (!IS_NPC(ch)) {
                    ...
                    if (ch->pcdata->snd_attr == APPLY_STR)
                        chance += 5;
                    if (ch->pcdata->autocombat == AUTOC_CRITICAL)
                        chance += get_skill_percent(ch, gsn_deadly_strike) / 2;
                    if (ch->pcdata->snd_attr == APPLY_DEX)
                        chance += 3;
...
void do_frenzy(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch) && get_eq_char(ch, WEAR_WIELD_2) && nDualSklv >= 4) {
        ...
        if (nDualSklv < 6)
            nDualPerc = nDualPerc * nDualPrac / 100;
    }
    /*
         ...
         */
    if (!IS_NPC(ch) && (IS_SET(race_table[ch->race].race_abilities, RACE_BEHOLDER) || IS_SET(race_table[ch->race].race_abilities, RACE_WEREWOLF))) {
    ...
    if (!IS_NPC(ch)) {
        if (ch->class == SHAMAN)
            chance = 5;
        else if (ch->class == WARLORD)
            chance = 5;
        else if (ch->class == MARTIALMAN)
            chance = 0;
        else
            chance = 10;
    ...
    if (!IS_NPC(ch) && number_percent() + chance > get_skill_percent(ch, gsn_frenzy)) {
        send_to_char("你失敗了！\n\r", ch);
        return;
    }
    ...
    if (sklv < 4 && !IS_NPC(ch)) {
        ...
        for (i = 0; i < 4 + extrahit; i++) {
            if (!(victim->in_room == ch->in_room) || (victim->position == 0) || ch->hit < 0)
                break;
        ...
        for (i = 0; i < extrahit; ++i) {
            if (!(victim->in_room == ch->in_room) || (!IS_NPC(ch) // pos_dead 及 ch 小於 0 的情況 加入 pc 限定 2022/12/29
                                                      && ((victim->position == 0) || ch->hit < 0)))
                break;
            one_hit(ch, victim, gsn_frenzy, WEAR_WIELD);
            if (number_percent() <= nDualPerc)
                one_hit(ch, victim, gsn_frenzy, WEAR_WIELD_2);
...
void do_bashhead(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch)) {
        if (ch->pcdata->autocombat == AUTOC_STOP || ch->pcdata->autocombat == AUTOC_CONCENTRATION || ch->pcdata->autocombat == AUTOC_PARRY || ch->pcdata->autocombat == AUTOC_DODGE || ch->pcdata->autocombat == AUTOC_INSTANT) {
            return;
        }
    }
    if (is_affected(ch, gsn_stone_form)) {
...
*/
    if (!IS_NPC(ch)) {
        if (!IS_SET(ch->act, PLR_FIST_FIGHT) && !get_eq_char(ch, WEAR_WIELD) && !get_eq_char(ch, WEAR_WIELD_2) && !get_eq_char(ch, WEAR_TWO_HAND)) {
...
if ( arg[0] != '\0' )
{
    ...
    */
    ...
    if ((!IS_NPC(ch) && ((get_skill_percent(ch, gsn_bashhead) / 2 + sklv * 6) * weakness_percent(victim, gsn_bashhead) / 100) > number_percent() + (get_curr_dex(ch) - get_curr_dex(victim))) || (IS_NPC(ch) && ch->level > number_percent() + (get_curr_dex(ch) - get_curr_dex(victim)))) {
        ...
        > ( number_percent() * weakness_percent(victim, gsn_bashhead) / 100 ) + 5 - ( ( sklv == 6 ) ? sklv * 2 : ( ( sklv == 5 ) ? sklv : sklv / 2 ) ) )
{
    ...
    ch->damroll += ch->damroll * sklv/15;
        ...
        if (get_eq_char(ch, WEAR_WIELD_2) && !get_eq_char(ch, WEAR_WIELD) && !get_eq_char(ch, WEAR_TWO_HAND)) {
...
void do_stare(CHAR_DATA *ch, char *argument) /*{{{*/
{
    ...
    if (ch->race != race_lookup("Beholder") && ch->race != race_lookup("God")) {
        send_to_char("什麼？\n\r", ch);
        return;
    }
    ...
    if ((IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM)) || IS_AFFECTED(victim, AFF_CHARM))
        return;
    ...
    if (saves_spell(ch->level + sklv, victim) > number_percent() + 20) {
        // charm
        if (ch->level >= victim->level - 1 - sklv && !IS_SET(victim->special, NO_CHARM)) {
            if (victim->master)
        ...
        // sleep
        if (ch->level >= victim->level - 2 - sklv && !IS_SET(victim->special, NO_SLEEP)) {
            af.type      = gsn_sleep;
            af.duration  = 4 + sklv;
            af.location  = APPLY_NONE;
            af.modifier  = 0;
        ...
        */
        // crazy
        if (ch->level >= victim->level - 3 - sklv && !IS_SET(victim->special, NO_CRAZY)) {
...
void do_circular(CHAR_DATA *ch, char *argument) {
    ...
    /* Don't allow charmed mobs to do this, check player's level */
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && get_skill_percent(ch, gsn_circular) + sklv * 3 < number_percent())) {
        send_to_char("你失敗了。\n\r", ch);
        return;
    }
    ...
    if (!IS_NPC(ch)) {
        if (ch->pcdata->autocombat == AUTOC_STOP || ch->pcdata->autocombat == AUTOC_DODGE || ch->pcdata->autocombat == AUTOC_PARRY || ch->pcdata->autocombat == AUTOC_CONCENTRATION || ch->pcdata->autocombat == AUTOC_INSTANT)
            return;
    }
    ...
    for (vch = ch->in_room->people; vch; vch = next_vch) {
        ...
        if (ch != vch && !vch->deleted && (is_same_group(vch->fighting, ch) || (ch->fighting == vch))) {
            ...
            if (!IS_NPC(ch) && (number_percent() < (get_skill_percent(ch, gsn_circular) / 3) + sklv * (13 + sklv) / 2 + get_curr_int(ch) / (7 - nTacticSklv))) {
...
void do_meteoric_arrow(CHAR_DATA *ch, char *argument) {
    ...
    /* Don't allow charmed mobs to do this, check player's level */
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && get_skill_percent(ch, gsn_meteoric_arrow) < number_percent())) {
        send_to_char("你失敗了。\n\r", ch);
        return;
    }
    if (is_affected(ch, gsn_stone_form)) {
    ...
    if (!IS_NPC(ch)) { // pc 發數計算 2021/10/24
        ...
        if (victim && !victim->deleted)
            multi_hit(ch, victim, gsn_meteoric_arrow); // base multi hit
        wait += ch->wait;
        ch->wait = 0;
...
void do_rainfall_arrow(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED) || ch->clan)) || (!IS_NPC(ch) && get_skill_percent(ch, gsn_rainfall_arrow) < number_percent())) {
        send_to_char("你失敗了。\n\r", ch);
        return;
    }
    if (is_affected(ch, gsn_stone_form)) {
...
void do_charge(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_SET(race_table[ch->race].race_abilities, RACE_CENTAUR)) {
        if ((nRoomType == SECT_HILLS && !(horse->value[1] & 3776))                                                                                                    // 64 | 128 | 512 | 1024 | 2048
            || ((nRoomType == SECT_MOUNTAIN || nRoomType == SECT_FOREST) && !(horse->value[1] & 2048)) || (nRoomType == SECT_WATER_SWIM && !(horse->value[1] & 1536)) // 512 | 1024
            || ((nRoomType == SECT_WATER_NOSWIM || nRoomType == SECT_UNDERWATER) && !(horse->value[1] & 1024)) || ((nRoomType == SECT_AIR || nRoomType == SECT_DESERT) && !(horse->value[1] & 512))) {
            if (nRideSklv < 5) {
    ...
    if (wield->value[3] != 2 && nChrgSklv != 6) {
        if (get_skill_level(ch, gsn_lance_mastery) > 5) {
            if (wield->value[3] != 1 && wield->value[3] != 3 && wield->value[3] != 7 && wield->value[3] != 9) {
    ...
    */
    ...
    nHitRate = (nHitRoll + get_curr_dex(ch) * 2) * get_vrace_hrp(victim, wield) / 100 + get_vrace_hrq(victim, wield);
    ...
    */
    ...
    if (get_charge_hit_rate(nHitRate, nRideSklv, nDodge) * weakness_percent(victim, gsn_charge) / 100 > number_percent()) {
        ...
        if (wield) {
            ...
            if (wield->hp || wield->material->hp) {
                for (; ohp; ohp = ohp->next) {
                    if (IS_SET(ohp->event, HP_FIGHT)) {
                        if (IS_SET(ch->act, PLR_ARTIFACT) && (artifact(ch) * ohp->percent * nCurrWis / 2500 + (get_skill_level(ch, gsn_artifact) == 6 ? 40 : 30)) > number_percent()) {
            ...
            // 武器損耗
            if (wield->value[3] != 14 && number_bits(5) < 8)
                waste_dura(wield); // 33%
        }
        nHitCount++;
    } else {
    ...
    if (nHitCount == 0)
        nSelfDmg = 0;
    if (nSelfDmg > 0)
        damage(ch, ch, nSelfDmg, gsn_charge, WEAR_NONE, MSG_NORMAL, DMG_NORMAL);
        ...
        if (IS_SET(race_table[ch->race].race_abilities, RACE_CENTAUR) ||
            ...
            (horse->value[1] & 64)
                ...
                && (get_curr_size(ch) - get_curr_size(victim)) * 8 // 替換體型 size 公式 2022/12/23
                           + get_skill_percent(ch, gsn_charge) / 4 + nChrgSklv >
                       number_percent()) {
            door = number_door(); /*generate a random number from 0-5*/
            if ((pexit = ch->in_room->exit[door]) != 0 && pexit->to_room && (!IS_SET(pexit->exit_info, EX_CLOSED))) {
                ringout = TRUE;
...
void do_throw(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch)) { // pc
        ...
        if (obj->item_type == ITEM_POTION || obj->item_type == ITEM_PILL || obj->item_type == ITEM_SCROLL || obj->item_type == ITEM_WAND || obj->item_type == ITEM_STAFF) {
        ...
        if (IS_OBJ_STAT(obj, ITEM_SPELL)) {
            ...
            if (IS_SET(race_table[victim->race].race_abilities, RACE_ANTI_MAGIC)) {
                if (IS_NPC(victim)) {
                    ...
                    EncDam = EncDam * (99 - 34 * (get_skill_level(victim, gsn_inborn_ability) * get_skill_level(victim, gsn_inborn_ability)) / 36) / 100;
                }
            }
        ...
        if (obj && (obj->hp || obj->material->hp)) {
            for (; ohp; ohp = ohp->next) {
                if (IS_SET(ohp->event, HP_WEAPON) || IS_SET(ohp->event, HP_ARROW) || IS_SET(ohp->event, HP_DART))
                    if (IS_SET(ch->act, PLR_ARTIFACT) && ohp->percent * (get_curr_int(ch) + sklv) / 25 > number_percent())
...
void do_combat_howl(CHAR_DATA *ch, char *argument) /*{{{*/
{
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (d->connected == CON_PLAYING && !IS_SET(och->in_room->room_flags, ROOM_CONE_OF_SILENCE)) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        victim_next = victim->next_in_room;
        if (sklv > 4 && is_same_group(ch, victim))
            continue;
        if (!IS_NPC(ch) && !IS_NPC(victim) && (!registered(ch, victim) || (!licensed(ch) && victim->race != race_lookup("Vampire")))) {
            continue;
        }
        ...
        if (victim != ch) {
            ...
            if (is_affected(victim, gsn_will_fortress) && number_percent() < -(affected_value(victim, gsn_will_fortress, APPLY_SAVING_ILLUSION)))
                continue;

            if (victim->hit > 0 && victim->wait == 0 && (ch->level + 3 + sklv - victim->level) > number_bits(3) && check_exit(victim, 6) > 0) // Added by Razgriz 20050908
                do_flee(victim, "");
        }
    }
    WAIT_STATE(ch, skill_table[gsn_combat_howling].beats);
