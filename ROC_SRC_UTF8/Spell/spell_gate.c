...
void spell_flame_elemental(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (ch->in_room->sector_type == SECT_WATER_SWIM || ch->in_room->sector_type == SECT_WATER_NOSWIM || ch->in_room->sector_type == SECT_AIR) {
    ...
    if (!IS_NPC(ch))
        AddOwnerNameToElemental(victim, ch, szEleType, count);
    return;
}
...
void spell_water_elemental(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (ch->in_room->sector_type == SECT_DESERT || ch->in_room->sector_type == SECT_AIR) {
...
void spell_clay_golem(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((!IS_NPC(ch) && !IS_IMMORTAL(ch)) || !strcmp(ch->in_room->area->identity, "school") || !strcmp(ch->in_room->area->identity, "skarabrae") || IS_SET(ch->in_room->room_flags, ROOM_SAFE)) {
...
void spell_gate(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((!IS_NPC(ch) && !IS_IMMORTAL(ch)) || IS_AFFECTED(ch, AFF_SUMMONED) || IS_AFFECTED(ch, AFF_CHARM) || !strcmp(ch->in_room->area->identity, "school") || !strcmp(ch->in_room->area->identity, "skarabrae") || IS_SET(ch->in_room->room_flags, ROOM_SAFE)) {
        send_to_char("一股神聖力場阻絕了你的企圖！\n\r", ch);
        return;
    }
    ...
    for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
        if (IS_NPC(gch) && !IS_AFFECTED(gch, AFF_CHARM))
            npccount++;
        if (!IS_NPC(gch) || (IS_NPC(gch) && IS_AFFECTED(gch, AFF_CHARM)))
            pccount++;
    }
    ...
    if (level < 20 || (IS_NPC(ch) && !IS_SET(ch->special, EXPERT) && number_percent() > ch->level - 10) || ((sklv < 5) && number_percent() > level * 2)) {
        ...
        if (vam->spec_fun == spec_lookup("spec_thief") || vam->spec_fun == spec_lookup("spec_janitor"))
            vam->spec_fun = 0;
        ...
        for (;;) {
            ...
            if (pMobIndex && pMobIndex->count > 1 && !pMobIndex->mobprogs && IS_SET(pMobIndex->act, ACT_AGGRESSIVE)) {
                if (pMobIndex->level > level - 21 + sklv * 2 && pMobIndex->level < level + sklv * 2) {
                    if (sklv != 6)
                        break;
