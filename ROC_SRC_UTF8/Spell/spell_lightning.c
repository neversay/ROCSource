            ...
            if (ch->class == SAVANT) {
                if (IS_GOOD(ch)) {
                    dam = dam * ((new_saves_spell(ch->pp * 2, vch, MAGIC_SAINT) >= (new_saves_spell(ch->pp * 1.5, vch, MAGIC_LIGHTNING)))
                                     ? (new_saves_spell(ch->pp * 2, vch, MAGIC_SAINT)) / 100
                                     : (new_saves_spell(ch->pp * 1.5, vch, MAGIC_LIGHTNING)) / 100);
                } else if (IS_EVIL(ch)) {
                    dam = dam * ((new_saves_spell(ch->pp * 2, vch, MAGIC_DARK) >= (new_saves_spell(ch->pp * 1.5, vch, MAGIC_LIGHTNING)))
                                     ? (new_saves_spell(ch->pp * 2, vch, MAGIC_DARK)) / 100
                                     : (new_saves_spell(ch->pp * 1.5, vch, MAGIC_LIGHTNING)) / 100);
                } else {
...
void spell_chain_lightning(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    dam = dam * (new_saves_spell(ch->pp, victim, MAGIC_LIGHTNING)) / 100;
    if (!IS_NPC(ch))
        dam = dam * (250 + nLBSklv * (nLBSklv + 4)) / 250;
    ...
    while (times--) {
        for (victim = ch->in_room->people; victim; victim = victim_next) {
            ...
            if (!IS_NPC(victim) && IS_SET(victim->act, PLR_WIZINVIS))
                continue;
            if (IS_NPC(ch) && IS_NPC(victim) && !(victim->fighting == ch))
                continue;
...
void spell_electrostatic_bolt(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (magic_damage(ch, victim, dam, sn, WEAR_NONE) == -2) {
        ...
        // 有 mystic armor 不作用負面 2020/10/26
        if (!IS_AFFECTED(victim, AFF_MYSTIC_ARMOR)) {
            if (sklv * 2 + old_saves_spell(ch->pp, victim, MAGIC_LIGHTNING) > (number_percent() * 100 / weakness_percent(victim, sn)) && dam > 0) // 有傷害才會付加效果 add at 19/02/04
            {
                af.type = sn;
                // af.duration  = 3 * ( new_saves_timer( ch->pp, victim,MAGIC_SPELL)) / 100;
                //  修改作用時間 2020/11/06
    ...
    while (times-- && !victim->deleted && victim->in_room == ch->in_room) {
        if (number_percent() > 66 - sklv * 5) {
            ...
            if (magic_damage(ch, victim, dam, sn, WEAR_NONE) == -2) {
                ...
                // 有 mystic armor 不作用負面 2020/10/26
                if (!IS_AFFECTED(victim, AFF_MYSTIC_ARMOR)) {
                    if (sklv * 2 + new_saves_spell(ch->pp, victim, MAGIC_LIGHTNING) > (number_percent() * 100 / weakness_percent(victim, sn)) && dam > 0) // 有傷害才會付加效果 add at 19/02/04
                    {
                        af.type = sn;
                        // af.duration  = 3 * ( new_saves_timer( ch->pp, victim,MAGIC_SPELL)) / 100;
                        //  修改作用時間 2020/11/06
...
void spell_lightning_bolt(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    //     before: 100 - 25 / 3 = 92
    //     after : 100 - 20 / 5 = 96
    ex_md = ch->amdr - 0.4 * UMAX(ch->amdr - 50 - sklv * 5, 0) - 0.4 * UMAX(ch->amdr - 80 - sklv * 5, 0);
    dam += ex_md * 1.5;
    ...
    */
    ...
    while (times-- && !victim->deleted && victim->in_room == ch->in_room && victim->in_room == chk_room) {
        if (number_percent() > 66 - sklv * 5) {
            if (ch->mana > 40)
                ch->mana -= 40;
            else
                ...
                */
                ...
                while (sklv--) {
                    for (i = 0; i < 15; ++i) { // max try 15 times
                        ...
                        if ((_armor = get_eq_char(victim, place)) && (_armor->item_type == ITEM_ARMOR)) {
                            waste_dura(_armor);
                            waste_dura(_armor);
                            waste_dura(_armor);
                            waste_dura(_armor);
