...
void spell_armor(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (nAcBonus < 200)
        nAcBonus = (nAcBonus + 200) / 2;

    if (!IS_NPC(ch) && ch == vo && ch->pcdata->prime_attr == APPLY_INT)
        nAcBonus += nAcBonus / 2;
...
void spell_day_of_brightness(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim->next_in_room) {
        ...
        if (is_affected(victim, skill_lookup("sun flash")) || is_affected(victim, gsn_blindness)) {
            if (!isBattleCast)
                spell_cure_blindness(sn, level, ch, (void *)victim);
        }
    ...
    /* Defensive spell - remove ALL effects */
    {
        for (paf = victim->affected; paf; paf = paf->next) {
            ...
            */
            ...
            if (victim->special != 0 && ((paf->type == skill_lookup("sanctuary") && IS_SET(victim->special, ALY_SAN)) || (paf->type == gsn_invis && IS_SET(victim->special, ALY_INVIS)) || (paf->type == gsn_hide && IS_SET(victim->special, ALY_HIDE)) || (paf->type == gsn_sneak && IS_SET(victim->special, ALY_SNEAK)) || (paf->type == skill_lookup("detect invis") && IS_SET(victim->special, ALY_DI)) || (paf->type == skill_lookup("detect hidden") && IS_SET(victim->special, ALY_DH)) || (paf->type == skill_lookup("infravision") && IS_SET(victim->special, ALY_INF)) || (paf->type == skill_lookup("barrier") && IS_SET(victim->special, ALY_BAR))))
                continue;

            if (paf->duration < 0)
                continue;
            if ((number_percent() * 100 / weakness_percent(victim, sn)) > (paf->duration - ch->level - (get_curr_wis(ch) * 2) - get_skill_level(ch, sn) * 6)) {
        ...
        return;
    } else { /* Offensive spell - enforced by multi_hit whether succeeds or fails */
        for (paf = victim->affected; paf; paf = paf->next) {
            if (paf->deleted)
                continue;
            ...
            if (IS_IMMORTAL(ch) || UMAX(5, saves_spell(ch->pp, victim) + (WIS_DIFF(ch, victim) * 4) - 23 + get_skill_level(ch, sn) * 2) > (number_percent() * 100 / weakness_percent(victim, sn))) {
        ...
        if (victim->affected_by != 0) {
            for (; aff <= 21990232555552LL; aff <<= 1) {
                ...
                if (victim->special != 0 && ((aff == AFF_INVISIBLE && IS_SET(victim->special, ALY_INVIS)) || (aff == AFF_DETECT_INVIS && IS_SET(victim->special, ALY_DI)) || (aff == AFF_DETECT_HIDDEN && IS_SET(victim->special, ALY_DH)) || (aff == AFF_SANCTUARY && IS_SET(victim->special, ALY_SAN)) || (aff == AFF_HIDE && IS_SET(victim->special, ALY_HIDE)) || (aff == AFF_SNEAK && IS_SET(victim->special, ALY_SNEAK)) || (aff == AFF_INFRARED && IS_SET(victim->special, ALY_INF))))
                    continue;

                if (IS_IMMORTAL(ch) || UMAX(2, saves_spell(ch->pp, victim) + (WIS_DIFF(ch, victim) * 4) - 23 + get_skill_level(ch, sn) * 2) > (number_percent() * 100 / weakness_percent(victim, sn))) {
...
void spell_flaming_armor_strike(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!IS_NPC(ch)) {
        ...
        if (victim->mdr >= 0) {
            if (nSklv < 3)
                dam = dam - victim->mdr * 2 + ch->amdr * 0.4;
            if (nSklv >= 3 && nSklv < 6)
                dam = dam - victim->mdr * 1.5 + ch->amdr * 0.6;
            if (nSklv > 5)
                dam = dam - victim->mdr * 0.8 + ch->amdr * 0.8;
        } else if (victim->mdr < 0) {
            if (nSklv < 3)
                dam = dam - victim->mdr * 0.8 + ch->amdr * 0.4;
            if (nSklv >= 3 && nSklv < 6)
                dam = dam - victim->mdr * 1.5 + ch->amdr * 0.6;
            if (nSklv > 5)
                dam = dam - victim->mdr * 2 + ch->amdr * 0.8;
        }
...
void spell_lore(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    sprintf(rgBuf, "%s - %s(%s):\n\r等級：%2d  性別：%s  陣營： %d\n\r",
            ...
            victim->level,
            victim->sex == SEX_MALE ? "男" : victim->sex == SEX_FEMALE ? "女"
                                                                       : "中",
            victim->alignment);
    strcat(rgLore, rgBuf);
    ...
    if (nSklv >= 6 && ch->level >= 41) {
        if (victim->weakness) {
            nWeakStat = 100;
            if (get_skill_exp(ch) > 1000000)
                nWeakStat += (get_skill_exp(ch) - 1000000) / 20000;
            if (nWeakStat > 120)
                nWeakStat = 120;
...
void spell_recall(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (nRecallSklv > 4)
        nManaRegain = (nRecallSklv - 1) * nRecallSklv * (nRecallSklv + 1);
    ...
    if (IS_SET(to_room->room_flags, ROOM_NO_RECALL_IN) && ch->in_room->area != to_room->area) {
...
void spell_mark(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!IS_SET(ch->in_room->room_flags, ROOM_SAVE) && str_cmp(ch->in_room->area->identity, "skarabrae") && !IS_IMMORTAL(ch))
        dura -= 2;
