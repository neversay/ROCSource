...
int skill_lookup(const char *name) {
    ...
    for (sn = 0; sn < MAX_SKILL; sn++) {
        ...
        if (LOWER(arg1[0]) == LOWER(skill_table[sn].name[0]) && !str_prefix(arg1, skill_table[sn].name)) {
            tmp2 = one_argument(skill_table[sn].name, arg2);
            tmp2 = one_argument(tmp2, arg2);
            tmp1 = one_argument(tmp1, arg1);
            while (arg2[0] != '\0' && arg1[0] != '\0') {
                if (LOWER(arg1[0]) != LOWER(arg2[0]) || str_prefix(arg1, arg2)) {
                    break;
                }
                tmp2 = one_argument(tmp2, arg2);
                tmp1 = one_argument(tmp1, arg1);
...
void delayed_do_cast(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(ch) && number_percent() > 10 + get_skill_percent(ch, sn) + 5 * get_skill_level(ch, sn)) {
        ...
        if (victim) {
            ...
            if ((IS_SET(victim->special, REF_MAGIC)) && skill_table[sn].target == TAR_CHAR_OFFENSIVE && sn != gsn_wizard_rage && sn != gsn_crushing_hand && sn != gsn_ice_comet && sn != gsn_fire_breath && sn != gsn_acid_breath && sn != gsn_frost_breath && sn != gsn_gas_breath // gas breath 轉換範圍 2022/12/23
                && sn != gsn_lightning_breath) {
                vo     = ch;
                ch     = victim;
                victim = vo;
            } else if (IS_AFFECTED(victim, AFF_MYSTIC_MIRROR) && skill_table[sn].target == TAR_CHAR_OFFENSIVE && sn != gsn_wizard_rage && sn != gsn_crushing_hand && sn != gsn_ice_comet && sn != gsn_high_explosive && sn != gsn_fire_breath && sn != gsn_acid_breath && sn != gsn_frost_breath && sn != gsn_lightning_breath && sn != gsn_gas_breath // gas breath 轉換範圍 2022/12/23
            ...
            */
            ...
            for (i = 0; i < times; i++) {
                if (IS_AFFECTED(victim, AFF_MYSTIC_ARMOR) && ch != victim && (sn == gsn_mute || sn == skill_lookup("remove curse") || sn == skill_lookup("dispel magic") || sn == gsn_seal_magic || sn == gsn_charm_person || sn == gsn_blindness || sn == gsn_fear || sn == gsn_paralyze || sn == gsn_disease || sn == gsn_poison || sn == gsn_faerie_fire || sn == gsn_tardy || sn == gsn_sleep || sn == gsn_weaken || sn == gsn_curse || sn == gsn_polymorph || sn == gsn_crushing_hand || sn == gsn_soulsteal)) {
                    timesMA += 1;
        ...
        for (i = 0; i < times; i++) {
            ...
            if (victim && (victim->position == POS_DEAD || victim->in_room != ch->in_room || victim->in_room != chk_room)) {
                return;
            }
...
void do_cast(CHAR_DATA *ch, char *argument) {
    ...
    */
    ...
    if ((sn = skill_lookup(arg1)) < 0 || (!IS_NPC(ch) && get_skill_percent(ch, sn) == 0))
    ...
    if (!IS_NPC(ch) && ch->position < skill_table[sn].minimum_position) {
        if (ch->fighting) {
            for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
                if (gch->fighting == ch) {
                    if (ch->pcdata->autocombat == AUTOC_CONCENTRATION && get_skill_percent(ch, gsn_concentration) + get_skill_level(ch, gsn_concentration) * 2 < number_percent() / 2 + 5) {
                        send_to_char("你無法專心.\n\r", ch);
                        return;
                    } else if (ch->pcdata->autocombat != AUTOC_CONCENTRATION && get_skill_percent(ch, gsn_concentration) + get_skill_level(ch, gsn_concentration) * 3 < number_percent() + 30) {
                        send_to_char("你無法專心.\n\r", ch);
                        return;
                    }
                }
    ...
    */
    ...
    if (!delay || skill_table[sn].spell_fun == spell_null || (IS_NPC(ch) && qm && qm->v0 > 1) || (!IS_NPC(ch) && ch->pcdata->autocombat == AUTOC_INSTANT)) {
...
void obj_cast_spell(int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj) {
    ...
    if (get_skill_level(ch, gsn_artifacts_mastery) > 3)
        ArtiMasSklv += 2;
    if (get_skill_level(ch, gsn_artifacts_mastery) > 4)
        ArtiMasSklv += 3;
    if (get_skill_level(ch, gsn_artifacts_mastery) > 5)
        ArtiMasSklv += 4;
    ...
    case TAR_IGNORE:
        ...
        if (sn == skill_lookup("magic enchantment") || sn == skill_lookup("wind enchantment") || sn == skill_lookup("flame enchantment") || sn == skill_lookup("water enchantment") || sn == skill_lookup("earth enchantment") || sn == skill_lookup("lightning enchantment") || sn == skill_lookup("saint enchantment") || sn == skill_lookup("dark enchantment")) {
            if (!obj) {
                send_to_char("你不能這麼做.\n\r", ch);
                return;
            }
    ...
    case TAR_CHAR_SELF:
        if (sn == skill_lookup("teleport") || sn == skill_lookup("word of recall") || sn == skill_lookup("dispel magic"))
            return;
        vo = (void *)ch;
        break;
    ...
    if (!(skill_table[sn].target == TAR_OBJ_INV)) {
        ...
        if (IS_SET(victim->special, ANTI_MAGIC) && ch != victim && ch->class != WIZARD) {
            act("你的魔法對$N起不了作用，看來他可能是魔法免疫.", ch, NULL, victim, TO_CHAR);
            return;
        }
        if (IS_AFFECTED(victim, AFF_MYSTIC_ARMOR) && ch != victim && (sn == gsn_mute || sn == skill_lookup("remove curse") || sn == skill_lookup("dispel magic") || sn == gsn_seal_magic || sn == gsn_charm_person || sn == gsn_blindness || sn == gsn_fear || sn == gsn_paralyze || sn == gsn_disease || sn == gsn_poison || sn == gsn_faerie_fire || sn == gsn_tardy || sn == gsn_sleep || sn == gsn_weaken || sn == gsn_curse || sn == gsn_polymorph || sn == gsn_crushing_hand || sn == gsn_soulsteal)) {
...
void dm_effect(CHAR_DATA *ch) {
    ...
    for (obj = ch->carrying; obj; obj = obj_next) {
        ...
        if ((CAN_WEAR(obj, ITEM_WIELD) && !IS_SET(race_table[ch->race].race_abilities, RACE_WEAPON_WIELD)) || (!IsBeholderCanWear(obj) && IS_SET(race_table[ch->race].race_abilities, RACE_BEHOLDER))) {
            act("你被 $p 給排斥了.", ch, obj, NULL, TO_CHAR);
            act("$n 被 $p 排斥了.", ch, obj, NULL, TO_ROOM);
            unequip_char(ch, obj);
            continue;
        }
        if (!IS_SET(race_table[ch->race].race_abilities, RACE_BEHOLDER) && (get_eq_char(ch, WEAR_FINGER_3) || get_eq_char(ch, WEAR_FINGER_4) ||
    ...
    for (vch = ch->in_room->people; vch; vch = vch_next) {
        ...
        if (vch->in_room == ch->in_room && !is_same_group(ch, vch)) {
            if (IS_NPC(ch)) {
                ...
                if (IS_AFFECTED(vch, sn) || IS_SET(vch->special, NO_PARA) || IS_SET(race_table[vch->race].race_abilities, RACE_NO_POISON) || number_percent() > chance || number_percent() > get_skill_percent(ch, sn))
                    continue;
...
void spell_general_purpose(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (gch = ch->cooperator; gch; gch = gch->next_in_coop) {
        if (ch->in_room != gch->in_room)
            continue;
        if (gch->move < gch->max_move / 15)
            continue;
        if (gch->mana < gch->max_mana / 15)
            continue;
        gch_count++;
        if (IS_GOOD(ch)) {
            if (IS_GOOD(gch))
    ...
    for (vgch = victim->in_room->people; vgch; vgch = vgch->next_in_room) {
        if (!is_same_group(victim, vgch))
            continue;
        if (victim->in_room != vgch->in_room)
            continue;
        vgch_count++;
        if (vgch_count > 4)
            break;
    ...
    if (is_affected(ch, gsn_mental_empower))
        dam = dam * 1.1;
...
void spell_shocking_grasp(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((saves_spell(ch->pp, victim) / 2 + nSklv * 2 + WIS_DIFF(ch, victim)) * weakness_percent(victim, sn) / 97 > number_percent())
        disarm(ch, victim);
...
void spell_create_sound(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (vch = ch->in_room->people; vch; vch = vch->next_in_room) {
        if (!is_name(speaker, vch->name))
            send_to_char(saves_spell(ch->pp, vch) < number_percent() ? buf2 : buf1, vch);
    }
    return;
}
...
void spell_death_field(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (vch = ch->in_room->people; vch; vch = vch_next) {
        ...
        if (!IS_NPC(ch) ? IS_NPC(vch) : IS_NPC(vch)) {
            ...
            if (saves_spell(ch->pp, vch) && (level <= vch->level + 5 && level >= vch->level - 5)) {
...
void spell_domination(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(ch, AFF_CHARM) || level < victim->level || saves_spell(ch->pp, victim) < number_percent())
        return;
    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if (IS_NPC(ch)) // NPC Casting
        {
            ...
            if (IS_AFFECTED(ch, AFF_SUMMONED) || ch->master) {
                if (victim->fighting == ch->master || victim->fighting == ch || (IS_NPC(victim) && !IS_AFFECTED(victim, AFF_CHARM) && !IS_AFFECTED(victim, AFF_SUMMONED))) {
        ...
        } else if (victim->fighting == ch || (IS_NPC(victim) && !IS_AFFECTED(victim, AFF_CHARM) && !IS_AFFECTED(victim, AFF_SUMMONED))) { //  PPL Casting
            dam = basic_dam * weakness_damage(victim, sn) / 100;
            dam = dam * (new_saves_spell(ch->pp, victim, MAGIC_WIND)) / 100;
...
void spell_magic_missile(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    while (times) {
        ...
        if (victim->position == POS_DEAD || victim->in_room != chk_room || victim->in_room != ch->in_room)
            return;
...
void spell_wind_screaming(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!victim->deleted && victim->position != POS_DEAD &&
        victim->in_room == ch->in_room && !(IS_NPC(victim) && IS_SET(victim->act, ACT_SENTINEL)) && old_saves_spell(ch->pp, victim, MAGIC_WIND) - 20 > number_percent()) {
        ...
        for (timer = 0; timer < 300; timer++) {
            ...
            if (pRoomIndex) {
                if (pRoomIndex->area->recall != victim->in_room->area->recall)
                    continue;
                if (!IS_SET(pRoomIndex->room_flags, ROOM_PRIVATE) && !IS_SET(pRoomIndex->area->area_flags, AREA_CHANGED) && !IS_SET(pRoomIndex->area->area_flags, AREA_BUILDING) && !IS_SET(pRoomIndex->area->area_flags, AREA_CLAN) && !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) && !IS_SET(pRoomIndex->room_flags, ROOM_NO_TELEPORT_TO))
                    break;
            }
        }
