    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if ((IS_NPC(ch) && (victim->fighting == ch || !IS_NPC(victim))) ||
            ...
            if (((UMAX(get_curr_int(ch) - get_curr_wis(victim), 0) + 5) * 4 +
                 (UMAX(saves_spell(ch->pp, victim), 0) >> 1) + sklv * 5) > number_percent() * 100 / weakness_percent(victim, sn)) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if ((IS_NPC(ch) && (victim->fighting == ch || !IS_NPC(victim))) ||
            ...
            if (((UMAX(get_curr_int(ch) - get_curr_wis(victim), 0) + 5) * 4 +
                ...
                if (isAffectedByIllusion(victim, sn))
                    continue;
...
void spell_ghosts_celebration(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if ((IS_NPC(ch) && (victim->fighting == ch || !IS_NPC(victim))) || (!IS_NPC(ch) && (victim->fighting == ch || (IS_NPC(victim) && !IS_AFFECTED(victim, AFF_CHARM) && !IS_AFFECTED(victim, AFF_SUMMONED))))) {
            if (!IS_AWAKE(victim) || (!IS_NPC(victim) && IS_SET(victim->act, PLR_WIZINVIS)))
                continue;
            ...
            if (((UMAX(get_curr_int(ch) - get_curr_wis(victim), 0) + 5) * 4) +
                ...
                act("" HIB "四周突然爆出許多幽魂的哭泣及交談聲" NOR "!", ch, NULL, victim, TO_VICT);
                ...
                if (!IS_NPC(ch))
                    dam = dice(20 + WIS_DIFF(ch, victim),
                               level + sklv);
                else
                    dam = dice(15 + WIS_DIFF(ch, victim),
                               level);
