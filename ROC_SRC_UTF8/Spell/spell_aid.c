...
void spell_blindness(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_SET(victim->special, NO_BLIND) || IS_SET(race_table[victim->race].race_abilities, RACE_BEHOLDER) || (is_affected(victim, gsn_invoke) && get_skill_level(victim, gsn_invoke) > 2) || number_percent() > chance)
...
void spell_paralyze(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((IS_SET(victim->special, NO_PARA)) || (victim->race == race_lookup("Vampire")) || (victim->race == race_lookup("Undead")) || (IS_AFFECTED(victim, AFF_GHOUL)) // 轉變為不死生物時等同不死系(undead) 2022/06/15
        || (is_affected(victim, gsn_invoke) && get_skill_level(victim, gsn_invoke) > 3) || (number_percent() > chance)) {
        send_to_char("你失敗了.\n\r", ch);
        return;
    }
...
void spell_charm_person(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(ch, AFF_CHARM) || ch->level + get_skill_level(ch, sn) < victim->level || saves_spell(ch->pp, victim) < (number_percent() * 100 / weakness_percent(victim, sn)))
        return;
    ...
    if (IS_NPC(ch)) {
        ...
        if (ch->class == SORCERER) {
            ...
            for (rch = ch->cooperator; rch; rch = rch->next_in_coop) {
                if (!IS_NPC(rch) && rch != ch && rch->class == SWORDSMAN && IS_AWAKE(rch)
                    // ch 也需 cooperate rch , 能指定配合者 2021/11/23
                    //&& is_same_group( ch, rch )
                    //&& rch->pcdata->autocombat == AUTOC_COOPERATE)
                    && (ch->in_room == rch->in_room) && (ch->cooperating == rch)) {
                    swd = rch;
                    break;
                }
            }
...
void spell_fear(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (chance < number_percent())
        return;
    if (is_affected(victim, gsn_will_fortress) && number_percent() < -(affected_value(victim, gsn_will_fortress, APPLY_SAVING_ILLUSION) - sklv * 4)) {
...
void spell_sleep(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_AFFECTED(victim, AFF_SLEEP) || ch->level < victim->level - sklv
        //      || (saves_spell( ch->pp, victim )+sklv + sklv/2 < (number_percent() * 100 / weakness_percent(victim, sn)))
        || chance < number_percent() || victim->race == race_lookup("Vampire") || IS_SET(victim->special, NO_SLEEP) || IS_IMMORTAL(victim)) {
        send_to_char("你失敗了.\n\r", ch);
        return;
    }
...
void spell_weaken(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_AFFECTED(victim, AFF_WEAKEN)) {
        if (is_affected(victim, intWeaken)) {
            if (sklv > 2 && (affected_value(victim, sn, APPLY_DAMROLL) > (get_damroll(victim, WEAR_WIELD) / (sklv - 11)) * weakness_damage(victim, sn) * 0.01)) {
