...
void spell_destroy_cursed(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (obj = victim->carrying; obj; obj = obj_next) {
        ...
        if (IS_SET(obj->extra_flags, ITEM_NODROP) && obj->wear_loc == WEAR_NONE) {
...
void spell_remove_alignment(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!IS_SET(obj->extra_flags, ITEM_EVIL) && !IS_SET(obj->extra_flags, ITEM_ANTI_GOOD) && !IS_SET(obj->extra_flags, ITEM_ANTI_EVIL) && !IS_SET(obj->extra_flags, ITEM_ANTI_NEUTRAL)) {
    ...
    if (!IS_NPC(ch) && number_percent() + 15 < get_skill_percent(ch, sn) * // ch->pcdata->learned[sn] *
                                                   (33 +
                                                    (33 * (ch->level - obj->level) / (float)LEVEL_HERO)) /
                                                   100.0) {
...
void spell_summon(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (room_is_private(victim, ch->in_room) || victim->in_room->area != ch->in_room->area || IS_SET(victim->in_room->room_flags, ROOM_SAFE) || IS_SET(victim->in_room->room_flags, ROOM_SUMMON_BIT) || IS_SET(ch->in_room->room_flags, ROOM_SUMMON_BIT)) {
        send_to_char("你失敗了.\n\r", ch);
        return;
    }

    // 角色狀態判定 2022/12/20
    if (victim == ch || !victim->in_room || (!IS_NPC(victim) && !victim->desc) // 連線狀態判定加入 pc 限定 2022/12/20
        || (get_skill_level(ch, sn) < 5 && IS_AFFECTED(victim, AFF_CURSE)) || victim->level > ch->level + get_skill_level(ch, sn) || victim->fighting) {
        send_to_char("你失敗了.\n\r", ch);
        return;
    }
...
void spell_teleport(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!victim->in_room || !victim->desc || IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL) || IS_SET(victim->in_room->room_flags, ROOM_NO_TELEPORT) || (!IS_NPC(ch) && victim->fighting) || (victim != ch && (new_saves_spell(ch->pp, victim, MAGIC_WIND) + get_skill_level(ch, sn) < (number_percent() * 100 / weakness_percent(victim, sn))))) {
        send_to_char("你失敗了.\n\r", ch);
        return;
    }
    ...
    for (times_of_search = 0; times_of_search < TIMES_LIMIT; times_of_search++) {
        ...
        if (pRoomIndex) {
            ...
            if (!IS_SET(pRoomIndex->room_flags, ROOM_PRIVATE)
                ...
                && !IS_SET(pRoomIndex->room_flags, ROOM_NO_TELEPORT_TO) && !IS_SET(pRoomIndex->area->area_flags, AREA_CHANGED) && !IS_SET(pRoomIndex->area->area_flags, AREA_BUILDING) && !IS_SET(pRoomIndex->area->area_flags, AREA_CLAN) && pRoomIndex->area->recall == victim->in_room->area->recall)
                break;
        }
    }
...
void do_ammo(CHAR_DATA *ch, char *argument) {
    ...
    for (gem = ch->carrying; gem; gem = gem->next_content) {
        if (gem->wear_loc == WEAR_NONE && can_see_obj(ch, gem) && (gem->pIndexData == get_obj_index(OBJ_VNUM_POWER_GEM))) {
            GemCount++;
            break;
        }
    }
...
void do_abstraction(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && (IS_AFFECTED(ch, AFF_CHARM) || IS_AFFECTED(ch, AFF_SUMMONED))) || (!IS_NPC(ch) &&
...
void do_power(CHAR_DATA *ch, char *argument) {
    ...
    if ((IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM)) || (!IS_NPC(ch) && number_percent() > get_skill_percent(ch, gsn_abstraction))) {
        send_to_char("你試著將魔晶石中的法力引發出來，但失敗了。\n\r", ch);
        extract_obj(obj);
        return;
    }
...
OBJ_DATA *get_powergem_carry(CHAR_DATA *ch, char *argument) {
    ...
    for (obj = ch->carrying; obj; obj = obj->next_content) {
        if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && (obj->pIndexData == get_obj_index(OBJ_VNUM_POWER_GEM))) {
            if (++count == number)
                return obj;
        }
    }
    ...
    if (IS_NPC(ch)) {
        ...
        for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
            ...
            if (!is_same_group(ch, rch))
                continue;
            if (rch->pcdata->autocombat != AUTOC_COOPERATE)
                continue;
            if (rch->class == SAVANT)
                break;
        }
...
void spell_evacuate(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!(to_room = get_room_index(ch->in_room->area->recall)) && !(qmark = is_qmark(ch, "evacuate_loc")) == NULL) {
        send_to_char("這裡不能進行傳送.\n\r", ch);
        return;
    }

    if ((get_skill_level(ch, sn) < 5 && IS_SET(ch->in_room->room_flags, ROOM_NO_RECALL)) || (get_skill_level(ch, sn) < 4 && IS_SET(ch->in_room->room_flags, ROOM_NO_TELEPORT))) {
    ...
    for (gch = ch->in_room->people; gch; gch = gch_next) {
        ...
        if (gch == ch)
            continue;
        if (IS_NPC(gch))
            continue; // Added by Razgriz	20050831
...
void spell_magic_enchantment(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    */
    ...
    if (!CAN_WEAR(obj, ITEM_WIELD) && !CAN_WEAR(obj, ITEM_TWO_HAND) && !CAN_WEAR(obj, ITEM_WEAR_HANDS) && !CAN_WEAR(obj, ITEM_WEAR_FEET) && !(obj->item_type == ITEM_ARROW) && !(obj->item_type == ITEM_DART) && !(obj->item_type == ITEM_AMMO)) // item ammo 2022/04/30
    {
        send_to_char("這件物品無法附魔.\n\r", ch);
        return;
    }
