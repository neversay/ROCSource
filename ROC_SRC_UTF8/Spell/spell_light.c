...
void spell_dispel_evil(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (victim)
        ali_diff = ch->alignment - victim->alignment;
    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if (IS_NPC(ch)) {
            if (victim->fighting == ch || !IS_NPC(victim)) {
                ...
                if ((IS_AFFECTED(victim, AFF_SANCTUARY) || IS_SET(race_table[victim->race].race_abilities, RACE_SANCT)) && number_percent() < (0 - NEW_GET_SVS(victim, MAGIC_SAINT))) {
...
void spell_sun_flash(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (sn == gsn_sun_slash)
        nSklv = get_skill_level(ch, gsn_sun_flash);
    ...
    for (vch = ch->in_room->people; vch; vch = vch_next) {
        ...
        if (!is_same_group(ch, vch) && ((IS_NPC(ch) && (vch->fighting == ch || !IS_NPC(vch))) || (!IS_NPC(ch) && (vch->fighting == ch || (IS_NPC(vch) && !IS_AFFECTED(vch, AFF_CHARM) && !IS_AFFECTED(vch, AFF_SUMMONED)))))) {
            ...
            // 帶有聖光術額外減免 2022/10/26
            if ((IS_AFFECTED(vch, AFF_SANCTUARY) || IS_SET(race_table[vch->race].race_abilities, RACE_SANCT)) && number_percent() < (0 - NEW_GET_SVS(vch, MAGIC_SAINT))) {
            ...
            if (nChance > (number_percent() + 2)) {
                ...
                if (IS_AFFECTED(vch, AFF_BLIND) || IS_SET(vch->special, NO_BLIND) || number_percent() > nDmgRate * (10 + nSklv) / 20)
                    continue;
