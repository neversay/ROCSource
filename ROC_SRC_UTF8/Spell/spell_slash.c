...
void spell_icecore_slash(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (ch->damroll > 50)
        dr_curve += (ch->damroll - 50) / 4;
    if (ch->damroll > 60)
        dr_curve += (ch->damroll - 60) / 3 - (ch->damroll - 50) / 4;
    if (ch->damroll > 70)
        dr_curve += (ch->damroll - 70) / 2 - (ch->damroll - 60) / 3;
...
void spell_sun_slash(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_NPC(ch) && nBlindBase < 20)
        nBlindBase = 20;

    for (vch = ch->in_room->people; vch; vch = vch_next) {
        vch_next = vch->next_in_room;
        if (vch == ch || vch->deleted || vch->position == POS_DEAD)
            continue;
        if (!IS_NPC(vch) && IS_SET(vch->act, PLR_WIZINVIS))
            continue;
        ...
        if (!is_same_group(ch, vch) && ((IS_NPC(ch) && (vch->fighting == ch || !IS_NPC(vch))) || (!IS_NPC(ch) && (vch->fighting == ch || (IS_NPC(vch) && !IS_AFFECTED(vch, AFF_CHARM) && !IS_AFFECTED(vch, AFF_SUMMONED)))))) {
            ...
            nDamage = nDamage * (1 * new_saves_spell(ch->pp, vch, MAGIC_SPELL) + (1 + nCurrStr * nCurrStr / 300) * new_saves_spell((ch->apdr * 2) / 9, vch, MAGIC_SAINT)) / 350;
            ...
            if (!IS_AFFECTED(vch, AFF_MYSTIC_ARMOR)) {
                if (weather_info.sky < SKY_RAINING || IS_NPC(ch)) {
                    if (affected_value(vch, skill_lookup("day of brightness"), APPLY_DOB_SAVING_FLASH) > number_percent()) {
                        ...
                        if (!IS_OUTSIDE(ch) && !IS_NPC(ch))
                            nBlindRate = nBlindRate * 2 / 3;
                        if (number_percent() < nBlindRate) {
                            if (!(IS_AFFECTED(vch, AFF_BLIND) || IS_SET(vch->special, NO_BLIND)) && number_percent() < nBlindBase) {
...
void spell_elemental_slash(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (nFireSklv > 4)
        ex_pp += UMAX(ch->pp - 30, 0);
    if (nFireSklv > 3)
        ex_pp += UMAX(ch->pp - 40, 0);
    if (nFireSklv > 2)
        ex_pp += UMAX(ch->pp - 50, 0);
    ...
    if (victim == NULL || victim->deleted || victim->in_room != ch->in_room)
        return;
...
void spell_earth_slash(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    while (times--) {
        ...
        for (victim = ch->in_room->people; victim; victim = victim_next) {
            ...
            if (sklv <= 4 && is_flying(victim) && !IS_AFFECTED(victim, AFF_TARDY))
                continue;
    ...
    if (is_affected(ch, skill_lookup("berserk")) && !IS_NPC(ch) && EXtimes > 0) {
        ...
        while (EXtimes--) {
            ...
            if (victim_temp == NULL || victim_temp->deleted || victim_temp->in_room != ch->in_room)
                return;
