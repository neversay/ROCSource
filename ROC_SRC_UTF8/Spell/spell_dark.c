    ...
    for (victim = ch->in_room->people; victim; victim = victim_next) {
        ...
        if (IS_NPC(ch)) {
            if (victim->fighting == ch || !IS_NPC(victim)) {
                ali_diff = 0;
                if (victim)
                    ali_diff = victim->alignment - ch->alignment;
                dam = 0;
                dam = (40 + ch->level) / 2;
...
void spell_finger_of_death(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((IS_AFFECTED(victim, AFF_SANCTUARY) || IS_SET(race_table[victim->race].race_abilities, RACE_SANCT)) && number_percent() < (0 - NEW_GET_SVS(victim, MAGIC_DARK))) {
...
void spell_lifesteal(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (dam < 0) {
        ...
        // 帶有聖光術額外減免 2022/10/26
        if ((IS_AFFECTED(victim, AFF_SANCTUARY) || IS_SET(race_table[victim->race].race_abilities, RACE_SANCT)) && NEW_GET_SVS(ch, MAGIC_DARK) > NEW_GET_SVS(victim, MAGIC_DARK))
            dam = (dam * 2) / 3;
        if (!isNeu)
...
void spell_soulsteal(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    // inborn DETECT_ALIGN 的 race (fairy , gnome) 有額外加成 2020/05/29
    if (IS_SET(race_table[ch->race].race_abilities, RACE_DETECT_ALIGN) && get_skill_level(ch, gsn_inborn_ability)) {
        if (nAffNum > 0) {
...
void spell_energy_drain(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (victim->level <= 10) {
        ...
        if (!IS_AFFECTED(victim, AFF_MYSTIC_ARMOR)) {
            ...
            // affect by no heal wouldn't gain hitpoint add at 2018.11.20
            if (nHpDmg > 0 && number_percent() < nSklv * 5 + get_curr_int(ch) - get_curr_con(victim) * 2 && !IS_AFFECTED(ch, AFF_NO_HEAL)) {
