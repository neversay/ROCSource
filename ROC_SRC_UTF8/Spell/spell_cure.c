...
void spell_calm(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    for (victim = ch->in_room->people;
        ...
        if (is_same_group(ch, victim)) {
            if ((get_curr_wis(ch) * 2 + get_curr_int(ch) + ch->pp + sklv * (4 + sklv)) * weakness_percent(victim, sn) > number_percent() * 100) {
                isCalmed = TRUE;
            }
        } else if ((WIS_DIFF(ch, victim) + INT_DIFF(ch, victim) + 4) * 7 + saves_spell(ch->pp, victim) + sklv * 8 > (number_percent() * 105 / weakness_percent(victim, sn))) {
            isCalmed = TRUE;
        }
        if (isCalmed) {
...
void spell_cure_critical(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (IS_NPC(ch)) {
        ...
        if (IS_GOOD(ch)) {
            ...
            if (IS_EVIL(victim))
                heal = heal * 5 / 4;
            else if (IS_GOOD(victim) && ch->class != BISHOP)
                heal = heal * 3 / 4;
        }
    }
    // 修改 blood thirsty 設定 , 故移除降低補血 2022/02/21
...
void spell_cure_poison(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!is_affected(victim, gsn_poison) && !is_affected(victim, gsn_venomous_strike) && !is_affected(victim, gsn_gas_breath))
        return;
...
void spell_heal(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (get_curr_wis(ch) > 16)
        heal += heal * (get_curr_wis(ch) - 16) * (get_curr_wis(ch) - 16) / 200; // lv 41 , wis 23 , 164
    else
    ...
    if (heal < 50)
        heal = 50; // 給予 heal 基本值 2020/10/22

    /* Keric Modified for different alignment will affect the heal */
    // 加入BISHOP 不受陣營負面影響 2020/10/22
    if (IS_NPC(ch)) { /* NPC Will not be affected */
        ;
    } else {
        if (IS_GOOD(ch)) {
            if (IS_GOOD(victim))
        ...
        */
        ...
        if (IS_NPC(ch)) {
            ...
            if (IS_GOOD(ch)) {
                ...
                if (IS_EVIL(gch))
                    nFinalHeal = nFinalHeal * 5 / 4;
                else if (IS_GOOD(gch) && ch->class != BISHOP)
                    nFinalHeal = nFinalHeal * 3 / 4;
            }
        }
