...
char *mprog_type_to_name(int type) {
    ...
    case KILL_PROG:
        return "kill_prog";
    case ENTRY_GREET_PROG:
        return "entry_greet_prog"; // add at 2021/12/18
    default:
        return "ERROR_PROG";
    }
}
...
void check_position(CHAR_DATA *ch, CHAR_DATA *victim, int dam) {
    ...
    case POS_STUNNED:
        if (!is_chinese(victim, NULL)) {
            ...
            act("$n 昏倒了，過一會也許會回復.", victim, NULL, NULL, TO_ROOM);
        }
        break;
    ...
    default:
        if (dam > victim->max_hit / 4)
            send_to_char((is_chinese(victim, NULL)) ? HIY "天哪! 痛死人了!" NOR "\n\r" : HIY "That really did HURT!" NOR "\n\r",
                         victim);
        if (victim->hit < victim->max_hit / 4)
            send_to_char((is_chinese(victim, NULL)) ? HIR "你正在噴血!" NOR "\n\r" : HIR "You sure are BLEEDING!" NOR "\n\r",
                         victim);
        break;
    }
    return;
...
void do_mastat(CHAR_DATA *ch, char *argument) {
    ...
    sprintf(buf, "Short description: %s.\n\rLong  description: %s",
            target->short_descr,
            target->long_descr[0] != '\0' ? target->long_descr : "(none).\n\r");
    send_to_char(buf, ch);
