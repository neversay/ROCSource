...
//*  PURPOSE     :         修正 當玩家檔案錯誤時會發生二轉的 BUG  *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940725 Jye                                    *
//*  PURPOSE     :         ReSet Sor Skill 並 拿掉舊技能          *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940918 Razgriz                                *
//*  PURPOSE     :         加入 extra_value 的處理到下面函數中    *
...
void fwrite_char(CHAR_DATA *ch, FILE *fp) {
    ...
    if ((ch->in_room && IS_SET(ch->in_room->room_flags, ROOM_SAVE)) || (ch->in_room && !str_cmp(ch->in_room->area->identity, "skarabrae")) || IS_IMMORTAL(ch)) {
        fprintf(fp, "Room        %d\n", ch->in_room->vnum);
...
bool load_char_obj(DESCRIPTOR_DATA *d, char *name) {
    ...
    char         sorry_player[] =
        "********************************************************\n\r"
        "** 您的角色存檔發生損壞, 請於 BBS 上回報問題          **\n\r"
        "** 或使用其他角色 note to IMMORTAL 回報回報問題,      **\n\r"
        "** 並靜候修正. 造成您的不便, 深感抱歉.                **\n\r"
        "**                                ROC 全體工作人員    **\n\r"
        "********************************************************\n\r";
    char sorry_object[] =
        "********************************************************\n\r"
        "** 您角色的持有物品發生問題, 可能造成物品損失.        **\n\r"
        "** 請使用其他角色 note to IMMORTAL 回報回報問題.      **\n\r"
        "** 或向線上 IMM 回報. 謝謝.                           **\n\r"
        "********************************************************\n\r";
    ...
    ch->last_note      = 0;
    ch->act            = PLR_BLANK | PLR_COMBINE | PLR_PROMPT | PLR_AUTOEXIT
              //| PLR_REPLACE
              | PLR_COMBAT | PLR_CHINESE | PLR_STATUS | PLR_ARTIFACT | PLR_ASSIST | PLR_NOTIFY | PLR_AUTOGOLD;
...
int fread_char(CHAR_DATA *ch, FILE *fp) {
    ...
    char  def_prompt[]     = "[m<生命[1;32m%h[m魔法[1;33m%m[m移動[1;35m%v[m> ";
    ...
    struct key_data key_tab[] =
        {
            ...
            {"HpMnMv", FALSE, MAND, {&ch->hit, &ch->max_hit, &ch->mana, &ch->max_mana, &ch->move, &ch->max_move, NULL}},
            ...
            {"Bmfin", TRUE, DEFLT, {&ch->pcdata->bamfin, NULL}},
            {"Bmfout", TRUE, DEFLT, {&ch->pcdata->bamfout, NULL}},
            {"Immskll", TRUE, DEFLT, {&ch->pcdata->immskll, NULL}},
            {"Ttle", TRUE, (int)&def_title, {&ch->pcdata->title, NULL}},
            {"Rank", FALSE, 0, {&ch->pcdata->rank, NULL}},
            {"AtrPrm", FALSE, MAND, {&ch->perm_str, &ch->perm_int, &ch->perm_wis, &ch->perm_dex, &ch->perm_con, NULL}},
            {"AtrMd", FALSE, MAND, {&ch->mod_str, &ch->mod_int, &ch->mod_wis, &ch->mod_dex, &ch->mod_con, NULL}},
            {"Cond", FALSE, DEFLT, {&ch->pcdata->condition[0], &ch->pcdata->condition[1], &ch->pcdata->condition[2], NULL}},
            ...
            {"ChSize", FALSE, 0, {&ch->mod_size, NULL}}, // 體型 2022/12/23
        ...
        if (*key_tab[i].key) /* Key entry found in key_tab */
        {
            ...
            /* Entry is an integer */
                ...
                for (j = 0; key_tab[i].ptrs[j]; j++) {
                    ...
                    if (!status) {
                        if (str_cmp(key_tab[i].key, "AffdBy") && (str_cmp(key_tab[i].key, "Act")))
            ...
            if (ch->pcdata->version < 11 && ch->class == SORCERER) {
                // 拿掉old sor skill
                if (!str_cmp(sbuf, "gate") || !str_cmp(sbuf, "water spirit") || !str_cmp(sbuf, "fire elemental") || !str_cmp(sbuf, "clay golem"))
                    i = 0;
                else
                    i %= 1000;
            }
            ...
            if (ch->pcdata->version < 20 && (ch->class == NINJA || ch->class == WARLORD || ch->class == KNIGHT) && ch->pcdata->prime_attr == APPLY_CON)
                ch->in_room = get_room_index(11000);
            ...
            if (is_qmark(ch, p) != NULL) {
                ...
                if (ch->pcdata->version < 5) {
                    if (!str_cmp(p, "immhall_for_skillexp") || !str_cmp(p, "immhall_for_skillexp2")) {
                        qmark->v0 = 0;
                        qmark->v1 = 0;
                        qmark->v2 = 0;
                    }
            ...
            if (!cn) {
                if (!ch->desc)
                    ...
                    write_to_buffer(ch->desc, "您的幫派已經不存在了。\n\r", 0);
                    // ch->pcdata->respect --;
    ...
    if (ch->pcdata->version < 36 && ((ch->pcdata->learned[gsn_dual] > 1000) || (ch->pcdata->learned[gsn_dodge] > 1000) || (ch->pcdata->learned[gsn_block] > 1000) || (ch->pcdata->learned[gsn_parry] > 1000) || (ch->pcdata->learned[gsn_second_attack] > 1000) || (ch->pcdata->learned[gsn_third_attack] > 1000) || (ch->pcdata->learned[gsn_fourth_attack] > 1000) || (ch->pcdata->learned[gsn_fifth_attack] > 1000) || (ch->pcdata->learned[gsn_disarm] > 1000) || (ch->pcdata->learned[gsn_barehand] > 1000) || (ch->pcdata->learned[gsn_bash] > 1000) || (ch->pcdata->learned[gsn_fool] > 1000) || (ch->pcdata->learned[gsn_blind_fight] > 1000) || (ch->pcdata->learned[gsn_bandage] > 1000) || (ch->pcdata->learned[gsn_rescue] > 1000) || (ch->pcdata->learned[gsn_morphing] > 1000) || (ch->pcdata->learned[gsn_scrolls] > 1000) || (ch->pcdata->learned[gsn_staves] > 1000) || (ch->pcdata->learned[gsn_wands] > 1000))) {
        int sn = 0;
        for (sn = 0; sn < MAX_SKILL; ++sn) {
            ch->pcdata->learned[sn] %= 1000;
        }
    ...
    if (ch->pcdata->version < 37 && ((ch->pcdata->learned[gsn_throwing] > 1000) || (ch->pcdata->learned[gsn_acid_breath] > 1000) || (ch->pcdata->learned[gsn_fire_breath] > 1000) || (ch->pcdata->learned[gsn_frost_breath] > 1000) || (ch->pcdata->learned[gsn_gas_breath] > 1000) || (ch->pcdata->learned[gsn_lightning_breath] > 1000))) {
        int sn = 0;
        for (sn = 0; sn < MAX_SKILL; ++sn) {
            ch->pcdata->learned[sn] %= 1000;
        }
    ...
    if (ch->pcdata->version < 26) {
        if (ch->class >= WIZARD) {
            ...
            send_to_char("你因為轉職得到 10 萬點技能經驗點！\n\r", ch);
        }
    }
    ...
    struct key_data key_tab[] =
        {
            ...
            {"Values", FALSE, MAND, {&obj.value[0], &obj.value[1], &obj.value[2], &obj.value[3], NULL}},
            {"\0", FALSE, 0}
    };
...
void do_backup(CHAR_DATA *ch, char *argument) {
    ...
    if (ch->level < 5) {
        send_to_char("[33m你最少要到第五級以上才能備份。[m\n\r", ch);
        if (switched)
            ch = ch->pcdata->horse;
        return;
    }

    // 新增死亡後找回屍體前一段時間內無法 backup 2022/10/10
    if (is_qmark(ch, "death_stop_backup")) {
        send_to_char("趕緊找回你的屍體和物品，才能進行 backup 的動作。\n\r", ch);
        return;
    }
...
CHAR_DATA *load_horse_obj(DESCRIPTOR_DATA *d, char *name) {
    ...
    ch->pcdata->prompt = str_dup(daPrompt);
    ch->last_note      = 0;
    ch->act            = PLR_BLANK | PLR_COMBINE | PLR_PROMPT | PLR_AUTOEXIT | PLR_ASSIST
              //| PLR_REPLACE
              | PLR_CHINESE | PLR_AUTOGOLD | PLR_HORSE;
    ch->pcdata->pwd                    = str_dup("");
    ch->pcdata->bamfin                 = str_dup("");
    ch->pcdata->bamfout                = str_dup("");
    ch->pcdata->immskll                = str_dup("");
