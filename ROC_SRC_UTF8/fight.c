    ...
    if (affected_value(ch, gsn_blood_thirsty, APPLY_SAVING_LIFE)) {
        if (affected_value(ch, gsn_blood_thirsty, APPLY_DAMAGE_COUNT) > 0) {
            ch->hit = UMIN(ch->max_hit,
                           ch->hit + 10 + affected_value(ch, gsn_blood_thirsty, APPLY_DAMAGE_COUNT) / 10 * (3 + get_skill_level(ch, gsn_blood_thirsty) * 2));
        }
...
void violence_update(void) {
    ...
    for (ch = char_list; ch; ch = ch->next) {
        ...
        if ((victim = ch->fighting)) {
            ...
            if (IS_SET(race_table[ch->race].race_abilities, RACE_VAMPIRE) && (time_info.hour < 6 || time_info.hour > 18) && !IS_AFFECTED(ch, AFF_NO_HEAL)) {
                if ((!IS_NPC(ch) && !IS_NPC(victim)) || (!IS_NPC(victim) && victim->pcdata->clanpk && IS_SET(victim->in_room->room_flags, ROOM_PK)))
            ...
            */
            ...
            if (ch != victim && !IS_AFFECTED(ch, AFF_NO_HEAL) // no heal wouldn't regeneracy add at 2018.11.20
                ...
                && (IS_SET(race_table[ch->race].race_abilities, RACE_NO_POISON) || IS_SET(ch->special, NO_POISON) || (!IS_AFFECTED(ch, AFF_POISON)) || (IS_AFFECTED(ch, AFF_POISON) && (NEW_GET_SVS(ch, MAGIC_POISON) <= ch->level))) // poison 基本值為角色 level
                && (IS_SET(race_table[ch->race].race_abilities, RACE_REGENERACY) || (IS_NPC(ch) && IS_SET(ch->act, ACT_REGENERACY)) || (!IS_NPC(ch) && ch->pcdata->learned[gsn_regeneracy] > 0))) {
                if (IS_NPC(ch)) {
                    ch->hit += ch->level * 10;
                    if (ch->hit > ch->max_hit)
                        ch->hit = ch->max_hit;
            ...
            if (!ch->fighting || victim->deleted || ch->in_room != victim->in_room || IS_SET(ch->in_room->room_flags, ROOM_SAFE))
                continue;
            ...
            */
            ...
            if (IS_AWAKE(ch) && !victim->deleted && (victim = ch->fighting)) { // victim may change
                /* Ok here we test for switch if victim is charmed */
                if ((IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(victim, AFF_SUMMONED) || (IS_NPC(victim) && victim->clan)) && victim->master && victim->in_room && victim->in_room == victim->master->in_room && victim->master != ch && number_percent() > 30)
        ...
        */
        ...
        for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
            if ((ch == rch) || rch->deleted || !IS_AWAKE(rch) || !(rch->fighting))
                continue;
            ...
            if (is_qmark(ch, "enable_mob_assist")) {
                if (IS_NPC(ch) && !IS_AFFECTED(ch, AFF_CHARM) && !IS_AFFECTED(ch, AFF_SUMMONED) && !ch->clan && !IS_SET(ch->act, ACT_PET) && IS_NPC(rch) && !IS_AFFECTED(rch, AFF_CHARM) && !IS_AFFECTED(rch, AFF_SUMMONED) && !rch->clan && !IS_SET(rch->act, ACT_PET)) {
                    mobfighting = TRUE;
                    break;
                }
            }
        ...
        if (mobfighting) {
            ...
            if ((rch->pIndexData != ch->pIndexData && number_bits(3) != 0) || abs(victim->level - ch->level) > 5)
        ...
        if ((IS_NPC(ch) && !is_same_group(ch, victim)) || (!IS_NPC(ch) && IS_SET(ch->act, PLR_ASSIST))) {
            if (!ch->fighting) {
...
void update_pos(CHAR_DATA *victim) {
    ...
    if (victim->hit > 0) {
        if (victim->position <= POS_STUNNED)
            ...
            victim->position = POS_STANDING;
        else if (victim->position == POS_FIGHTING && (!victim->fighting || victim->fighting->in_room != victim->in_room))
            victim->position = POS_STANDING;
        return;
    }
...
void init_char_data(CHAR_DATA *ch) {
    ...
    if (IS_SET(race_table[ch->race].race_abilities, RACE_VAMPIRE || ch->class < WIZARD || ch->level < 31)) {
...
bool raw_kill(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!IS_NPC(victim) && ((!IS_NPC(ch) && victim->in_room && IS_SET(victim->in_room->room_flags, ROOM_PK)) || (victim->level < 4 && victim->class < 5) || victim->pcdata->clanpk)) {
        pk_kill(ch, victim);
    ...
    if (IS_NPC(victim)) {
        ...
        cn = ch->clan;
        if (cn && victim->level > ch->level && victim->pIndexData->killed < 4 && IS_SET(victim->act, ACT_AGGRESSIVE))
            cn->construct++;
        if (victim->level > 39 && cn && !victim->pIndexData->pShop && !victim->clan && !IS_SET(victim->act, ACT_PRACTICE) && victim->pIndexData->vnum != cn->kill[0]) {
        ...
        if (victim->clan) {
            victim->clan->hire -= (10 + UMAX(0, victim->level - 35) * 3 + UMAX(0, victim->level - 38) * 5);
            if (victim->spec_fun)
                victim->clan->hire -= 15;
            if (victim->class != 0)
                victim->clan->hire -= 15;
...
void group_gain(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
        ...
        for (obj = gch->carrying; obj; obj = obj_next) {
            ...
            if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(gch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(gch)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(gch))) {
...
int xp_compute(CHAR_DATA *gch, CHAR_DATA *victim) {
    ...
    if (level < -18)
        level = -18;
    if (victim->level < 40 && level < -10)
        level = -10;
    if (gch->class < WIZARD && gch->level < 36)
        level -= 5;
    bonus = 1.0;
    xp    = 100;
    if (IS_NPC(victim))
    ...
    */
    ...
    if (!IS_NPC(gch) || IS_AFFECTED(gch, AFF_SUMMONED) || IS_AFFECTED(gch, AFF_CHARM)) { // npc's alignment is fixed.
    ...
    if (IS_NPC(victim)) {
        ...
        if (IS_SET(victim->special, ALY_SAN))
            bonus += (float)1 / 5;
        if (IS_SET(victim->special, NO_MUTE) || IS_SET(victim->special, NO_SEALMAGIC) || IS_SET(victim->special, NO_CRAZY))
            bonus += (float)1 / 10;
        ...
        if (victim->spec_fun) {
            if (victim->spec_fun == spec_lookup("spec_breath_any") || victim->spec_fun == spec_lookup("spec_cast_psionicist") || victim->spec_fun == spec_lookup("spec_cast_undead") || victim->spec_fun == spec_lookup("spec_breath_gas") || victim->spec_fun == spec_lookup("spec_cast_mage"))
                bonus += (float)1 / 3;

            if (victim->spec_fun == spec_lookup("spec_breath_fire") || victim->spec_fun == spec_lookup("spec_breath_cold") || victim->spec_fun == spec_lookup("spec_breath_acid") || victim->spec_fun == spec_lookup("spec_breath_lightning") || victim->spec_fun == spec_lookup("spec_cast_cleric") || victim->spec_fun == spec_lookup("spec_cast_judge") || victim->spec_fun == spec_lookup("spec_cast_ghost"))
                bonus += (float)1 / 5;

            if (victim->spec_fun == spec_lookup("spec_poison") || victim->spec_fun == spec_lookup("spec_thief"))
                bonus += (float)1 / 10;
...
void disarm(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!IS_NPC(victim) && victim->pcdata->learned[gsn_swordmaster] > 0) {
        ...
        if (!IS_NPC(ch)) {
            if ((get_skill_percent(victim, gsn_swordmaster) / 2 + get_skill_level(victim, gsn_swordmaster) * 30 + (victim->class == SWORDSMAN ? get_skill_level(victim, gsn_swordmaster) * 10 : 0) - get_skill_level(ch, gsn_barehand) * 3 - get_skill_level(ch, gsn_disarm) * 3) > number_percent()) {
            ...
            if ((get_skill_percent(victim, gsn_swordmaster) / 2 + get_skill_level(victim, gsn_swordmaster) * 45 + (victim->class == SWORDSMAN ? get_skill_level(victim, gsn_swordmaster) * 10 : 0) - ch->level - get_skill_level(ch, gsn_disarm) * get_skill_level(ch, gsn_disarm)) > number_percent()) {
                if (!is_chinese(ch, victim))
                    act("$N's swordmaster skill prevent from $n's disarm!", ch, NULL, victim, TO_NOTVICT);
                else
                    act("$N 高超的御劍術防止 $n 擊落$S武器！", ch, NULL, victim, TO_NOTVICT);
    ...
    // npc victim 判定
    if (IS_NPC(victim)) {
        if (IS_SET(victim->special, NO_DISARM) || (IS_SET(ch->act, ACT_HARD_DIS) && (victim->level - get_skill_level(ch, gsn_disarm) * 5 - get_skill_level(ch, gsn_barehand) * 5 > number_percent()))) {
            act((!is_chinese(ch, victim)) ? "$N's swordmaster skill prevent from $n's disarm!" : "$N 高超的御劍術防止 $n 擊落$S武器！", ch, NULL, victim, TO_NOTVICT);
            return;
        }
    }
    ...
    if (IS_NPC(ch)) {
        if (!IS_SET(ch->class, 32) // npc class 32(bloody) 等同於 fist fight 效果 2022/02/18
            && !get_eq_char(ch, WEAR_WIELD) && !get_eq_char(ch, WEAR_WIELD_2) && !get_eq_char(ch, WEAR_TWO_HAND)) {
    ...
    obj_from_char(obj);
    if (IS_NPC(victim) || get_skill_level(victim, gsn_swordmaster) >= 3 || IS_SET(obj->extra_flags, ITEM_NODROP)) // item_nodrop 不會掉到地上 2022/02/19
...
void trip(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (is_flying(victim) || victim->position <= POS_RESTING || IS_SET(race_table[victim->race].race_abilities, RACE_LIZARDMAN))
        return;

    if (!IS_NPC(victim) && (victim->class == KNIGHT || victim->class == LORD) && get_eq_char(victim, WEAR_RIDE))
        return; // zcecil
    ...
    if ((!IS_NPC(victim) && (get_skill_level(victim, gsn_bodycontrol) + (get_skill_level(victim, gsn_bodycontrol) > 5 ? get_skill_level(victim, gsn_bodycontrol) : 0) + get_skill_percent(victim, gsn_bodycontrol) + 10 * (get_curr_wis(victim) - 15)) * 3 / 4 > (number_percent() * mod / 100) + ((IS_SET(race_table[ch->race].race_abilities, RACE_LIZARDMAN)) ? 20 : 0)) || (IS_NPC(victim) && IS_SET(victim->special, EXPERT) && number_percent() > ch->level - 20)) {
        if (!is_chinese(ch, victim)) {
            act(HIG "$N's bodycontrol skill prevent from your trip!" NOR, ch, NULL, victim, TO_CHAR);
        } else {
            act(HIG "$N 巧妙地保持住身體的平衡！" NOR, ch, NULL, victim, TO_CHAR);
...
void do_kill(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_NPC(victim)) {
        if (!IS_SET(victim->act, PLR_KILLER) && !IS_SET(victim->act, PLR_THIEF) && !IS_SET(race_table[victim->race].race_abilities, RACE_VAMPIRE) && !IS_SET(victim->in_room->room_flags, ROOM_PK)) {
            send_to_char("請使用 MURDER.\n\r", ch);
            return;
        }
    } else {
    ...
    if (IS_AFFECTED(victim, AFF_CHARM) && victim->master == ch)
        return; // 加入 master 判定 2022/12/31
...
void do_flee(CHAR_DATA *ch, char *argument) {
    ...
    if (get_eq_char(ch, WEAR_RIDE)) {
        if (get_skill_level(ch, gsn_ride) > 4)
            nStun--;
    }

    if (IS_AFFECTED(ch, AFF_FEAR))
        IsFear = 1; // 如果有 fear 狀態則失敗時不會 stun 2022/10/13
    ...
    if (get_eq_char(ch, WEAR_RIDE)) {
        ...
        if (nRideSklv > 0)
            adjust += (nRideSklv + 4) * nRideSklv / 2;
    }
    ...
    if (arg[0] == '\0' || (!IS_NPC(ch) && int_number_percent + 30 > int_escape_percent + sklv * 6)) {
        for (attempt = 0; attempt < 6; attempt++) {
            ...
            if ((pexit = was_in->exit[door]) == 0 || !pexit->to_room || (IS_SET(pexit->exit_info, EX_CLOSED) && !IS_AFFECTED(ch, AFF_PASS_DOOR) && !IS_SET(race_table[ch->race].race_abilities, RACE_PASSDOOR) && !IS_SET(pexit->exit_info, EX_PASSPROOF)) || (IS_NPC(ch) && (IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) || (IS_SET(ch->act, ACT_STAY_AREA) && pexit->to_room->area != ch->in_room->area))))
                continue;
        ...
        if ((pexit = was_in->exit[dir]) == 0 || !pexit->to_room || (IS_SET(pexit->exit_info, EX_CLOSED) && !IS_AFFECTED(ch, AFF_PASS_DOOR) && !IS_SET(race_table[ch->race].race_abilities, RACE_PASSDOOR) && !IS_SET(pexit->exit_info, EX_PASSPROOF)) || (IS_NPC(ch) && (IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) || (IS_SET(ch->act, ACT_STAY_AREA) && pexit->to_room->area != ch->in_room->area)))) {
...
void track_hunt(CHAR_DATA *ch, CHAR_DATA *victim, ROOM_INDEX_DATA *was_in, int door) {
    ...
    for (vch = was_in->people; vch; vch = vch_next_in_room) {
        ...
        if ((IS_NPC(vch) && !IS_SET(vch->act, ACT_TRACK)) || (!IS_NPC(vch) && (vch->tracking != ch || number_percent() > get_skill_percent(vch, gsn_track) - get_skill_level(ch, gsn_escape) * 3)))
            continue;
        if (IS_NPC(vch) && number_percent() < get_skill_level(ch, gsn_escape) * 3)
            continue;
        if (fighting_by(ch, victim, was_in))
            continue;
...
void do_feed(CHAR_DATA *ch, char *argument) {
    ...
    if (IS_NPC(victim)) {
        nHitRate = (nHitRoll + nSklv * (4 + nSklv) * 3 / 4 + GET_DODGE(victim) / 30) * weakness_percent(ch, gsn_vampiric_bite) / 100;
    } else {
        nHitRate = (nHitRoll + nSklv * (4 + nSklv) / 2 + GET_DODGE(victim) / 10) * weakness_percent(ch, gsn_vampiric_bite) / 100;
    }
    ...
    if (get_skill_level(ch, gsn_enhanced_damage) >= 4)
        dam *= 1.05;
    if (get_skill_level(ch, gsn_enhanced_damage) >= 6)
        dam *= 1.10;
...
void do_stake(CHAR_DATA *ch, char *argument) {
    ...
    if (number_percent() < chance || !IS_AWAKE(victim) || victim->race != race_lookup("Vampire")) {
        if (victim->race == race_lookup("vampire")) {
...
void use_magical_item(CHAR_DATA *ch) /*{{{*/
{
    ...
    for (obj = ch->carrying; obj; obj = obj->next_content) {
        if ((obj->item_type == ITEM_SCROLL || obj->item_type == ITEM_WAND || obj->item_type == ITEM_STAFF || obj->item_type == ITEM_PILL) && number_range(0, number) == 0) {
            cobj = obj;
            number++;
        }
    }
...
void trigger_obj_hp(OBJ_DATA *obj, int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (obj->pIndexData->hp_use_class) {
        if ((IS_SET(obj->pIndexData->hp_use_class, ALLOW_BIT) && !IS_SET(obj->pIndexData->hp_use_class, NUM_BIT(ch->class))) || (IS_SET(obj->pIndexData->hp_use_class, DENY_BIT) && IS_SET(obj->pIndexData->hp_use_class, NUM_BIT(ch->class))))
            return;
    }
    if (obj->pIndexData->hp_use_race) {
        if ((IS_SET(obj->pIndexData->hp_use_race, ALLOW_BIT) && !IS_SET(obj->pIndexData->hp_use_race, NUM_BIT(ch->race))) || (IS_SET(obj->pIndexData->hp_use_race, DENY_BIT) && IS_SET(obj->pIndexData->hp_use_race, NUM_BIT(ch->race))))
            return;
    }
    if (obj->pIndexData->hp_use_sex) {
        if ((IS_SET(obj->pIndexData->hp_use_sex, ALLOW_BIT) && !IS_SET(obj->pIndexData->hp_use_sex, NUM_BIT(ch->sex))) || (IS_SET(obj->pIndexData->hp_use_sex, DENY_BIT) && IS_SET(obj->pIndexData->hp_use_sex, NUM_BIT(ch->sex))))
            return;
    }
    if (obj->pIndexData->hp_use_sex) {
        if ((IS_SET(obj->pIndexData->hp_use_align, ALLOW_BIT) && !IS_SET(obj->pIndexData->hp_use_align, NUM_BIT(get_align(ch)))) || (IS_SET(obj->pIndexData->hp_use_align, DENY_BIT) && IS_SET(obj->pIndexData->hp_use_align, NUM_BIT(get_align(ch)))))
            return;
    }
...
void show_vs_graphy(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!IS_SET(ch->act, PLR_STATUS))
        return;
    if (!victim || victim->deleted)
        return;
    if (!ch || !ch->in_room || ch->deleted)
        return;

    if (victim_hp_remain > 10)
        victim_hp_remain = 10;
    ...
    if (victim_hp_remain < 7 && victim_hp_remain > 4)
        color_of_victim = HIY;
    if (victim_hp_remain < 4 && victim_hp_remain > 0)
        color_of_victim = HIR;
    if (victim_hp_remain < 0)
        color_of_victim = NOR;
...
void death_declare(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!IS_NPC(victim) && victim->level > 3 && !IS_SET(victim->act, PLR_HORSE) && victim != ch && !IS_IMMORTAL(ch)) {
        if (ch->sex == SEX_FEMALE) {
            ...
            case 0:
                sprintf(buf, "" HIR "大主教黛妮亞(Dynia)憐惜地宣布：「%s不幸戰死沙場，有人能幫助他嗎？」" NOR "\n\r",
                        victim->name);
                break;
            case 1:
                sprintf(buf, "" HIR "%s" HIR "大喊：「無能的冒險者%s陳屍於%s，有誰願意幫助他？」" NOR "\n\r",
                        ch->short_descr, victim->name, ch->in_room->name);
                break;
            case 2:
                sprintf(buf, "" HIR "死神捎來一份訊息：「%s悲慘地被%s" HIR "殺掉了，請大家幫助他。」" NOR "\n\r",
                        victim->name, ch->short_descr);
                break;
            case 3:
                sprintf(buf, "" HIR "%s" HIR "顫抖地高喊：「糟糕，我失手把%s殺死了！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 4:
                sprintf(buf, "" HIR "%s" HIR "尖叫著：「%s有一具%s的屍體，喬藍多(Joe Rando)麻煩快收走好嗎。」" NOR "\n\r",
                        ch->short_descr, ch->in_room->name, victim->name);
                break;
            case 5:
                sprintf(buf, "" HIR "%s" HIR "呼籲著：「%s你還太嫩了，請別來打擾我的生活。」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 6:
                sprintf(buf, "" HIR "%s" HIR "惋惜地說：「%s又被殺掉了，希望下次闇之冥王星普魯多(Pluto)能夠保佑他！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 7:
                sprintf(buf, "" HIR "%s" HIR "悲戚地喊：「早已滿手鮮血，現在又多了%s的亡魂！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            }
        } else {
            switch (number_bits(3)) {
            case 0:
                sprintf(buf, "" HIR "%s" HIR "高喊著：「哇哈哈，有誰願意幫%s收屍啊！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 1:
                sprintf(buf, "" HIR "%s" HIR "恥笑著：「無能的%s，你還有膽來一次嗎？」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 2:
                sprintf(buf, "" HIR "%s" HIR "狂笑著：「哇哈哈，我隨隨便便就把%s殺掉了！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 3:
                sprintf(buf, "" HIR "%s" HIR "狂笑說：「可憐的%s，回家向你的母親哭訴吧。」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 4:
                sprintf(buf, "" HIR "%s" HIR "大喊著：「%s，我還可以再殺你一次，只要你敢來的話！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 5:
                sprintf(buf, "" HIR "%s" HIR "大笑著：「天底下居然還有像%s這麼自不量力的人！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 6:
                sprintf(buf, "" HIR "%s" HIR "狂笑說：「看著%s鮮血亂噴，真是一種無上的享受！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            case 7:
                sprintf(buf, "" HIR "%s" HIR "噁心的大叫：「打死%s，就像踩扁一隻蟑螂一樣，真是噁心！」" NOR "\n\r",
                        ch->short_descr, victim->name);
                break;
            }
        }
