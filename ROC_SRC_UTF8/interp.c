...
void interpret(CHAR_DATA *ch, char *argument) {
    ...
    */
    for (cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++) {
        if (command[0] == cmd_table[cmd].name[0] && !str_prefix(command, cmd_table[cmd].name) && (cmd_table[cmd].level <= trust)) {
            ...
            if (ch->ordered == TRUE && IS_NPC(ch)) {
                // 阻檔 order 對 mob 下 skilldrv 2022/10/27
                if (!str_cmp(cmd_table[cmd].name, "skilldrv")) {
                    ch->ordered = FALSE;
                    break;
                }
                // summoned mob 不能用的技能
                if (IS_AFFECTED(ch, AFF_SUMMONED) && (!str_cmp(cmd_table[cmd].name, "backstab") || !str_cmp(cmd_table[cmd].name, "empslash") || !str_cmp(cmd_table[cmd].name, "circle") || !str_cmp(cmd_table[cmd].name, "berserk") || !str_cmp(cmd_table[cmd].name, "change fighting") || !str_cmp(cmd_table[cmd].name, "kick") || !str_cmp(cmd_table[cmd].name, "assassination") || !str_cmp(cmd_table[cmd].name, "frenzy") || !str_cmp(cmd_table[cmd].name, "bashhead") || !str_cmp(cmd_table[cmd].name, "disarm") || !str_cmp(cmd_table[cmd].name, "barehand") || !str_cmp(cmd_table[cmd].name, "circular chop") || !str_cmp(cmd_table[cmd].name, "flaming slash") || !str_cmp(cmd_table[cmd].name, "storm slash") || !str_cmp(cmd_table[cmd].name, "sprinkle slash") || !str_cmp(cmd_table[cmd].name, "earth slash") || !str_cmp(cmd_table[cmd].name, "cast") || !str_cmp(cmd_table[cmd].name, "snaek") || !str_cmp(cmd_table[cmd].name, "invoke") || !str_cmp(cmd_table[cmd].name, "pick"))) {
    ...
    if (ch->desc && ch->desc->snoop_by) {
        // 將下指令的角色名代入 snoop_by 2023/01/17
    ...
    if (ch->position < cmd_table[cmd].position) {
        ...
        // 如果 npc 有 death_prog 則死前下指令不受影響 2022/12/29
...
void ointerpret(OBJ_DATA *obj, CHAR_DATA *ch, char *argument) {
    ...
    for (cmd = 0; ocmd_table[cmd].name[0] != '\0'; cmd++) {
        if (command[0] == ocmd_table[cmd].name[0] && !str_prefix(command, ocmd_table[cmd].name)) {
            found = TRUE;
            break;
        }
    }
...
void rinterpret(ROOM_INDEX_DATA *room, CHAR_DATA *ch, char *argument) {
    ...
    for (cmd = 0; rcmd_table[cmd].name[0] != '\0'; cmd++) {
        if (command[0] == rcmd_table[cmd].name[0] && !str_prefix(command, rcmd_table[cmd].name)) {
            found = TRUE;
            break;
        }
    }
...
void zact(const char *format, CHAR_DATA *ch, const void *arg1,
    ...
    // 修正 social_table 無法判定 != "" , 會造成顯示chat 無內容的情況 2021/09/21
    if (strlen(format) <= 12)
        return;
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (!to)
            continue;
        if ((to->deleted) || (!to->desc && IS_NPC(to)) || !IS_AWAKE(to) || IS_SET(to->deaf, CHANNEL_CHAT))
            continue;
...
bool chat_social(CHAR_DATA *ch, char *command, char *argument) {
    ...
    if (fEmote) {
        if (argument[0] == '\0')
            zact("【閒聊】$n看起來表情豐富。", ch, NULL, argument, TO_ROOM); // Changed by Amethyst
        else
            zact("【閒聊】$n$T。", ch, NULL, argument, TO_ROOM); // Changed by Amethyst
        return TRUE;
    }
...
void do_chatemote(CHAR_DATA *ch, char *argument) {
    ...
    if (IS_NPC(ch) && (IS_AFFECTED(ch, AFF_SUMMONED) || IS_AFFECTED(ch, AFF_CHARM)))
        return;
...
bool immtalk_social(CHAR_DATA *ch, char *command, char *argument) {
    ...
    if (fEmote) {
        if (argument[0] == '\0')
            immact("【神諭】$n看起來表情豐富。", ch, NULL, argument, TO_ROOM); // Changed by Amethyst
        else
            immact("【神諭】$n$T。", ch, NULL, argument, TO_ROOM); // Changed by Amethyst
        return TRUE;
    }
