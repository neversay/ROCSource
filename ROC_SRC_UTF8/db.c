...
//*  PURPOSE     :         修改將strcpy改用strncpy                *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940712 jye                                    *
//*  PURPOSE     :         修改將load_down_time 時間固定隔天20:00 *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940717 jye                                    *
//*  PURPOSE     :         修改get_mob_index Crash Bug            *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940720 Razgriz                                *
//*  PURPOSE     :         修改 load_mobiles() 增加 fAutoset 的運 *
//*              :         算式，修改 create_mobile() 以公式配合  *
//*              :         fAutoset 的判斷，讓 lv > 35 的 mob     *
//*              :         假如未在區域檔中以 S 設定其 pp pd pdr  *
//*              :         md mdr 的話，將由系統來自動決定.       *
//*              :         (暫時取消此功能)                       *
//*---------------------------------------------------------------*
//*  MODIFY  BY  : 0940918 Razgriz                                *
//*  PURPOSE     :         修改 new_object() 增加 extra_value 初值*
//*****************************************************************
...
void boot_db(void) {
     ...
     */
    {
        ...
        fBootDb  = FALSE;
        daPrompt = str_dup(
            "[m<生命[31m%h[m魔法[1;34m%m[m移動[35m%v[m> ");
        boot_done();
        area_update();
        load_notes();
        MOBtrigger = TRUE;
        ...
        for (;;) {
            ...
            case 'S':
                ...
                case APPLY_AGE:
                    break;
                case APPLY_HEIGHT:
                    pMobIndex->mod_size += mod; // 體型 2022/12/23
                    break;
                case APPLY_WEIGHT:
                    break;
                case APPLY_MANA:
                ...
                case APPLY_SAVING_EVIL:
                    ...
                    break;
                /*
                // 新增 mob sv poison 傷害判定  2021/11/03
                // 將 evil , good 移至 sv dark 及 sv saint 下, ancient 分開為不同項目
...
void load_rooms(FILE *fp) {
    ...
    for (;;) {
        ...
        pRoomIndex->fall        = 0; // load room 固定將 fall 的值設為 0 2023/01/14
        ...
        for (;;) {
            ...
            if (letter == 'F') {
                ...
                pexit->exit_size   = 0; // 門的大小 2022/12/22
...
void load_down_time(void) {
    ...
    if (check_time->tm_mon == 1 || check_time->tm_mon == 3 || check_time->tm_mon == 5 || check_time->tm_mon == 7 || check_time->tm_mon == 8 || check_time->tm_mon == 10) {
...
void fix_exits(void) {
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++) {
        for (pRoomIndex = room_index_hash[iHash];
             ...
             pRoomIndex = pRoomIndex->next) {
            for (door = 0; door <= 5; door++) {
                if ((pexit = pRoomIndex->exit[door]) && (to_room = pexit->to_room) && (pexit_rev = to_room->exit[rev_dir[door]]) && pexit_rev->to_room != pRoomIndex) {
    ...
    for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next) {
        ...
        case 'O':
            ...
            if (pRoom->area->clan)
                ...
                case ITEM_ARMOR:
                    pRoom->area->clan->construct -= 3;
                    break;
                case ITEM_WEAPON:
                    pRoom->area->clan->construct -= 8;
                    break;
                default:
                    pRoom->area->clan->construct--;
                    break;
                }
        ...
        case 'P':
            ...
            if (pRoom->area->nplayer > 0
                ...
                || (!LastObj && !(LastObj = get_obj_type(pObjToIndex))) || (LastObj->in_room != pRoom) || (LastObj->carried_by && !IS_NPC(LastObj->carried_by)) || count_obj_list(pObjIndex, LastObj->contains) > 0)
                break;
...
CHAR_DATA  *create_mobile(MOB_INDEX_DATA *pMobIndex) {
    ...
    if (IS_SET(mob->special, EXPERT)) {
        ...
        if ((mob->gold = pMobIndex->gold) < 0)
            mob->gold = number_fuzzy(50) * number_fuzzy(pMobIndex->level) * UMAX((mob->level / 7), 1) * UMAX((mob->level / 17), 1) * number_range(race_table[mob->race].glow, race_table[mob->race].ghigh) / 100;
        mob->armor = pMobIndex->mod_ac + ((mob->level > 36) ? interpolate1(36, -40, 45, -290, mob->level - 36) : interpolate(mob->level, 100, -80)) + ((mob->level > 30) ? race_table[mob->race].nature_ac : 40);
        if (mob->level > 45)
            mob->armor -= mob->level * 4;
        if (mob->level > 55)
            mob->armor -= mob->level * 2;

        mob->dodge = pMobIndex->dodge + ((mob->level > 36) ? interpolate1(36, 0, 44, -140, mob->level - 36) : interpolate(mob->level, 250, -90)) + ((mob->level > 30) ? race_table[mob->race].nature_dodge : 0);
        if (mob->level > 54)
            mob->dodge = mob->dodge - (mob->level / 6);
        if (mob->level > 59)
            mob->dodge = mob->dodge - (mob->level / 5);
        ...
        mob->hitroll  = pMobIndex->mod_hitroll + ((mob->level > 28) ? interpolate1(28, 30, 45, 40, mob->level - 28) : interpolate(mob->level, 5, 25));
        mob->damroll  = pMobIndex->mod_damroll + ((mob->level > 30) ? interpolate1(30, 40, 40, 65, mob->level - 30) : interpolate(mob->level, 3, 50));
        for (i = 0; i < MAX_SAVING; ++i) {
            mob->saving_throw[i] = pMobIndex->mod_svs[i] + ((mob->level > 42) ? interpolate1(42, -15, 60, -20, mob->level - 42) : interpolate(mob->level, 5, -15));
            ;
        }
        for (i = 0; i < 9; i++)
        ...
        if ((mob->gold = pMobIndex->gold) < 0)
            mob->gold = number_fuzzy(50) * number_fuzzy(pMobIndex->level) * UMAX((mob->level / 9), 1) * UMAX((mob->level / 20), 1) * number_range(race_table[mob->race].glow, race_table[mob->race].ghigh) / 100;
        mob->armor = pMobIndex->mod_ac + ((mob->level > 36) ? interpolate1(36, -20, 45, -200, mob->level - 36) : interpolate(mob->level, 110, -60)) + ((mob->level > 36) ? race_table[mob->race].nature_ac : 60);
        if (mob->level > 45)
            mob->armor -= mob->level * 4;

        mob->dodge = pMobIndex->dodge + ((mob->level > 34) ? interpolate1(34, 60, 44, -90, mob->level - 34) : interpolate(mob->level, 300, 0)) + ((mob->level > 36) ? race_table[mob->race].nature_dodge : 0);
        ...
        mob->mana     = mob->max_mana;
        mob->hitroll  = pMobIndex->mod_hitroll + ((mob->level > 28) ? interpolate1(28, 20, 45, 30, mob->level - 28) : interpolate(mob->level, 5, 25));
        mob->damroll  = pMobIndex->mod_damroll + ((mob->level > 33) ? interpolate1(33, 35, 40, 47, mob->level - 33) : interpolate(mob->level, 3, 50));
        for (i = 0; i < MAX_SAVING; ++i) {
            mob->saving_throw[i] = pMobIndex->mod_svs[i] + ((mob->level > 42) ? interpolate1(42, -8, 60, -20, mob->level - 42) : interpolate(mob->level, 5, -15));
            ;
        }
        for (i = 0; i < 9; ++i)
            mob->saving_throw[i] += race_table[pMobIndex->race].nature_svs[i];
    ...
    // mob 設有 mystic armor 在 reborn 時會作用效果 2020/10/27
...
OBJ_DATA *create_object(OBJ_INDEX_DATA *pObjIndex, int level) {
    ...
    case ITEM_DART: // 把 dart 及 arrow 的 create 分成同類 2022/05/06
    case ITEM_ARROW:
        dmgbase = level + obj->weight / 3;
        if (obj->value[1] == 0)
            obj->value[1] = number_fuzzy(number_fuzzy(dmgbase));
    ...
    case ITEM_WEAPON:
        ...
        case 14: // shot
            ...
            avgdif  = 3;
            break;
        case 16: // ammo 一定要設有值 2022/05/06
            dmgbase = 0;
            avgdif  = 0;
            break;
        }

        // 當有設定值時 value 值不再會跑隨機值 2022/05/08
        if (obj->value[1] == 0)
            obj->value[1] = number_fuzzy(number_fuzzy(dmgbase));
        /*else{
...
OBJ_DATA *fcreate_object(OBJ_INDEX_DATA *pObjIndex, int level) {
    ...
    case ITEM_WEAPON:
        ...
        // 當有設定值時 value 值不會跑隨機值 2022/05/08
        if (obj->value[1] == 0)
            obj->value[1] = number_fuzzy(number_fuzzy(dmgbase));
        /*else{
                //bugf("weapon [%d] has value1 != 0 ", pObjIndex->vnum);
...
void do_areas(CHAR_DATA *ch, char *argument) {
    ...
    buf1[0] = '\0';
    if (!z_buf || z_buf[0] == '\0') {
        send_to_char("Areas指令可以讓你查詢本Mud現有的區域列表\n\r", ch);
        send_to_char("Areas後面的有效參數如下：\n\r\n\r", ch);
        send_to_char("(1) 區域原著 \n\r", ch);
        send_to_char("(2) 區域翻譯 \n\r", ch);
        send_to_char("(3) 區域改編 \n\r", ch);
        return;
    ...
    if (type == 1)
        strcat(buf1,
               "  區  域  名  稱           作 者             區  域  名  稱           作 者   \n\r"

        );
    else if (type == 2)
        strcat(buf1,
               "  區  域  名  稱           翻譯者            區  域  名  稱           翻譯者  \n\r"

        );
    else if (type == 3)
        strcat(buf1,
               "  區  域  名  稱           改編者            區  域  名  稱           改編者  \n\r"
