...
bool spec_assassin(CHAR_DATA *ch) {
    ...
    if (!IS_AWAKE(ch) || ch->deleted)
        return FALSE;

    if (!(victim = ch->fighting) || victim->deleted || victim->in_room != ch->in_room)
        return FALSE;
...
bool spec_cast_water(CHAR_DATA *ch) {
    ...
    if (IS_AFFECTED(ch, AFF_MUTE) || IS_AFFECTED(ch, AFF_SEAL_MAGIC) || IS_SET(race_table[ch->race].race_abilities, RACE_MUTE) || IS_SET(ch->in_room->room_flags, ROOM_CONE_OF_SILENCE))
        return FALSE;
...
bool spec_cast_cleric(CHAR_DATA *ch) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim->next_in_room) {
        if (victim->fighting == ch && can_see(ch, victim) && number_bits(2) == 0)
            break;
    }
...
bool spec_cast_mage(CHAR_DATA *ch) {
    ...
    for (victim = ch->in_room->people; victim; victim = victim->next_in_room) {
        if (victim->fighting == ch && can_see(ch, victim)) {
            if (!IS_SET(victim->special, EXPERT) && number_bits(2) == 1)
                break;
            else if (IS_SET(victim->special, EXPERT) && number_bits(1) == 1)
                break;
        }
    }
    ...
    for (;;) {
        ...
        case 11:
            min_level = 30;
            spell     = ((IS_SET(ch->special, EXPERT) ? "whirlwind" : "soulsteal"));
            break;
        case 12:
            min_level = 42;
            spell     = ((IS_SET(ch->special, EXPERT) ? "polymorph other" : "paralyze"));
            break;
        case 13:
            min_level = 38;
            spell     = ((IS_SET(ch->special, EXPERT) ? "teleport" : "2 fireball"));
            break;
        case 14:
            min_level = 18;
            spell     = "magic miss";
...
bool spec_cast_undead(CHAR_DATA *ch) {
    ...
    if (IS_AFFECTED(ch, AFF_MUTE) || IS_SET(race_table[ch->race].race_abilities, RACE_MUTE) || IS_SET(ch->in_room->room_flags, ROOM_CONE_OF_SILENCE))
        return FALSE;
    ...
    for (;;) {
        ...
        case 7:
            min_level = 30;
            spell     = ((IS_SET(ch->special, EXPERT) ? "teleport" : "paralyze"));
            break;
        case 8:
            min_level = 15;
            spell     = "lifesteal";
...
bool spec_janitor(CHAR_DATA *ch) {
    ...
    for (trash = ch->in_room->contents; trash; trash = trash_next) {
        ...
        if (!IS_SET(trash->wear_flags, ITEM_TAKE))
            continue;
        if (trash->item_type == ITEM_DRINK_CON || trash->item_type == ITEM_FOOD || trash->item_type == ITEM_TRASH || trash->cost < 1000) {
...
bool spec_poison(CHAR_DATA *ch) {
    ...
    if (ch->position != POS_FIGHTING || !(victim = ch->fighting) || number_percent() > 2 * ch->level)
        return FALSE;
...
bool spec_thief(CHAR_DATA *ch) {
    ...
    for (victim = ch->in_room->people; victim;
        ...
        if (IS_NPC(victim) || victim->level >= LEVEL_IMMORTAL || number_bits(3) != 0 || !can_see(ch, victim)) /* Thx Glop */
            continue;

        /* Thanks to Zeke from MudX for pointing the percent bug */
        if (IS_AWAKE(victim) && victim->level > 5 && number_percent() + victim->level - ch->level >= 33) {
            act("你發現 $n 的手正摸著你的錢包！",
...
bool spec_repairman(CHAR_DATA *ch) {
    ...
    if (IS_SET(pexit->exit_info, EX_BASHED)) {
        ...
        if ((to_room = pexit->to_room) && (pexit_rev = to_room->exit[rev_dir[door]]) && pexit_rev->to_room == ch->in_room) {
            CHAR_DATA *rch;
