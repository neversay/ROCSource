...
bool IsUnderFistFight(CHAR_DATA *ch) {
    ...
    if (!get_eq_char(ch, WEAR_HOLD) && !get_eq_char(ch, WEAR_SHIELD) && !get_eq_char(ch, WEAR_WIELD) && !get_eq_char(ch, WEAR_WIELD_2) && !get_eq_char(ch, WEAR_TWO_HAND)) {
        return TRUE;
    }
...
int GetSorGateMobLevel(int nMobIndex) {
    ...
    */
    if (nMobIndex == MOB_VNUM_SOR_FLAME2 || nMobIndex == MOB_VNUM_SOR_WATER2 || nMobIndex == MOB_VNUM_SOR_EARTH2 || nMobIndex == MOB_VNUM_SOR_WIND2 || nMobIndex == MOB_VNUM_SOR_LIGHTNING2) {
        return 1;
    }
    if (nMobIndex == MOB_VNUM_SOR_FLAME1 || nMobIndex == MOB_VNUM_SOR_WATER1 || nMobIndex == MOB_VNUM_SOR_EARTH1 || nMobIndex == MOB_VNUM_SOR_WIND1 || nMobIndex == MOB_VNUM_SOR_LIGHTNING1 || nMobIndex == MOB_VNUM_SOR_FLAME0 || nMobIndex == MOB_VNUM_SOR_WATER0 || nMobIndex == MOB_VNUM_SOR_EARTH0 || nMobIndex == MOB_VNUM_SOR_WIND0 || nMobIndex == MOB_VNUM_SOR_LIGHTNING0) {
        return 0;
    }
    return -1;
}
...
int GET_DODGE(CHAR_DATA *ch) {
    ...
    if (nDodge < 0)
        nDodge = 0;
    if (IS_NPC(ch)) {
        nDodge += ch->dodge + ((IS_AWAKE(ch)) ? dex_app[get_curr_dex(ch)].defensive : 100) + (is_flying(ch) ? -40 : 0);
        ...
        nDodge += (ch->class == NINJA ? (ch->pcdata->prime_attr == APPLY_DEX ? -60 : (ch->pcdata->prime_attr == APPLY_STR ? -40 : (ch->pcdata->prime_attr == APPLY_CON ? -20 : 0))) : 0) + ch->dodge + ((IS_AWAKE(ch)) ? dex_app[get_curr_dex(ch)].defensive : 100) + (ch->level * (-5) + (is_flying(ch) ? -40 : 0));
    }
    return nDodge;
}
...
int get_eff_str(CHAR_DATA *ch) {
    ...
    value += (ch->pcdata->prime_attr == APPLY_STR) ? 8 : (ch->pcdata->snd_attr == APPLY_STR) ? 5
                                                     : (ch->class > 4)                       ? 2
                                                                                             : 0;
...
int get_eff_int(CHAR_DATA *ch) {
    ...
    value += (ch->pcdata->prime_attr == APPLY_INT) ? 8 : (ch->pcdata->snd_attr == APPLY_INT) ? 5
                                                     : (ch->class > 4)                       ? 2
                                                                                             : 0;
    value += (ch->level > 39) ? 1 : 0;
    if (ch->race == 0 && ch->class > 4 && !is_affected(ch, gsn_werewolf))
...
int get_eff_wis(CHAR_DATA *ch) {
    ...
    value += (ch->pcdata->prime_attr == APPLY_WIS) ? 8 : (ch->pcdata->snd_attr == APPLY_WIS) ? 5
                                                     : (ch->class > 4)                       ? 2
                                                                                             : 0;
    value += (ch->level > 39) ? 1 : 0;
    if (ch->race == 0 && ch->class > 4 && !is_affected(ch, gsn_werewolf))
...
int get_eff_dex(CHAR_DATA *ch) {
    ...
    value += (ch->pcdata->prime_attr == APPLY_DEX) ? 8 : (ch->pcdata->snd_attr == APPLY_DEX) ? 5
                                                     : (ch->class > 4)                       ? 2
                                                                                             : 0;
    value += (ch->level > 39) ? 1 : 0;
    if (ch->race == 0 && ch->class > 4 && !is_affected(ch, gsn_werewolf))
...
int get_eff_con(CHAR_DATA *ch) {
    ...
    value += (ch->pcdata->prime_attr == APPLY_CON) ? 8 : (ch->pcdata->snd_attr == APPLY_CON) ? 5
                                                     : (ch->class > 4)                       ? 2
                                                                                             : 0;
    value += (ch->level > 39) ? 1 : 0;
    if (ch->race == 0 && ch->class > 4 && !is_affected(ch, gsn_werewolf))
...
void equip_char(CHAR_DATA *ch, OBJ_DATA *obj, int iWear) {
    ...
    */
    ...
    if ((IS_OBJ_STAT(obj, ITEM_NO_MALE) && ch->sex == SEX_MALE) || (IS_OBJ_STAT(obj, ITEM_NO_FEMALE) && ch->sex == SEX_FEMALE) || (IS_OBJ_STAT(obj, ITEM_NO_NEUTRAL) && ch->sex == SEX_NEUTRAL)) {
    ...
    if ((IS_OBJ_STAT(obj, ITEM_SPECIAL))) {
        if (obj->material->class && !IS_NPC(ch))
            if ((IS_SET(obj->material->class, ALLOW_BIT) && !(IS_SET(obj->material->class, NUM_BIT(ch->class)) || IS_SET(obj->class, NUM_BIT(get_allow_deny_bit(ch))))) || (IS_SET(obj->material->class, DENY_BIT) && (IS_SET(obj->material->class, NUM_BIT(ch->class)) || IS_SET(obj->class, NUM_BIT(get_allow_deny_bit(ch)))))) {
                send_to_char("你的職業無法使用這樣裝備！\n\r", ch);
                return;
            }
    } else {
        if (obj->material->class && !IS_NPC(ch))
            if ((IS_SET(obj->material->class, ALLOW_BIT) && !IS_SET(obj->material->class, NUM_BIT(ch->class))) || (IS_SET(obj->material->class, DENY_BIT) && IS_SET(obj->material->class, NUM_BIT(ch->class)))) {
                send_to_char("你的職業無法使用這樣裝備！\n\r", ch);
                return;
            }
    }
    ...
    if (obj->material->race && !IS_NPC(ch)) {
        ...
        if ((IS_SET(obj->material->race, ALLOW_BIT) && !IS_SET(obj->material->race, NUM_BIT(nRaceBit))) || (IS_SET(obj->material->race, DENY_BIT) && IS_SET(obj->material->race, NUM_BIT(nRaceBit)))) {
    ...
    if ((IS_OBJ_STAT(obj, ITEM_SPECIAL))) {
        if (obj->class && !IS_NPC(ch))
            if ((IS_SET(obj->class, ALLOW_BIT) && !(IS_SET(obj->class, NUM_BIT(ch->class)) || IS_SET(obj->class, NUM_BIT(get_allow_deny_bit(ch))))) || (IS_SET(obj->class, DENY_BIT) && (IS_SET(obj->class, NUM_BIT(ch->class)) || IS_SET(obj->class, NUM_BIT(get_allow_deny_bit(ch)))))) {
                send_to_char("你的職業無法使用這樣裝備！\n\r", ch);
                return;
            }
    } else {
        if (obj->class && !IS_NPC(ch))
            if ((IS_SET(obj->class, ALLOW_BIT) && !IS_SET(obj->class, NUM_BIT(ch->class))) || (IS_SET(obj->class, DENY_BIT) && IS_SET(obj->class, NUM_BIT(ch->class)))) {
                send_to_char("你的職業無法使用這樣裝備！\n\r", ch);
                return;
            }
    }
    ...
    if (obj->race && !IS_NPC(ch)) {
        int nRaceBit = get_ppl_race_bit(ch->race);
        if ((IS_SET(obj->race, ALLOW_BIT) && !IS_SET(obj->race, NUM_BIT(nRaceBit))) || (IS_SET(obj->race, DENY_BIT) && IS_SET(obj->race, NUM_BIT(nRaceBit)))) {
            send_to_char("你的種族無法使用這樣裝備！\n\r", ch);
            return;
        }
    }
    ...
    if (IS_HORSE(ch)) {
        if (!get_eq_char(ch, WEAR_HOLD) && !get_eq_char(ch, WEAR_SHIELD) && !get_eq_char(ch, WEAR_WIELD) && !get_eq_char(ch, WEAR_WIELD_2) && !get_eq_char(ch, WEAR_LIGHT) && !get_eq_char(ch, WEAR_FINGER_R) && !get_eq_char(ch, WEAR_FINGER_L) && !get_eq_char(ch, WEAR_FINGER_3) && !get_eq_char(ch, WEAR_HANDS) && !get_eq_char(ch, WEAR_FINGER_3) && !get_eq_char(ch, WEAR_TWO_HAND)) {
    ...
    if (obj && (obj->hp || obj->material->hp)) {
        ...
        for (; ohp; ohp = ohp->next) {
            if (IS_SET(ohp->event, HP_WEAR)) {
                if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_ARTIFACT) && get_skill_percent(ch, gsn_artifact) * ohp->percent * get_curr_wis(ch) / 2500 > number_percent()) || (IS_NPC(ch) && ohp->percent * get_curr_wis(ch) / 25 > number_percent())) {
...
void extract_char(CHAR_DATA *ch, bool fPull) {
    ...
    if (IS_NPC(ch) || (!IS_NPC(ch) && ch->desc && ch->desc->connected == CON_PLAYING) || (!IS_NPC(ch) && !ch->desc))
        char_from_room(ch);
...
OBJ_DATA *get_obj_carry(CHAR_DATA *ch, char *argument) {
    ...
    if (!is_number(argument)) {
        ...
        count  = 0;
        for (obj = ch->carrying; obj; obj = obj->next_content) {
            if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && is_pfxname(arg, obj->name)) {
                if (++count == number)
                    return obj;
            }
        }
        ...
        for (obj = ch->carrying; obj; obj = obj->next_content) {
            if (obj->pIndexData->vnum == number && obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj))
                return obj;
        }
    }
...
OBJ_DATA *get_obj_wear(CHAR_DATA *ch, char *argument) {
    ...
    if (!is_number(argument)) {
        ...
        for (obj = ch->carrying; obj; obj = obj->next_content) {
            if (!obj)
                continue;
            if (obj->wear_loc != WEAR_NONE && can_see_obj(ch, obj) && obj->pIndexData->vnum == number)
                return obj;
        }
    }
    return NULL;
...
OBJ_DATA *check_using_wp(CHAR_DATA *ch, int dt, int wield) {
    ...
    if (dt == 108) // kick
    {
        ...
        if (get_eq_char(ch, WEAR_WIELD) // 有手持武器
            || get_eq_char(ch, WEAR_TWO_HAND) || get_eq_char(ch, WEAR_WIELD_2) && dt != gsn_throwing) {
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    if (!IS_NPC(victim) && IS_SET(victim->act, PLR_WIZINVIS) && get_trust(ch) < get_wizinvis_level(victim))
        return FALSE;
    ...
    if ((IS_AFFECTED(victim, AFF_INVISIBLE) || IS_SET(race_table[ch->race].race_abilities, RACE_INVIS)) && !IS_SET(race_table[ch->race].race_abilities, RACE_DETECT_INVIS) && !IS_AFFECTED(ch, AFF_DETECT_INVIS))
        return FALSE;

    if (IS_AFFECTED(victim, AFF_HIDE) && !IS_SET(race_table[ch->race].race_abilities, RACE_DETECT_HIDDEN) && !IS_AFFECTED(ch, AFF_DETECT_HIDDEN) && !victim->fighting)
        return FALSE;
...
bool can_see_obj(CHAR_DATA *ch, OBJ_DATA *obj) {
    ...
    if (room_is_dark(ch->in_room) && !IS_SET(race_table[ch->race].race_abilities, RACE_INFRAVISION) && !IS_AFFECTED(ch, AFF_INFRARED))
        return FALSE;

    if (IS_SET(obj->extra_flags, ITEM_INVIS) && !IS_SET(race_table[ch->race].race_abilities, RACE_DETECT_INVIS) && !IS_AFFECTED(ch, AFF_DETECT_INVIS))
        return FALSE;
...
char *affect_loc_name(int location) {
    ...
    case APPLY_POWER_POTENTIAL:
        return "power potential";
    case APPLY_RDAMAGE:
        return "reduce physical damage";
    case APPLY_RMAGIC:
        return "reduce magic damage";
    case APPLY_ADAMAGE:
        return "enhance physical damage";
    case APPLY_AMAGIC:
        return "enhance magic damage";
    case APPLY_GPHYSICAL:
        return "physical damage gate";
    case APPLY_GMAGIC:
        return "magic damage gate";
    case APPLY_SAVING_ILLUSION:
        return "save vs illusion";
    case APPLY_ACTION_ABILITY:
        return "action ability";
    case APPLY_DOB_SAVING_FLASH:
        return "save vs sun flash";
    case APPLY_WEAPON_SPELL:
        return "spell weapon"; // add at 2021/09/19
    ...
    case APPLY_WEAPON_LIGHTNING:
        return "lightning weapon";
    case APPLY_WEAPON_SAINT:
        return "saint weapon";
    case APPLY_WEAPON_DARK:
        return "dark weapon";
    case APPLY_MAGIC_IMMUNE:
        return "magic immune"; // add at 2021/11/03 for mystic armor
    case APPLY_TOXICITY_POISON:
        return "toxicity poison"; // add at 2021/11/10 for poison item
    case APPLY_DAMAGE_COUNT:
        return "damage count"; // add at 2022/02/21 for blood thirsty
    case APPLY_SHOOT_HITROLL:
        return "shoot hitroll"; // add at 22022/05/06
    case APPLY_SHOOT_DAMROLL:
        return "shoot damroll"; // add at 2022/05/06
    case APPLY_SHOOT_ADAMAGE:
        return "shoot physical damage"; // add at 2022/05/06
    }
...
char *affect_bit_name(long long int vector) {
    ...
    if (vector & AFF_SEAL_MAGIC)
        strcat(buf, " seal magic");
    if (vector & AFF_GILLS)
        strcat(buf, " gills");
    if (vector & AFF_VAMP_BITE)
        strcat(buf, " vampiric bite");
    if (vector & AFF_GHOUL)
        strcat(buf, " undead curse"); // 把ghoul curse 換成 undead curse 2022/01/16
    if (vector & AFF_FLAMING)
        strcat(buf, " flaming shield");
    if (vector & AFF_PARALYZED)
        strcat(buf, " paralyzed");
    if (vector & AFF_CRAZY)
        strcat(buf, " crazy");
    if (vector & AFF_WEAKEN)
        strcat(buf, " weaken");
    if (vector & AFF_NIGHTSTALK)
        strcat(buf, " nightstalk");
    if (vector & AFF_DARK_RAID)
        strcat(buf, " dark raid");
    if (vector & AFF_MANA_SHIELD)
        strcat(buf, " mana shield");
    if (vector & AFF_TARDY)
        strcat(buf, " tardy");
    if (vector & AFF_CONTINUE_CURE)
        strcat(buf, " continue cure"); // add at 2022.11.02
    if (vector & AFF_DISEASE)
        strcat(buf, " disease"); // add at 2022.11.02
    if (vector & AFF_FEAR)
        strcat(buf, " fear"); // add at 2022.11.02
    if (vector & AFF_NO_HEAL)
        strcat(buf, " no heal"); // add at 2018.11.20
...
char *extra_bit_name(int extra_flags) {
    ...
    if (extra_flags & ITEM_POISONED)
        strcat(buf, " poisoned");
    if (extra_flags & ITEM_BROKEN)
        strcat(buf, " broken");
    if (extra_flags & ITEM_VAMPIRE_BANE)
        strcat(buf, " vampire bane");
    ...
    if (extra_flags & ITEM_UNIQUE)
        strcat(buf, " unique");
    if (extra_flags & ITEM_CLAN)
        strcat(buf, " clan");
    if (extra_flags & ITEM_PERSONAL)
        strcat(buf, " personal");
    if (extra_flags & ITEM_SPELL)
        strcat(buf, " spell"); // ITEM_RACIAL change to ITEM_SPELL 2021/09/19
    ...
    if (extra_flags & ITEM_NO_NEUTRAL)
        strcat(buf, " noneutral");
    if (extra_flags & ITEM_RECOVER)
        strcat(buf, " recovered");
    if (extra_flags & ITEM_REMOVE_NODROP)
        strcat(buf, " remove_nodrop");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *wear_bit_name(int vector) {
    ...
    if (vector & ITEM_HOLD)
        strcat(buf, " hold");
    if (vector & ITEM_TWO_HAND)
        strcat(buf, " two-handed");
    if (vector & ITEM_WEAR_ARROW)
        strcat(buf, " arrow");
    if (vector & ITEM_WEAR_RIDE)
        strcat(buf, " ride");
    if (vector & ITEM_WEAR_AMMO)
        strcat(buf, " ammo"); // add at 2022/04/30
...
char *special_bit_name(int vector) {
    ...
    if (vector & NO_CURSE)
        strcat(buf, " no_curse");
    if (vector & NO_FAEFIRE)
        strcat(buf, " no_faefire");
    if (vector & NO_POISON)
        strcat(buf, " no_poison");
    if (vector & NO_SLEEP)
        strcat(buf, " no_sleep");
    if (vector & DAM_HALF)
        strcat(buf, " half_magic");
    if (vector & ANTI_MAGIC)
        strcat(buf, " anti_magic");
    if (vector & REF_MAGIC)
        strcat(buf, " ref_magic");
    if (vector & ALY_SAN)
        strcat(buf, " aly_san");
    if (vector & ALY_INVIS)
        strcat(buf, " alc_inv");
    ...
    if (vector & NO_MUTE)
        strcat(buf, " no_mute");
    if (vector & NO_SEALMAGIC)
        strcat(buf, " no_sealmagic");
    if (vector & EXPERT)
        strcat(buf, " expert");
    if (vector & NO_CRAZY)
        strcat(buf, " no_crazy");
    ...
    if (vector & NO_PARA)
        strcat(buf, " no_para");
    if (vector & SPELL_FURY)
        strcat(buf, " spell_fury");
    if (vector & NO_FEAR)
        strcat(buf, " no_fear");
    if (vector & NO_DISARM)
        strcat(buf, " no_disarm");
    if (vector & NO_TRIP)
        strcat(buf, " no_trip");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *ppl_act_bit_name(long long int vector) {
    ...
    if (vector & PLR_IS_NPC)
        strcat(buf, " npc");
    if (vector & PLR_STATUS)
        strcat(buf, " status");
    if (vector & PLR_REGISTER)
        strcat(buf, " pk");
    ...
    if (vector & PLR_AFK)
        strcat(buf, " afk");
    if (vector & PLR_ARTIFACT)
        strcat(buf, " artifact");
    if (vector & PLR_FIST_FIGHT)
        strcat(buf, " fist_fight");
    if (vector & PLR_COMBAT)
        strcat(buf, " combat");
    if (vector & PLR_CHINESE)
        strcat(buf, " chinese");
    if (vector & PLR_HORSE)
        strcat(buf, " horse");
    if (vector & PLR_NOTIFY)
        strcat(buf, " notify");
    if (vector & PLR_VERBOSEALL)
        strcat(buf, " verall");
    if (vector & PLR_TITLE)
        strcat(buf, " title");
    if (vector & PLR_ASSIST)
        strcat(buf, " assist");
    if (vector & PLR_RIDE)
        strcat(buf, " ride");
    if (vector & PLR_DEAD)
        strcat(buf, " dead");
    if (vector & PLR_GRAPHIC)
        strcat(buf, " graphic");
    if (vector & PLR_IDLE)
        strcat(buf, " idle");
    return (buf[0] != '\0') ? buf + 1 : "none";
...
char *act_bit_name(long long int vector) {
    ...
    if (vector & ACT_PRACTICE)
        strcat(buf, " practice");
    if (vector & ACT_CRITICAL)
        strcat(buf, " critical");
    if (vector & ACT_HARD_DIS)
        strcat(buf, " swordmaster");
    if (vector & ACT_HUNT)
        strcat(buf, " hunt");
    if (vector & ACT_DUAL)
        strcat(buf, " dual");
    if (vector & ACT_NO_FLEE)
        strcat(buf, " mob_no_flee"); // 將 no_flee改為 mob_no_flee 避免 null bstr 2022/01/03
    if (vector & ACT_MOVED)
        strcat(buf, " moved");
    if (vector & ACT_TACTICS)
        strcat(buf, " tactics");
    if (vector & ACT_TRACK)
        strcat(buf, " track");
    if (vector & ACT_REGENERACY)
        strcat(buf, " regeneracy");
    if (vector & ACT_GAMBLE)
        strcat(buf, " gamble");
    if (vector & ACT_BANKER)
        strcat(buf, " banker");
    if (vector & ACT_MOB_AI)
        strcat(buf, " mob-ai");
    if (vector & ACT_REFLEXION)
        strcat(buf, " reflexion");
    if (vector & ACT_MPROG_DEATH)
        strcat(buf, " deathprog");
    if (vector & ACT_NOPAR)
        strcat(buf, " nopar");
    if (vector & ACT_IGNORE_NOMOB)
        strcat(buf, " ignore_no_mob");
    if (vector & ACT_SMITH)
        strcat(buf, " smith");
    if (vector & ACT_HORSE)
        strcat(buf, " horse");
...
int room_clanlv(ROOM_INDEX_DATA *pRoom) {
    if (IS_SET(pRoom->room_flags, ROOM_CLAN_LV3))
        return 3;
    else if (IS_SET(pRoom->room_flags, ROOM_CLAN_LV2))
        return 2;
    else if (IS_SET(pRoom->room_flags, ROOM_CLAN_LV1))
        return 1;
    else
        return 0;
}
...
int gain_skill_percent(CHAR_DATA *ch, int sn, int value) {
    ...
    mod                     = UMIN(skill_max_percent(ch, sn), mod + value);
    ch->pcdata->learned[sn] = mod + (ch->class >= WIZARD ? get_skill_level(ch, sn) * 1000 : 0);
    return mod;
}
    ...
    if (!IS_NPC(ch) && get_skill_percent(ch, gsn_armor_mastery) > 0) {
        ...
        if (armor < 0) {
            if (ch->pcdata->prime_attr == APPLY_STR) {
                ...
                if (get_skill_level(ch, gsn_armor_mastery) >= 5)
                    armor -= 100;
                if (get_skill_level(ch, gsn_armor_mastery) >= 6)
                    armor -= 150;
            } else {
...
void gain_merit(CHAR_DATA *ch, int merit) {
    ...
    if (ch->level < 41) {
        if (!is_qmark(ch, "enable_level_41")) {
            send_to_char("你尚未成為英雄還不能建立功勳。\n\r", ch);
            return;
        }
    }

    if (merit >= 0) {
        sprintf(buf, "你的功勳提升了 %d 點！\n\r", merit);
    } else {
        sprintf(buf, "你的功勳降低了 %d 點！\n\r", -merit);
    }
    send_to_char(buf, ch);
...
int skill_difficulty(CHAR_DATA *ch, int sn) {
    ...
    if (skill_table[sn].apply_attr[ch->pcdata->prime_attr - 1] < 0)
        return -1;
    if (skill_table[sn].apply_attr[ch->pcdata->snd_attr - 1] < 0)
        return -1;
