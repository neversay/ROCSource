...
int VenomousStrike(CHAR_DATA *pCh, CHAR_DATA *pVictim, int wpn) {
    ...
    if (nVenomSN == 0)
        nVenomSN = skill_lookup("venomous strike");
    ...
    if (nVenomPerc <= 0 || pCh == pVictim) {
        ...
        if ((nPoisonSklv + nToxicity / 30 + new_saves_spell(pCh->level, pVictim, MAGIC_SPELL) / 200) > number_percent()) {
...
void AutoCorpse(CHAR_DATA *ch, CHAR_DATA *victim, char *vchname) {
    ...
    if (!corpse)
        return; // 死前 purge 沒有屍體的情況就 return
...
int IsRangedAttack(int nDmgType) {
    ...
    if (nDmgType == 1014 || nDmgType == 1015 || nDmgType == 1016 // gun shoot
...
int GetPhysicalWithShield(CHAR_DATA *ch, CHAR_DATA *victim, int nDamage, int dt) {
    ...
    if (!IS_NPC(ch) || (IS_NPC(ch) && IS_SET(ch->class, 8))) {
        if (Break_Perc / 20 + Break_Sklv * 10 >= number_percent() + Block_Perc / 20 + Block_Sklv * Block_Sklv) {
            act((is_chinese(victim, ch)) ? HIY "$n 運用技巧突破你的盾牌防禦!" NOR : HIY "$n break through your shield defend!" NOR, ch, NULL, victim, TO_VICT);
            return nDamage;
        }
    }
    ...
    if (is_affected(ch, gsn_powerup)) {
        ...
        if (Power_Perc / 33 + affected_value(ch, gsn_powerup, APPLY_DAMROLL) * 2 >= number_percent() + Block_Perc / 20 + Block_Sklv * Block_Sklv) {
            act((is_chinese(victim, ch)) ? HIY "$n 運用發勁突破你的盾牌防禦!" NOR : HIY "$n break through your shield defend!" NOR, ch, NULL, victim, TO_VICT);
            return nDamage;
        }
    }
...
int PhysicalElemental(CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj) {
    ...
    // 有正面加成時 裝備耐久較易耗損
    if (EncDam > 0 && (!IS_NPC(ch) || (IS_NPC(ch) && IS_SET(ch->act, ACT_PET))) && (obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_WEAPON))
        waste_dura(obj);
...
int GetEnchantmentPhysicalDamage(CHAR_DATA *ch, CHAR_DATA *victim, int dt, OBJ_DATA *wpn, OBJ_DATA *wpn2) {
    ...
    if (dt == 108) { // kick 獨立判定
        ...
        if (get_eq_char(ch, WEAR_WIELD) // 有手持武器
            || get_eq_char(ch, WEAR_TWO_HAND) || get_eq_char(ch, WEAR_WIELD_2)) {
            if (dt != 1014                          // shot
                && dt != 1015                       // dart 寫於 skill 下
                && dt != 1016 && dt != gsn_throwing // throw 寫於 skill 下
                && (dt >= TYPE_HIT || dt == gsn_frenzy || dt == gsn_bashhead || dt == gsn_charge || dt == gsn_backstab || dt == gsn_circle || dt == gsn_circular || dt == gsn_sprinkle_slash || dt == gsn_flaming_slash || dt == gsn_storm_slash || dt == gsn_fulgurant_slash || dt == gsn_earth || dt == gsn_empslash)) {
                if (!wpn)
                    return nDamage;
                if (wpn && IS_OBJ_STAT(wpn, ITEM_SPELL))
                    nDamage = nDamage + PhysicalElemental(ch, victim, wpn);
...
int damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn, int msg_mode, int dmg_mode) {
    ...
    if (!IS_NPC(ch) && (victim != ch) && dt != gsn_lifeshare // lifeshare 的傷害不會被判定到 2021/11/22
    ...
    int nRateIgnorePDR = ((nSklvShot * (nSklvShot + 4) + nSklvMete * (nSklvMete + 4)) / 2) * (get_curr_wis(ch) - 5) * (get_curr_wis(ch) - 5) / 400;
    ...
    */
    ...
    if (victim != ch && dt != gsn_lifeshare) {
        ...
        if (IsPK(ch, victim) || (!IS_NPC(victim) && victim->pcdata->clanpk && IS_SET(victim->in_room->room_flags, ROOM_PK)))
        ...
        */
        ...
        if (dmg_mode == DMG_NORMAL) {
            ...
            if (dt >= TYPE_HIT || dt == gsn_kick || (!IS_NPC(ch) && dt == gsn_frenzy && ch->pcdata->prime_attr != APPLY_CON) || (IS_NPC(ch) && dt == gsn_circular)) {
                // 處理 普擊 跟 飛腿, 傷害為0時也能作用
                if (dt != gsn_frenzy) {
                    // 擊落武器 (npc才有)
                    if (IS_NPC(ch) && number_percent() < UMIN(20, UMAX(leveldiff + (intdiff * 2), 5))) {
                        disarm(ch, victim);
                    ...
                    // 自動絆倒 (npc才有)
                    if (IS_NPC(ch) && ch->level > 10 && number_percent() < UMIN((leveldiff + intdiff * 4), (IS_SET(ch->class, 1) ? 15 : 3))) {
                        trip(ch, victim);
                    }

                    // 主屬性是str 或者 職業為僧侶 的 pc, 將有機會自動絆倒對方
                    if (!IS_NPC(ch) && (ch->pcdata->prime_attr == APPLY_STR || ch->class == 13) && number_percent() < UMAX(15, get_skill_percent(ch, gsn_fool) / 6 + UMAX(0, (intdiff + dexdiff) * 4) + leveldiff + 10)) {
                        trip(ch, victim);
                    }

                    // 天生 RACE_WEAPON_WIELD 的 mob, 有一定機率使用身上的 scroll wand staff potion pill
                    if (IS_NPC(ch) && IS_SET(race_table[ch->race].race_abilities, RACE_WEAPON_WIELD) && number_percent() < UMIN(25, UMAX(10, ch->level)) && !IS_NPC(victim)) {
                        use_magical_item(ch); // 這個函式沒判斷好, potion 沒判斷到
                        if (ch->deleted || victim->deleted || !ch->in_room || ch->in_room != victim->in_room || !ch->fighting) {
                            return -2;
                        }
                ...
                if (dam >= 0) {
                    ...
                    if (!isShot && !isAmmo) // if( dt != 1014 )
                    {
                        ...
                        if (dt == gsn_frenzy) {
                            ...
                            if (!IS_NPC(victim) && victim->class == KNIGHT && victim->pcdata->prime_attr == APPLY_STR) {
    ...
    if (dam > 0 && dt != gsn_lifeshare && dt != gsn_kick && dt != 1014 && dt != 1016 && dt != gsn_firearm_mastery // head shot
        && dt != gsn_dart_mastery                                                                                 // coin dart
        && dt != 146                                                                                              // feed
        && is_wielding_poisoned(ch, wpn) && !IS_SET(victim->special, NO_POISON) && !IS_SET(race_table[victim->race].race_abilities, RACE_NO_POISON))
    ...
    if (dam > 0 && (dt == 1014 || dt == 1016 || dt == gsn_firearm_mastery) && dt != gsn_lifeshare && is_wielding_poisoned(ch, wpn) && !IS_SET(victim->special, NO_POISON) && !IS_SET(race_table[victim->race].race_abilities, RACE_NO_POISON))
    //&& !IS_SET( race_table[victim->race].race_abilities , RACE_UNDEAD )
    //&& victim->race != race_lookup ( "Vampire" )
    //&& victim->race != race_lookup ( "Undead" ) )
    {
    ...
    if (IS_NPC(victim)) {
        ...
        if (!(get_eq_char(ch, wpn)) && dt != 1000) {
            ...
            if (dt != 1014 && get_eq_char(ch, wpn) && (dt == gsn_bashhead || dt == gsn_frenzy || dt == gsn_empslash || dt == gsn_backstab || dt == gsn_circle || dt == gsn_circular)) {
                OBJ_DATA *wp_dt;
    ...
    if (is_affected(victim, gsn_blood_thirsty) && affected_value(victim, gsn_blood_thirsty, APPLY_SAVING_LIFE) > 0 && get_skill_percent(victim, gsn_blood_thirsty) > 0 && victim->hit < 1) {
        victim->hit = 1;
    }
    ...
    if (!isShot     //  dt != 1014
        ...
        && (dt >= TYPE_HIT || dt == gsn_kick || dt == gsn_frenzy || dt == gsn_backstab || dt == gsn_circle) && (dam > -1) && IS_AFFECTED(victim, AFF_FLAMING) && victim != ch && victim->hit > 0 && victim->mana > 7) {
        victim->mana -= 7;
        ...
        if (victim->deleted || ch->deleted || !ch->in_room || ch->in_room != victim->in_room) {
    ...
    if (victim->position == POS_DEAD) {
        // 攻擊方取得經驗值
        if (orig != victim) { // 這邊是 ch(victim) 被 orig 的反向打死 -_-
            group_gain(orig, victim);
        } else { // 這邊是 ch 做掉 orig(victim)
            group_gain(ch, orig);
        }
...
void magic__damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn) {
    ...
    if (IS_AFFECTED(victim, AFF_MYSTIC_ARMOR) && ch != victim && dt != gsn_fire_breath && dt != gsn_acid_breath && dt != gsn_frost_breath && dt != gsn_gas_breath && dt != gsn_lightning_breath) {
     ...
     */
    if (!IS_NPC(victim) && !IS_SET(victim->act, PLR_HORSE) && !victim->desc && !IS_SET(victim->pcdata->ride, 2)) {
        if (number_range(0, victim->wait) == 0) {
            do_recall(victim, "");
            return;
        }
...
int magic_damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn) {
    ...
    if (dt != gsn_vampiric_bite && dt != gsn_fire_breath && dt != gsn_acid_breath && dt != gsn_frost_breath && dt != gsn_gas_breath && dt != gsn_lightning_breath)
        dam = check_spellfury(ch, victim, dam);
...
if (    IS_AFFECTED( victim, AFF_SANCTUARY ) || IS_SET( race_table[ victim->race ].race_abilities,RACE_SANCT ) )
{
    ...
    // 龍息效果加成 2022/04/06
    if (dam > 0 && (dt == gsn_acid_breath || dt == gsn_fire_breath || dt == gsn_frost_breath || dt == gsn_gas_breath || dt == gsn_lightning_breath)) {
        AFFECT_DATA af;
        ...
        if (dt == gsn_gas_breath) {
            ...
            if (affected_value(victim, dt, APPLY_SAVING_POISON) < nSvSDiff * (1 + nBreathDecay) / (1 + get_curr_con(victim))) {
                ...
                if (!affected_value(victim, dt, APPLY_SAVING_POISON) > 0 && !IS_SET(race_table[victim->race].race_abilities, RACE_NO_POISON) && !IS_SET(victim->special, NO_POISON)) {
