...
int oprog_name_to_type(char *name) {
    if (!str_cmp(name, "obj_act"))
        return OBJ_ACT;
    if (!str_cmp(name, "obj_repop"))
        return OBJ_REPOP;
    if (!str_cmp(name, "obj_break"))
        return OBJ_BREAK;
    if (!str_cmp(name, "obj_rand"))
        return OBJ_RAND;
    if (!str_cmp(name, "obj_hit"))
        return OBJ_HIT;
    if (!str_cmp(name, "obj_block"))
        return OBJ_BLOCK;
    if (!str_cmp(name, "obj_fight"))
        return OBJ_FIGHT;
    if (!str_cmp(name, "obj_defun"))
        return OBJ_DEFUN;
    if (!str_cmp(name, "obj_load"))
        return OBJ_LOAD;
    if (!str_cmp(name, "obj_disappear"))
        return OBJ_DISAPPEAR;
    if (!str_cmp(name, "obj_command"))
        return OBJ_COMMAND;
    if (!str_cmp(name, "obj_tick"))
        return OBJ_TICK;
    if (!str_cmp(name, "obj_kill"))
        return OBJ_KILL; // add at 2022/05/08
