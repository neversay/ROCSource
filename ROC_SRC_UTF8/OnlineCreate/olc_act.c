...
bool aedit_show(CHAR_DATA *ch, char *argument) {
    ...
    sprintf(buf, "Death:   [%5d] %s\n\r", pArea->death,
            get_room_index(pArea->death)
                ? get_room_index(pArea->death)->name
                : "none");
    send_to_char(buf, ch);
...
bool redit_create(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_IMMORTAL(ch))
        return pplredit_create(ch, argument);
    EDIT_ROOM(ch, pRoom);
    ...
    if ((value = flag_value(extra_flags, arg)) != NO_FLAG) {
        if (!IS_IMMORTAL(ch)) {
            ...
            */
            ...
            if (!check_clco(extra_cost, arg, ch))
                return FALSE;
            pObj->cost += cost;
        }
    ...
    if ((value = flag_value(wear_flags, arg)) != NO_FLAG) {
        if (!IS_IMMORTAL(ch) && str_cmp(arg, "take")) {
            ...
            if (pObj->item_type == ITEM_WEAPON) {
                if (str_cmp(arg, "wield") && str_cmp(arg, "twohanded")) {
            ...
            if (IS_SET(pObj->wear_flags, value))
                cost *= -1;
            if (!check_cost(ch, cost)) {
                send_to_char("你的幫費不夠了。\n\r", ch);
                return FALSE;
...
bool oedit_weight(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_IMMORTAL(ch)) {
        if (pObj->item_type == ITEM_FOUNTAIN || pObj->item_type == ITEM_CONTAINER || pObj->item_type == ITEM_DRINK_CON) {
...
bool oedit_cost(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_IMMORTAL(ch)) {
        if (pObj->item_type == ITEM_CONTAINER || pObj->item_type == ITEM_DRINK_CON) {
...
bool oedit_create(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_IMMORTAL(ch))
        return pploedit_create(ch, argument);
...
bool medit_create(CHAR_DATA *ch, char *argument) {
    ...
    if (!IS_IMMORTAL(ch))
        return pplmedit_create(ch, argument);
    one_argument(argument, arg);
    if (arg[0] == '\0') {
        sprintf(output, "用法：  create [0 < vnum < %d]\n\r", INT_MAX);
        send_to_char(output, ch);
