...
void show_olc_cmds(CHAR_DATA *ch, const struct olc_cmd_type *olc_table) {
    ...
    for (cmd = 0; olc_table[cmd].name[0] != '\0'; cmd++) {
        if (olc_table[cmd].immonly && !IS_IMMORTAL(ch))
            continue;
...
void aedit(CHAR_DATA *ch, char *argument) {
    ...
    for (cmd = 0; *aedit_table[cmd].name; cmd++) {
        if (!str_prefix(command, aedit_table[cmd].name)) {
            if (aedit_table[cmd].immonly && !IS_IMMORTAL(ch))
                continue;
...
void redit(CHAR_DATA *ch, char *argument) {
    ...
    for (cmd = 0; *redit_table[cmd].name; cmd++) {
        if (redit_table[cmd].immonly && !IS_IMMORTAL(ch))
            continue;
...
void oedit(CHAR_DATA *ch, char *argument) {
    ...
    for (cmd = 0; *oedit_table[cmd].name; cmd++) {
        if (oedit_table[cmd].immonly && !IS_IMMORTAL(ch))
            continue;
        ...
        case 'E':
            ...
            if (pMob->pShop) {
                ...
                sprintf(buf,
                        ...
                        pObj->short_descr,
                        pObj->name,
                        (pReset->command == 'G') ? flag_string(wear_loc_strings, WEAR_NONE)
