...
void game_loop_unix(int control) {
    ...
    while (!merc_down) {
        ...
        for (d = descriptor_list; d; d = d_next) {
            ...
            if (d->character && d->character->pcdata && d->character->pcdata->horse && d->character->pcdata->horse->wait && d->character->pcdata->horse->wait > 0)
     ...
     */
    {
        ...
        for (dcount = descriptor_list, i = 0; dcount; dcount = dcount->next, i++) {
            ...
            for (dcount2 = dcount->next; dcount2; dcount2 = dcount2->next) {
                if (dcount2->connected != CON_PLAYING)
                    continue;
...
void lost_link_quit(CHAR_DATA *ch, char *argument) {
    ...
    for (obj = ch->carrying; obj; obj = obj_next_content) {
        ...
        if (obj->deleted)
            continue;
        if (IS_SET(obj->extra_flags, ITEM_NOSAVE) || (ch->level < (obj->level - 5) && ch->class < 5)) {
            unequip_char(ch, obj);
            obj_from_char(obj);
...
void close_socket(DESCRIPTOR_DATA *dclose) {
    ...
    if ((ch = dclose->character)) {
        ...
        if (dclose->connected == CON_PLAYING) {
            ...
            if (ch->desc && ch->desc->original && ch->desc->original->pcdata && ch->desc->original->pcdata->switched) {
    ...
    while (*str != '\0') {
        ...
        case 'a':
            if (ch->level > 10)
                ...
                sprintf(buf2, "%s", IS_GOOD(ch) ? "善良" : IS_EVIL(ch) ? "邪惡"
                                                                       : "中立");
            i = buf2;
            break;
        case 'A':
        ...
        case 'b':
            if (ch->level > 10) {
                ...
                if (ch->alignment > 799)
                    ...
                    sprintf(buf2, "[m[31m");
                sprintf(buf3, "%s[m", IS_GOOD(ch) ? "善良" : IS_EVIL(ch) ? "邪惡"
                                                                          : "中立");
            }
            strcat(buf2, buf3);
            i = buf2;
        ...
        case 'i': /* Invisible notification on prompt sent by Kaneda */
            sprintf(buf2, "%s|%s|%s", IS_AFFECTED(ch, AFF_INVISIBLE) ? "隱形" : "",
                    IS_AFFECTED(ch, AFF_HIDE) ? "藏匿" : "",
                    IS_AFFECTED(ch, AFF_NIGHTSTALK) ? "暗夜" : "");
            i = buf2;
            break;
        case 't':
...
#endif
        {
            perror("Write_to_descriptor_Test");
            return FALSE;
        }
    }
...
void nanny(DESCRIPTOR_DATA *d, char *argument) {
    ...
    case CON_GET_NAME:
        ...
        for (check1 = descriptor_list; check1; check1 = check1->next) {
            ...
            if (d == check1)
                continue;
            if ((!check1->character || check1->character->deleted || IS_NPC(check1->character)) &&
                (!check1->original || !check1->original->name))
                continue;

            if (!str_cmp(d->character->name, check1->character->name) && (!IS_IMMORTAL(check1->character) || (check1->original && !str_cmp(check1->original->name, d->character->name))))
        ...
        for (check = char_list; check; check = check->next) {
            if (check->deleted || IS_NPC(check) || IS_SET(check->act, PLR_HORSE))
                continue;
            if (!str_cmp(d->character->name, check->name) && !IS_IMMORTAL(check))
                if (check->desc) {
                    write_to_buffer(d, "現在線上有這個人的存在。\n\r請重新輸入名字：", 0);
                    d->connected = CON_GET_NAME;
                    extract_char(ch, TRUE); // Added by Razgriz   20050802
        ...
        if (check_reconnect(d, argument, FALSE)) {
            ...
            if (wizlock
                //&& !IS_HERO( ch )
                && !IS_SET(ch->act, PLR_WIZBIT) && get_trust(ch) < L_APP) {
                write_to_buffer(d, "本遊戲目前只允許巫師登入。\n\r", 0);
                close_socket(d);
                return;
            }
            if (ch->level <= numlock && !IS_SET(ch->act, PLR_WIZBIT) && get_trust(ch) < L_APP && numlock != 0) {
                write_to_buffer(d,
    ...
    case CON_GET_NEW_RACE:
        for (iRace = 0; iRace < MAX_RACE; iRace++)
            if (!str_prefix(argument, race_table[iRace].name) && IS_SET(race_table[iRace].race_abilities,
                                                                        RACE_PC_AVAIL)) {
    ...
    case CON_READ_MOTD:
        ...
        if (ch->level == 0) {
            ...
            switch (ch->class) {
            case MAGE:
                ch->pcdata->prime_attr = APPLY_INT;
                ch->pcdata->snd_attr   = APPLY_NONE;
                break;
            case CLERIC:
                ch->pcdata->prime_attr = APPLY_WIS;
                ch->pcdata->snd_attr   = APPLY_NONE;
                break;
            case THIEF:
                ch->pcdata->prime_attr = APPLY_DEX;
                ch->pcdata->snd_attr   = APPLY_NONE;
                break;
            case WARRIOR:
                ch->pcdata->prime_attr = APPLY_STR;
                ch->pcdata->snd_attr   = APPLY_NONE;
                break;
            case BARBARIAN:
            ...
            ch->level = 1;
            ch->exp   = 1000;
            ch->gold  = 10000 + number_fuzzy(3) * number_fuzzy(4) * number_fuzzy(5) * 10;
            ch->hit   = ch->max_hit;
            ch->mana  = ch->max_mana;
            ch->move  = ch->max_move;
            sprintf(buf, "- %s",
...
break;
 ...
 case 'R': */
/* refresh current page of text */         /*
toggle = 1;
break;
        
case 'B': */
/* scroll back a page of text */           /*
toggle = 2;
break;
          
default: */
/*otherwise, stop the text viewing */      /*
if ( d->showstr_head )
{
...
void act(const char *format, CHAR_DATA *ch, const void *arg1,
    ...
    for (; to; to = to->next_in_room) {
        if ((to->deleted) || (!to->desc && IS_NPC(to) && !(to->pIndexData->progtypes & ACT_PROG)) || !IS_AWAKE(to))
            continue;
...
void combat_info_message(const char *format, CHAR_DATA *to, CHAR_DATA *ch, CHAR_DATA *vch) {
    ...
    while (*str != '\0') {
        ...
        if (!vch && *str >= 'A' && *str <= 'Z') {
            ...
            switch (*str) {
            default:
                bug("comm.c combat_info_message: bad code %d.", *str);
