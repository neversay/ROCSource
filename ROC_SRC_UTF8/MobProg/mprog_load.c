...
int mprog_name_to_type(char *name) {
    ...
    if (!str_cmp(name, "bribe_prog"))
        return BRIBE_PROG;
    if (!str_cmp(name, "talk_prog"))
        return TALK_PROG;
    if (!str_cmp(name, "tick_prog"))
        return TICK_PROG;
    if (!str_cmp(name, "repop_prog"))
        return REPOP_PROG;
    if (!str_cmp(name, "defun_prog"))
        return DEFUN_PROG;
    if (!str_cmp(name, "hurt_prog"))
        return HURT_PROG;
    if (!str_cmp(name, "kill_prog"))
        return KILL_PROG;
    if (!str_cmp(name, "entry_greet_prog"))
