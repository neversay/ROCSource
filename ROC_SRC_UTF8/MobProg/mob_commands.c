...
void do_self_skill(CHAR_DATA *ch, char *argument) {
    ...
    if (!str_cmp(arg0, "all")) {
        ...
        for (target = ch->in_room->people; target; target = next_target) {
            ...
            if (target == ch)
                continue;
            if (IS_NPC(target) && target->fighting != ch)
                continue;
...
void do_mpteleport(CHAR_DATA *ch, char *argument) {
    ...
    if (!str_cmp(arg1, "all")) {
        ...
        {
            pRoomIndex = get_room_index(number_range(0, MAX_VNUM));
            if (pRoomIndex && pRoomIndex->area && IS_SET(pRoomIndex->area->area_flags, AREA_CLAN))
                pRoomIndex = NULL;
            // count++;
        }
        ...
        {
            pRoomIndex = get_room_index(number_range(0, MAX_VNUM));
            if (pRoomIndex && pRoomIndex->area && (IS_SET(pRoomIndex->area->area_flags, AREA_CLAN) || (pRoomIndex->area->recall != 3001)))
                pRoomIndex = NULL;
            // count++;
        }
        ...
        {
            pRoomIndex = get_room_index(number_range(0, MAX_VNUM));
            if (pRoomIndex && pRoomIndex->area && (IS_SET(pRoomIndex->area->area_flags, AREA_CLAN) || pRoomIndex->area->recall != 4580))
                pRoomIndex = NULL;
            // count++;
        }
        ...
        {
            pRoomIndex = get_room_index(number_range(0, MAX_VNUM));
            if (pRoomIndex && pRoomIndex->area && (IS_SET(pRoomIndex->area->area_flags, AREA_CLAN) || pRoomIndex->area->recall != 14229))
                pRoomIndex = NULL;
            // count++;
        }
    ...
    if (is_number(arg2)) {
        if (!str_prefix("all.vnum", arg1)) {
            ...
            for (obj = target->carrying; obj; obj = obj_next) {
                ...
                if (obj->pIndexData->vnum == objvnum && can_see_obj(ch, obj) && obj->wear_loc == WEAR_NONE && can_drop_obj(ch, obj)) {
                    obj_from_char(obj);
                    extract_obj(obj);
                }
            }
    ...
    if (!str_cmp(type, "exp")) {
        for (gch = victim->in_room->people; gch; gch = gch->next_in_room) {
            ...
            if (is_same_group(victim, gch)) {
                if (((!str_cmp(level, "basic")) && gch->class < 5) || (lv < 0 && gch->level + lv < 1) || (lv > 0 && gch->level - lv > -1)) {
                    gain_exp(gch, v);
    ...
    if ((target = get_char_world(ch, arg))) {
        ...
        if (is_number(arg)) {
            ...
            for (obj = target->carrying; obj; obj = obj_next) {
                ...
                if (obj->wear_loc == WEAR_NONE)
                    continue;
                if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(target)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(target)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(target))) {
...
void do_mpget(CHAR_DATA *ch, char *argument) {
    ...
    for (obj = rch->carrying; obj; obj = obj_next) {
        ...
        if (IS_SET(obj->extra_flags, ITEM_PERSONAL) || IS_SET(obj->pIndexData->souvenir, Z4) || IS_SET(obj->wear_flags, ITEM_WEAR_RIDE) || obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_BOAT)
            continue;

        // 當有指定物品時而obj不是所指定物品不拿
        // 當沒有指定物品時由機率來挑物品
        if ((obj->wear_flags != value2 && value2 != int_type) || (value2 == int_type && (number_percent() >= number_percent())))
            continue;
