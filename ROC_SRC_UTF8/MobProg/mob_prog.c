    ...
    *argpt = '\0';
    ...
    if (!str_cmp(buf, "mpmax")) {
        if (arg[0] == '$')
            ...
            case 'n':
                if (actor)
                    return mprog_veval(actor->max_mana, opr, atoi(val), buf);
                else
                    return -1;
            case 't':
                if (vict)
                    return mprog_veval(vict->max_mana, opr, atoi(val), buf);
                else
                    return -1;
            case 'r':
                if (rndm)
                    return mprog_veval(rndm->max_mana, opr, atoi(val), buf);
                else
                    return -1;
            case 'b':
                if (remb)
                    return mprog_veval(remb->max_mana, opr, atoi(val), buf);
                else
                    return -1;
            default:
                if (mob)
    ...
    */
    ...
    if (!str_cmp(buf, "carrycount")) {
        ...
        if (!is_number(arg2)) {
            ...
            count  = 0;
            for (obj = vch->carrying; obj; obj = obj->next_content) {
                if (obj->wear_loc == WEAR_NONE && can_see_obj(vch, obj) && is_pfxname(arg, obj->name)) {
                    // if ( ++count == number )
                    countnum++;
                }
            }
            ...
            number = atoi(arg2);
            for (obj = vch->carrying; obj; obj = obj->next_content) {
                if (obj->pIndexData->vnum == number && obj->wear_loc == WEAR_NONE && can_see_obj(vch, obj)) {
                    countnum++;
                }
            }
        }
            ...
            if (room)
                ...
                if (obj->carried_by)
                    if (room_is_dark(obj->carried_by->in_room))
                        ...
                        return TRUE;
                else if (room_is_dark(obj->in_room))
                    return FALSE;
                else
                    return TRUE;
            }
    ...
    if (!str_cmp(buf, "hasdoor")) {
        ...
        if (mob)
            return (mob->in_room->exit[door]) ? TRUE : FALSE;
    ...
    if (!str_cmp(buf, "wp_dr")) {
        if (arg[0] == '$')
            ...
            case 'i':
                rhsvl = atoi(val);
                if (mob)
                    return mprog_veval(get_damroll(mob, (get_eq_char(mob, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, rhsvl, buf);
                else
                    return -1;
            case 'n':
                if (actor)
                    return mprog_veval(get_damroll(actor, (get_eq_char(actor, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'v': // 2022/10/08
                if (mob && mob->fighting) {
                    return mprog_veval(get_damroll(mob->fighting, (get_eq_char(mob->fighting, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                } else if (obj && obj->carried_by && actor && actor->fighting) {
                    return mprog_veval(get_damroll(actor->fighting, (get_eq_char(actor->fighting, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                } else
                    return -1;
            case 't':
                if (vict)
                    return mprog_veval(get_damroll(vict, (get_eq_char(vict, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'r':
                if (rndm)
                    return mprog_veval(get_damroll(rndm, (get_eq_char(rndm, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'b':
                if (remb)
                    return mprog_veval(get_damroll(remb, (get_eq_char(remb, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            default:
                if (mob)
    ...
    if (!str_cmp(buf, "wp_hr")) {
        if (arg[0] == '$')
            ...
            case 'i':
                rhsvl = atoi(val);
                if (mob)
                    return mprog_veval(get_hitroll(mob, (get_eq_char(mob, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, rhsvl, buf);
                else
                    return -1;
            case 'n':
                if (actor)
                    return mprog_veval(get_hitroll(actor, (get_eq_char(actor, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'v': // 2022/10/08
                if (mob && mob->fighting) {
                    return mprog_veval(get_hitroll(mob->fighting, (get_eq_char(mob->fighting, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                } else if (obj && obj->carried_by && actor && actor->fighting) {
                    return mprog_veval(get_hitroll(actor->fighting, (get_eq_char(actor->fighting, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                } else
                    return -1;
            case 't':
                if (vict)
                    return mprog_veval(get_hitroll(vict, (get_eq_char(vict, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'r':
                if (rndm)
                    return mprog_veval(get_hitroll(rndm, (get_eq_char(rndm, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            case 'b':
                if (remb)
                    return mprog_veval(get_hitroll(remb, (get_eq_char(remb, WEAR_TWO_HAND) ? WEAR_TWO_HAND : WEAR_WIELD)), opr, atoi(val), buf);
                else
                    return -1;
            default:
                if (mob)
...
void mprog_percent_check(CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, int type) {
    ...
    for (mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next) {
        ...
        int tmp_number_percent = number_percent();
        if ((mprg->type & type) && ((atoi(mprg->arglist) >= 100) || (tmp_number_percent < atoi(mprg->arglist)))) {
            mprog_driver(mprg->comlist, mob, actor, obj, vo);
...
void mprog_give_trigger(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj) {
    ...
    if (IS_NPC(mob) && (mob->pIndexData->progtypes & GIVE_PROG))
        for (mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next) {
            ...
            if ((mprg->type & GIVE_PROG) && ((!str_cmp(obj->name, mprg->arglist)) || (obj->pIndexData->vnum == atoi(mprg->arglist)) || (!str_cmp("all", buf)))) {
...
void old_mprog_greet_trigger(CHAR_DATA *ch) {
    ...
    for (vmob = ch->in_room->people; vmob != NULL; vmob = vmob_next_in_room) {
        ...
        if (IS_NPC(vmob) && ch != vmob && can_see(vmob, ch)
            ...
            //&& ( vmob->fighting == NULL )
            && (vmob->fighting == NULL || (is_qmark(vmob, "enable_fight_greet"))) && IS_AWAKE(vmob) && (vmob->pIndexData->progtypes & GREET_PROG) && !IS_AFFECTED(ch, AFF_NIGHTSTALK))
            mprog_percent_check(vmob, ch, NULL, NULL, GREET_PROG);
        else if (IS_NPC(vmob)
                 // 新增 mob 有 enable_fight_greet 的 qmark 時 能在戰鬥中判定 all_greet_prog 2021/12/19
                 //&& ( vmob->fighting == NULL )
                 && (vmob->fighting == NULL || (is_qmark(vmob, "enable_fight_greet"))) && IS_AWAKE(vmob) && (vmob->pIndexData->progtypes & ALL_GREET_PROG))
        ...
        for (mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next) {
            ...
            */
            if (mprg->type & HURT_PROG) {
                ...
                if (!(is_qmark(mob, "disable_hurt_element")) && (dt == 314      // gsn_magic_enchantment
                ...
                if (!(is_qmark(mob, "disable_hurt_all"))) {
                    ...
                    if (dt == 314 || dt == 315 || dt == 316 || dt == 317 || dt == 318 || dt == 319 || dt == 320 || dt == 321) {
                        return;
                    }
