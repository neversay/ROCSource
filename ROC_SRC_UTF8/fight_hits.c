...
void multi_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt) {
    ...
    if ((protector = victim->protector) && protector != victim) {
        for (; protector && protector != protector->next_in_protect; protector = protector->next_in_protect) {
            if (protector != ch && protector != victim && ch->in_room && protector->in_room == ch->in_room) {
                if ((IS_NPC(protector) && ((protector->level + GetGeneralMobSklv(protector) * 24) / 2 > number_percent())) || (!IS_NPC(protector) && (get_skill_percent(protector, gsn_protect) + get_skill_level(protector, gsn_protect) * 5 > number_percent()))) {
                    // modify by jye
                    // 修改為不stop_fighting
    ...
    if (dt == gsn_meteoric_arrow) {
        ...
        if ((arrow = get_eq_char(ch, WEAR_ARROW)) && arrow->item_type == ITEM_ARROW && arrow->value[0] > 0) {
            mydt = TYPE_UNDEFINED;
        } else {
            // npc 開戰的第一回合會觸發 fight_prog 22022/10/31
            if (ch->fighting && victim->fighting && !ch->deleted && !victim->deleted) {
    ...
    if (!ch->fighting || !ch->in_room || ch->in_room != victim->in_room || dt == gsn_backstab || dt == gsn_circle) {
    ...
    // dual 有穿 次要武器的情況
    if ((wield = get_eq_char(ch, WEAR_WIELD_2)) && wield->item_type == ITEM_WEAPON && ((IS_NPC(ch) && IS_SET(ch->act, ACT_DUAL)) || (!IS_NPC(ch) && get_skill_percent(ch, gsn_dual)))) {
        ...
        if (!ch->fighting || ch->fighting != victim || !ch->in_room || ch->in_room != victim->in_room || dt == gsn_backstab || dt == gsn_circle) {
            if (is_brief(ch, victim))
    ...
    if (!IS_NPC(ch) && IS_SET(race_table[ch->race].race_abilities, RACE_LIZARDMAN) && dt != gsn_meteoric_arrow) {
        ...
        if (!victim->deleted) {
            //  噴酸攻擊
            if (ch->pcdata->condition[COND_DRUNK] > 10 && number_percent() < UMAX(40, ch->pcdata->condition[COND_DRUNK] * 2) + nSklv * (nSklv + 4) / 2) {
                AFFECT_DATA af;
                int         nSVE        = NEW_GET_SVS(victim, MAGIC_EARTH);
                int         nPoisonRate = 0;
                if (nSklv > 3)
                    nPoisonRate = 5 * (nSklv - 3);
                int nBlindRate = 0;
                if (nSklv > 5)
                    nBlindRate = 7;
                int nDamage = dice(4 + nSklv, ch->level + nSklv) + ch->amdr * nSklv / 5;

                nPoisonRate = nPoisonRate * (100 + nSVE) / 100;
                if (nPoisonRate < 0)
                    nPoisonRate = 0;
                nBlindRate = nBlindRate * (100 + nSVE) / 100;
                if (nBlindRate < 0)
                    nBlindRate = 0;
                ...
                if (number_percent() < nPoisonRate && !IS_SET(victim->special, NO_POISON)) {
                    ...
                    if (affected_value(victim, gsn_poison, APPLY_SAVING_POISON) < (14 + nSklv * 6) * 10) {
                        ...
                        af.modifier = ((14 + nSklv * 6) * 25 / get_curr_con(victim)) * weakness_damage(victim, gsn_poison) * 0.01;
                        affect_join(victim, &af);
                    }
                ...
                if (number_percent() < nBlindRate && !IS_AFFECTED(victim, AFF_BLIND) && !IS_SET(victim->special, NO_BLIND) && !(is_affected(victim, gsn_invoke) && get_skill_level(victim, gsn_invoke) > 2)) {
                    // 天賦等級六以上、有機會讓敵人盲目
        ...
        if (!victim->deleted) { // 掃尾攻擊
            ...
            if (number_percent() < nTailAttackRate) {
                if (mone_hit(ch, victim, &gsn_tactics, -2, MSG_NORMAL, DT_NOCHANGE) >= 0 && !victim->deleted && number_percent() < nTripRate) {
                    trip(ch, victim);
                }
            }
            if (nSklv > 3) {
                for (vch = ch->in_room->people; vch; vch = vch_next) {
                    ...
                    if (number_percent() < (nTailAttackRate / 2)) {
                        if (mone_hit(ch, vch, &gsn_tactics, -2, MSG_NORMAL, DT_NOCHANGE) >= 0 && !vch->deleted && number_percent() < nTripRate) {
                            trip(ch, vch);
                        }
                    }
                }
...
int get_critical_damage(int dam, CHAR_DATA *ch, CHAR_DATA *victim, int dexdiff, bool fCritical, int dt, int darkraid) {
    ...
    if ((!IS_NPC(ch)                                      // PPL case
        ...
        || ((IS_NPC(ch)                                                                  // NPC case
             && (IS_SET(ch->act, ACT_CRITICAL) || IS_SET(ch->class, 4)) && (dt != 1014)) // arrow
...
int mone_hit(CHAR_DATA *ch, CHAR_DATA *victim, int *dt_orig, int wpn, int msg_mode, int dt_mode) {
    ...
    if (!IS_NPC(ch) && ch->class == MARTIALMAN && get_skill_percent(ch, gsn_blood_thirsty) > 0 && is_affected(ch, gsn_blood_thirsty) && affected_value(ch, gsn_blood_thirsty, APPLY_SAVING_LIFE) > 0) {
        isBloodThirsty = TRUE;
    }
     ...
     */
    if (victim->position == POS_DEAD || victim->hit <= 0 // 加入 hit <= 0 時也會 return 的判定 2022/12/30
    ...
    if (!IS_NPC(ch) && IS_SET(ch->pcdata->autocombat, AUTOC_CRITICAL) && (ch->move > 80 - get_skill_level(ch, gsn_deadly_strike) * 5)) {
...
chance += get_skill_percent( ch, gsn_deadly_strike ) / 3 + get_skill_level(ch, gsn_deadly_strike); */
    ...
    if (!wield) {
        if ((wield = get_eq_char(ch, WEAR_TWO_HAND))) { // 雙手武器
            ...
            if (wield->value[3] == 14) { // 武器攻擊型態為 射出箭矢 shot
                ...
                // ch 沒被打, 且還有箭, 處理箭隻消耗
                if (!vch && (arrow = get_eq_char(ch, WEAR_ARROW)) && arrow->item_type == ITEM_ARROW && arrow->value[0] > 0) {
                    isShootCost = TRUE;
                    ...
                    } else { //  ch 被打, ch 攻擊型態轉為 揮舞劈砍 slash
                        arrow = NULL;
                        dt    = 1003;
                        if (dt_mode == DT_CHANGE)
                            *dt_orig = dt;
    ...
    if (dt == 1016) {
        ...
        ammo = get_eq_char(ch, WEAR_AMMO);
        if (ammo && ammo->item_type == ITEM_AMMO && (ammo->value[0] > 0 || ammo->value[0] == -1)) // 加入 -1 的情況 2023/01/05
        {
    ...
    if (wield) {
        ...
        if (isReposte && ch->hitroll > 0)
            whr += (ch->hitroll * (25 + get_skill_level(ch, gsn_reposte) * 20 + get_skill_level(ch, gsn_reposte) * get_skill_level(ch, gsn_reposte))) / 100;
        ...
        if (dt == gsn_backstab && !IS_NPC(ch) && (ch->class == ASSASSIN || ch->class == NINJA || ch->class == RANGER)) {
        ...
        if (dt == 1014 && arrow != NULL && !IS_NPC(ch)) {
            ...
            nHrBonus = (nSklvShot * nSklvShot * nSklvShot / 10 + nSklvShot * (nSklvShot + 4) + nSklvShot * 3) * nHrBonus / 100 * ((nCurrDex + 5) * (nCurrDex + 5)) / 900;
        ...
        thac0 = interpolate(ch->level, thac0_00, thac0_42) - race_hit_bonus
        ...
        if (!IS_NPC(ch) && ch->pcdata->learned[gsn_weapon_master] > 0) {
            ...
            if (get_vrace_hrp(victim, wield) > 100) {
                thac0 -= (whr + wield->material->value[0]) * get_vrace_hrp(victim, wield) * (1 + nWmSklv * (nWmSklv + 2) * nCurrInt / 2000) / 100;
            } else {
            ...
            thac0 = thac0 - (whr + wield->material->value[0]) * get_vrace_hrp(victim, wield) / 100 - get_vrace_hrq(victim, wield);
        }
        ...
        */
        ...
        int phr = (ch->hitroll + (str_app[get_curr_str(ch)].tohit * 6 + dex_app[get_curr_dex(ch)].tohit * 5 + wis_app[get_curr_wis(ch)].tohit * 3) / 100);

        //  reposte 會隨 hitroll 及技能等級變動命中加成 2022/01/13
        //  reposte 判定改以 看 isReposte 的 bool 2022/05/07
        // if(dt == gsn_reposte && ch->hitroll > 0)
        if (isReposte && ch->hitroll > 0)
            phr += (ch->hitroll * (25 + get_skill_level(ch, gsn_reposte) * 20 + get_skill_level(ch, gsn_reposte) * get_skill_level(ch, gsn_reposte))) / 100;
        ...
        thac0 = interpolate(ch->level, thac0_00, thac0_42) - phr - race_hit_bonus - ((dt == gsn_frenzy) ? UMIN((ch->level - 56), -8) : 0)
                // archery 0 , thac0 = 55
                - ((arrow != NULL) ? (!IS_NPC(ch)
                                          ? get_skill_percent(ch, gsn_shot_arrow) / 4 + 3 + get_skill_level(ch, gsn_shot_arrow) * 2 + (get_skill_level(ch, gsn_shot_arrow) / 6) * 2
                                      : IS_SET(ch->class, 16) ? 20
                                                              : -60)
                                   : 0);
    }
    ...
    if (!IS_NPC(ch)) {
        if (IS_SET(ch->act, PLR_FIST_FIGHT) && !isLizTail && number_percent() < get_skill_percent(ch, gsn_fist_fight)) {
    ...
    if (dt == gsn_frenzy || dt == gsn_backstab || dt == gsn_bashhead || dt == gsn_circular || dt == gsn_tactics // lizardman 掃尾
        || dt == gsn_empslash) {
    ...
    if (wield && wield->value[3] == 1) // 銳利切割
    {
        if (weakness_percent(victim, gsn_slice_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_slice_attack) + (weakness_percent(victim, gsn_slice_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 2) // 威猛一戳
    {
        if (weakness_percent(victim, gsn_stab_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_stab_attack) + (weakness_percent(victim, gsn_stab_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 3) // 揮舞劈砍
    {
        if (weakness_percent(victim, gsn_slash_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_slash_attack) + (weakness_percent(victim, gsn_slash_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 4) // 灼熱一鞭
    {
        if (weakness_percent(victim, gsn_whip_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_whip_attack) + (weakness_percent(victim, gsn_whip_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 5) // 用力一抓
    {
        if (weakness_percent(victim, gsn_claw_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_claw_attack) + (weakness_percent(victim, gsn_claw_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 6) // 爆發噴擊
    {
        if (weakness_percent(victim, gsn_blast_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_blast_attack) + (weakness_percent(victim, gsn_blast_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 7) // 萬鈞力搥
    {
        if (weakness_percent(victim, gsn_pound_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_pound_attack) + (weakness_percent(victim, gsn_pound_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 8) // 狠狠碾壓
    {
        if (weakness_percent(victim, gsn_crush_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_crush_attack) + (weakness_percent(victim, gsn_crush_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 9) // 犀利一剁
    {
        if (weakness_percent(victim, gsn_grep_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_grep_attack) + (weakness_percent(victim, gsn_grep_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 10) // 張口猛咬
    {
        if (weakness_percent(victim, gsn_bite_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_bite_attack) + (weakness_percent(victim, gsn_bite_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 11) // 尖銳刺擊
    {
        if (weakness_percent(victim, gsn_pierce_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_pierce_attack) + (weakness_percent(victim, gsn_pierce_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 12) // 猛烈吸吮
    {
        if (weakness_percent(victim, gsn_suction_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_suction_attack) + (weakness_percent(victim, gsn_suction_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (wield && wield->value[3] == 13) // 剁菜連斬
    {
        if (weakness_percent(victim, gsn_chop_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_chop_attack) + (weakness_percent(victim, gsn_chop_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
            thac0 = thac0 * weakness_percent(victim, gsn_chop_attack) / 100;
    }
    if (wield // 射出箭矢 , 槍械射擊
        && ((wield->value[3] == 14 && arrow) || (wield->value[3] == 16 && ammo))) {
        if (weakness_percent(victim, gsn_shot_attack) > 100)
            thac0 = thac0 * (weakness_percent(victim, gsn_shot_attack) + (weakness_percent(victim, gsn_shot_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 15) / 100;
        else
    ...
    if (!IS_NPC(ch) || !IS_SET(ch->act, ACT_SPEEDHIT)) {
        if (dt != gsn_circle && dt != gsn_twin && ((diceroll != 0 && diceroll < thac0 - victim_dodge) || diceroll == -30)) {
            /* Miss. */
            // 進來這邊就代表沒打到了
            ...
            if (msg_mode == MSG_NORMAL)
                dam_message(ch, victim, -1, dt, wpn);
            ...
            if (IS_NPC(victim) && !IS_SET(victim->act, ACT_NO_FLEE)) {
                if ((IS_SET(victim->act, ACT_WIMPY) && number_bits(1) == 0 && victim->hit < victim->max_hit / 2 && victim->wait == 0) || ((IS_AFFECTED(victim, AFF_CHARM) || victim->clan || IS_AFFECTED(victim, AFF_SUMMONED)) && victim->master && victim->in_room && victim->master->in_room != victim->in_room)) {
                    do_flee(victim, "");
                }
            }

            // modified by coconet for let ppl in stun can not wimpy 0614
            if (!IS_NPC(victim) && victim->hit > 0 && victim->hit <= victim->wimpy && victim->wait < 3) {
                do_flee(victim, "");
            }
...
if ( dt == gsn_reposte ){
            ...
            */
            ...
            if (wpn == WEAR_TWO_HAND && !arrow && !ammo) {
                if (CAN_WEAR(wield, ITEM_WIELD)) { // item wield 與 twohand 並存時會優先判定為前者(能單手穿)
                    ...
                    dam += dam / 3; // 加成 33%
                    if ((number_percent() < (get_skill_percent(ch, gsn_powerblow) / 5 + get_skill_level(ch, gsn_powerblow) * 10)) || isBloodThirsty) {
                        dam = wield->value[2];
                        dam += dam / 3;
    ...
    if (wield && !IS_NPC(ch) && !isLizTail) {
        ...
        if (arrow != NULL && dt != gsn_frenzy && arrow == wield && bow) {
            ...
            nDamBonus = (get_damroll(ch, wpn) + ShootObjAffect(ch, arrow, 2)) * (nBowAvg * 2 + nArwAvg) * get_vrace_drp(victim, bow) / 5500;
            ...
            dam += ((gun->material->value[1] + wield->material->value[1] + get_vrace_drq(victim, gun) + get_vrace_drq(victim, wield) + nAmmoDam) * 23) / 55;
        } else {
            ...
            if (!IS_NPC(ch) && ch->pcdata->learned[gsn_weapon_master] > 0 && get_vrace_drp(victim, wield) > 100) {
                ...
                int nWmSklv = get_skill_level(ch, gsn_weapon_master);
                dam += get_damroll(ch, wpn) * (nWpnAvg + 44) * fist_dam * get_vrace_drp(victim, wield) * (1 + nWmSklv * (nWmSklv + 2) * get_curr_int(ch) / 2200) / (8 * 5500);
            } else {
            ...
            if (!IS_NPC(ch) && ch->pcdata->learned[gsn_weapon_master] > 0 && get_vrace_drq(victim, wield) > 0) {
                ...
                int nWmSklv = get_skill_level(ch, gsn_weapon_master);
                dam += fist_dam * (nWpnAvg + 44) * get_vrace_drq(victim, wield) * (1 + nWmSklv * (nWmSklv + 4) * get_curr_int(ch) / 2200) / (8 * 55);
            } else {
...
if ( wield && IS_SET( wield->extra_flags, ITEM_POISONED ) && dt != 1014 ){
    ...
    */
    ...
    if (!IS_NPC(ch) && (get_skill_percent(ch, gsn_enhanced_damage) + get_skill_level(ch, gsn_enhanced_damage) * 5 > number_percent() || nTpPerc + nTpSklv * 5 > number_percent()
        ...
        && (dt >= TYPE_HIT || dt == gsn_frenzy
            //|| dt == gsn_kick  /* kick 不會被代入 mone_hit 之中*/
            || dt == gsn_bashhead || dt == gsn_backstab || dt == gsn_circle || dt == gsn_circular || dt == gsn_sprinkle_slash || dt == gsn_flaming_slash || dt == gsn_storm_slash || dt == gsn_earth || dt == gsn_empslash || isLizTail)) {
    ...
    if (dt == gsn_circle) /* 200% to 300% at lev. 40 */
    {
        ...
        dam += dam + (dam * (2 * get_skill_level(ch, gsn_circle) + 3 * get_skill_level(ch, gsn_circle) / 2 + 3 * get_skill_level(ch, gsn_circle) / 5 + 4 * get_skill_level(ch, gsn_circle) / 6) / 14);
    ...
    if (dt == gsn_empslash || dt == gsn_circular || dt == gsn_bashhead || dt == gsn_frenzy) {
    ...
    if (wield && wield->value[3] == 1) // 銳利切割
    {
        if (weakness_damage(victim, gsn_slice_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_slice_attack) + (weakness_damage(victim, gsn_slice_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 2) // 威猛一戳
    {
        if (weakness_damage(victim, gsn_stab_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_stab_attack) + (weakness_damage(victim, gsn_stab_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 3) // 揮舞劈砍
    {
        if (weakness_damage(victim, gsn_slash_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_slash_attack) + (weakness_damage(victim, gsn_slash_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 4) // 灼熱一鞭
    {
        if (weakness_damage(victim, gsn_whip_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_whip_attack) + (weakness_damage(victim, gsn_whip_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 5) // 用力一抓
    {
        if (weakness_damage(victim, gsn_claw_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_claw_attack) + (weakness_damage(victim, gsn_claw_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 6) // 爆發噴擊
    {
        if (weakness_damage(victim, gsn_blast_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_blast_attack) + (weakness_damage(victim, gsn_blast_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 7) // 萬鈞力搥
    {
        if (weakness_damage(victim, gsn_pound_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_pound_attack) + (weakness_damage(victim, gsn_pound_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 8) // 狠狠碾壓
    {
        if (weakness_damage(victim, gsn_crush_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_crush_attack) + (weakness_damage(victim, gsn_crush_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 9) // 犀利一剁
    {
        if (weakness_damage(victim, gsn_grep_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_grep_attack) + (weakness_damage(victim, gsn_grep_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 10) // 張口猛咬
    {
        if (weakness_damage(victim, gsn_bite_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_bite_attack) + (weakness_damage(victim, gsn_bite_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 11) // 尖銳刺擊
    {
        if (weakness_damage(victim, gsn_pierce_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_pierce_attack) + (weakness_damage(victim, gsn_pierce_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 12) // 猛烈吸吮
    {
        if (weakness_damage(victim, gsn_suction_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_suction_attack) + (weakness_damage(victim, gsn_suction_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (wield && wield->value[3] == 13) // 剁菜連斬
    {
        if (weakness_damage(victim, gsn_chop_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_chop_attack) + (weakness_damage(victim, gsn_chop_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if ((bow && bow->value[3] == 14 && arrow) || (gun && gun->value[3] == 16 && ammo)) {
        if (weakness_damage(victim, gsn_shot_attack) > 100)
            dam = dam * (weakness_damage(victim, gsn_shot_attack) + (weakness_damage(victim, gsn_shot_attack) - 100) * get_skill_level(ch, gsn_weapon_master) / 10) / 100;
        else
    ...
    if (msg_mode == MSG_NORMAL || (msg_mode == MSG_BRIEF && !is_brief(ch, victim))) {
    ...
    if (ammo && ammo->value[3] > 0 && ammo->value[3] < MAX_SKILL) {
        if (skill_table[ammo->value[3]].target == TAR_CHAR_SELF && victim != ch) {
            ...
            if (ammo->value[3] == 314 || ammo->value[3] == 315 || ammo->value[3] == 316 || ammo->value[3] == 317 || ammo->value[3] == 318 || ammo->value[3] == 319 || ammo->value[3] == 320 || ammo->value[3] == 321)
                obj_cast_spell(ammo->value[3], ammo->value[2], ch, ch, ammo);
            else
                obj_cast_spell(ammo->value[3], ammo->value[2], ch, ch, ammo);
    ...
    if (!ch->deleted && !victim->deleted && ch->in_room == victim->in_room) {
        ...
        else if (wield) // 增加 obj_hit 判定 , 把 wield 與 hp 判定分開寫 2022/10/23
        {
            ...
            if (wield->hp || wield->material->hp) {
                ...
                if (get_skill_level(ch, gsn_weapon_master) > 3)
                    WmArtLv += 2;
                if (get_skill_level(ch, gsn_weapon_master) > 4)
                    WmArtLv += 3;
                if (get_skill_level(ch, gsn_weapon_master) > 5)
                    WmArtLv += 4;
        ...
        if (gloves && (gloves->hp || gloves->material->hp) && !(get_eq_char(ch, WEAR_WIELD) || get_eq_char(ch, WEAR_TWO_HAND))) {
        ...
        if (gloves && (gloves->hp || gloves->material->hp) && (dt == 1000 || dt == gsn_frenzy || (dt == gsn_bashhead && !get_eq_char(ch, WEAR_HANDS)))) // bashhead 副手套判定 2020/11/01
        {
            OBJ_HP *ohp = gloves->hp;
            // 手套耐久耗損
            if (number_bits(5) < 8)
...
int one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt, int wpn) {
    ...
    if (protector && protector != victim) {
        for (; protector && protector != protector->next_in_protect; protector = protector->next_in_protect) {
            if (protector != ch && protector != victim && ch->in_room && protector->in_room == ch->in_room) {
                if ((IS_NPC(protector) && (protector->level + GetGeneralMobSklv(protector) * 24) / 2 > number_percent()) || (!IS_NPC(protector) && get_skill_percent(protector, gsn_protect) + get_skill_level(ch, gsn_protect) * 4 > number_percent())) {
                    //  modify by jye
                    //  修改為不stop_fighting
                    //  因為stop_fighting會將ch之cast去掉
                    ch->fighting = NULL;
