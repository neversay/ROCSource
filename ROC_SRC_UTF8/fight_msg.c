...
void self_damage(CHAR_DATA *ch, CHAR_DATA *victim, int miss_times, int no_dam_times, int dam_times, int times, int dam, char *skill_name) {
    ...
    if (miss_times > 0) {
        sprintf(buf1, "你的 %d/%d 次%s[1;37m沒有打中[m$N.", miss_times, times, skill_name);
        sprintf(buf2, "$n的 %d/%d 次%s[1;37m沒有打中[m你.", miss_times, times, skill_name);
        sprintf(buf3, "$n的 %d/%d 次%s[1;37m沒有打中[m%s$N" NOR ".", miss_times, times, skill_name, stat_color1(victim->hit, victim->max_hit));
        sprintf(buf4, "你的 %d/%d 次%s[1;37m沒有打中[m你自己.", miss_times, times, skill_name);
        sprintf(buf5, "$n的 %d/%d 次%s[1;37m沒有打中[m$m.", miss_times, times, skill_name);
    ...
    if (no_dam_times > 0) {
        ...
        sprintf(buf1, "你的 %d/%d 次%s[1;37m無法傷害[m$N.", no_dam_times, times, skill_name);
        sprintf(buf2, "$n的 %d/%d 次%s[1;37m無法傷害[m你.", no_dam_times, times, skill_name);
        sprintf(buf3, "$n的 %d/%d 次%s[1;37m無法傷害[m%s$N" NOR ".", no_dam_times, times, skill_name, stat_color1(victim->hit, victim->max_hit));
        sprintf(buf4, "你的 %d/%d 次%s[1;37m無法傷害[m你.", no_dam_times, times, skill_name);
        sprintf(buf5, "$n的 %d/%d 次%s[1;37m無法傷害[m$m.", no_dam_times, times, skill_name);
    ...
    if (dam_times > 0) {
        if (dam <= 24) {
            strcat(punct, ".");
            sprintf(buf1, "你的 %d/%d 次%s%s$N%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s%s你%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s%s%s$N" NOR "%s", dam_times, times, skill_name, stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s%s你自己%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s%s$m%s", dam_times, times, skill_name, vp, punct);
        } else if (dam <= 230) {
            strcat(punct, "!");
            sprintf(buf1, "你的 %d/%d 次%s讓$N%s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s讓你 %s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s讓%s$N" NOR "%s%s", dam_times, times, skill_name,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s讓你自己 %s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s讓$m %s%s", dam_times, times, skill_name, vp, punct);
        } else {
            strcat(punct, ".");
            sprintf(buf1, "你的 %d/%d 次%s打得$N%s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s打得你 %s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s打得%s$N" NOR "%s%s", dam_times, times, skill_name,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s打得你自己 %s%s", dam_times, times, skill_name, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s打得$m %s%s", dam_times, times, skill_name, vp, punct);
        }
...
void brief_e_dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn, int total, int hit) {
    ...
    (dam <= 40) ? strcat(punct, ".") : (dam <= 220) ? strcat(punct, "!")
                                                    : strcat(punct, "!!!");
...
void brief_c_dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn, int total, int hit) {
    ...
    if (dam <= 40) {
        ...
        if (dt == TYPE_HIT) {
            ...
            attack = race_table[ch->race].dmg_message;
            sprintf(buf1, "你的 %d/%d 次%s%s$N%s", hit, total, attack, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s%s你%s", hit, total, attack, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s%s%s$N" NOR "%s", hit, total, attack, stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s%s你自己%s", hit, total, attack, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s%s$m%s", hit, total, attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的 %d/%d 次%s%s$N%s", hit, total, attack, vp, punct);
                sprintf(buf2, "$n的 %d/%d 次%s%s你 %s", hit, total, attack, vp, punct);
                sprintf(buf3, "$n的 %d/%d 次%s%s%s$N" NOR "%s", hit, total, attack, stat_color1(victim->hit, victim->max_hit), vp, punct);
                sprintf(buf4, "你的 %d/%d 次%s%s你 %s", hit, total, attack, vp, punct);
                sprintf(buf5, "$n的 %d/%d 次%s%s$m %s", hit, total, attack, vp, punct);
                ...
                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的 %d/%d 次劇毒%s%s$N%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次劇毒%s%s你 %s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次劇毒%s%s%s$N" NOR "%s", hit, total, attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的 %d/%d 次劇毒%s%s你 %s", hit, total, attack, vp, punct);
                    sprintf(buf5, "$n的 %d/%d 次劇毒%s%s$m %s", hit, total, attack, vp, punct);
                } else {
                    sprintf(buf1, "你的 %d/%d 次%s%s$N%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次%s%s你 %s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次%s%s%s$N" NOR "%s", hit, total, attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的 %d/%d 次%s%s你 %s", hit, total, attack, vp, punct);
                    sprintf(buf5, "$n的 %d/%d 次%s%s$m %s", hit, total, attack, vp, punct);
                }
        ...
        if (dt == TYPE_HIT) {
            ...
            sprintf(buf1, "你的 %d/%d 次%s讓$N%s%s", hit, total, attack, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s讓你 %s%s", hit, total, attack, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s讓%s$N" NOR "%s%s", hit, total, attack,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s讓你自己 %s%s", hit, total, attack, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s讓$m %s%s", hit, total, attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的 %d/%d 次%s讓$N%s%s", hit, total, attack, vp, punct);
                ...
                attack = cattack_table[dt - TYPE_HIT];
                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的 %d/%d 次劇毒%s讓$N%s%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次劇毒%s讓你 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次劇毒%s讓%s$N" NOR "%s%s", hit, total, attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的 %d/%d 次劇毒%s讓你自己 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf5, "$n的 %d/%d 次劇毒%s讓$m %s%s", hit, total, attack, vp, punct);
                } else {
                    sprintf(buf1, "你的 %d/%d 次%s讓$N%s%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次%s讓你 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次%s讓%s$N" NOR "%s%s", hit, total, attack,
        ...
        if (dt == TYPE_HIT) {
            ...
            sprintf(buf1, "你的 %d/%d 次%s打得$N%s%s", hit, total, attack, vp, punct);
            sprintf(buf2, "$n的 %d/%d 次%s打得你 %s%s", hit, total, attack, vp, punct);
            sprintf(buf3, "$n的 %d/%d 次%s打得%s$N" NOR "%s%s", hit, total, attack,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的 %d/%d 次%s打得你自己 %s%s", hit, total, attack, vp, punct);
            sprintf(buf5, "$n的 %d/%d 次%s打得$m %s%s", hit, total, attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的 %d/%d 次%s打得$N%s%s", hit, total, attack, vp, punct);
                ...
                attack = cattack_table[dt - TYPE_HIT];
                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的 %d/%d 次劇毒%s打得$N%s%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次劇毒%s打得你 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次劇毒%s打得%s$N" NOR "%s%s", hit, total, attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的 %d/%d 次劇毒%s打得你自己 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf5, "$n的 %d/%d 次劇毒%s打得$m %s%s", hit, total, attack, vp, punct);
                } else {
                    sprintf(buf1, "你的 %d/%d 次%s打得$N%s%s", hit, total, attack, vp, punct);
                    sprintf(buf2, "$n的 %d/%d 次%s打得你 %s%s", hit, total, attack, vp, punct);
                    sprintf(buf3, "$n的 %d/%d 次%s打得%s$N" NOR "%s%s", hit, total, attack,
    ...
    if ( dt > MAX_SKILL && number_bits( 3 ) == 1 && weapon ){
            ...
            if ( is_wielding_poisoned( ch, wpn ) )
            {
                            sprintf( buf1, "你運用劇毒%s向$N猛烈攻擊%s", wps, punct );
                            sprintf( buf2, "$n運用劇毒%s向你攻擊%s", wps, punct );
                            sprintf( buf3, "$n運用劇毒%s向$N猛烈攻擊%s", wps, punct );
                            sprintf( buf4, "你運用劇毒%s向你自己攻擊 %s", wps, punct );
                            sprintf( buf5, "$n運用劇毒%s向$m攻擊%s", wps, punct );
            }
            else
            {
                            sprintf( buf1, "你運用%s向$N猛烈攻擊%s", wps, punct );
                            sprintf( buf2, "$n運用%s向你攻擊%s", wps, punct );
                            sprintf( buf3, "$n運用%s向$N猛攻而去%s", wps, punct );
                            sprintf( buf4, "你運用%s向你自己攻擊 %s", wps, punct );
                            sprintf( buf5, "$n運用%s向$m攻擊%s", wps, punct );
            }
            if ( victim != ch ){
                    combat_info( buf1, ch, victim, TO_CHAR );
                    combat_info( buf2, ch, victim, TO_VICT );
...
void c_dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int wpn) {
    ...
    if (dam <= 40) {
        ...
        if (dt == TYPE_HIT) {
            ...
            attack = race_table[ch->race].dmg_message;
            sprintf(buf1, "你的%s%s$N%s", attack, vp, punct);
            sprintf(buf2, "$n的%s%s你%s", attack, vp, punct);
            sprintf(buf3, "$n的%s%s%s$N" NOR "%s", attack, stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的%s%s你自己%s", attack, vp, punct);
            sprintf(buf5, "$n的%s%s$m%s", attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的%s%s$N%s", attack, vp, punct);
                sprintf(buf2, "$n的%s%s你 %s", attack, vp, punct);
                sprintf(buf3, "$n的%s%s%s$N" NOR "%s", attack, stat_color1(victim->hit, victim->max_hit), vp, punct);
                sprintf(buf4, "你的%s%s你 %s", attack, vp, punct);
                sprintf(buf5, "$n的%s%s$m %s", attack, vp, punct);
            } else if (dt >= TYPE_HIT && dt < TYPE_HIT + sizeof(cattack_table) / sizeof(cattack_table[0])) {
                attack = cattack_table[dt - TYPE_HIT];

                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的劇毒%s%s$N%s", attack, vp, punct);
                    sprintf(buf2, "$n的劇毒%s%s你 %s", attack, vp, punct);
                    sprintf(buf3, "$n的劇毒%s%s%s$N" NOR "%s", attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的劇毒%s%s你 %s", attack, vp, punct);
                    sprintf(buf5, "$n的劇毒%s%s$m %s", attack, vp, punct);
                } else {
                    sprintf(buf1, "你的%s%s$N%s", attack, vp, punct);
                    sprintf(buf2, "$n的%s%s你 %s", attack, vp, punct);
                    sprintf(buf3, "$n的%s%s%s$N" NOR "%s", attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的%s%s你 %s", attack, vp, punct);
                    sprintf(buf5, "$n的%s%s$m %s", attack, vp, punct);
                }
        ...
        if (dt == TYPE_HIT) {
            ...
            sprintf(buf1, "你的%s讓$N%s%s", attack, vp, punct);
            sprintf(buf2, "$n的%s讓你 %s%s", attack, vp, punct);
            sprintf(buf3, "$n的%s讓%s$N" NOR "%s%s", attack,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的%s讓你自己 %s%s", attack, vp, punct);
            sprintf(buf5, "$n的%s讓$m %s%s", attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的%s讓$N%s%s", attack, vp, punct);
                ...
                attack = cattack_table[dt - TYPE_HIT];
                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的劇毒%s讓$N%s%s", attack, vp, punct);
                    sprintf(buf2, "$n的劇毒%s讓你 %s%s", attack, vp, punct);
                    sprintf(buf3, "$n的劇毒%s讓%s$N" NOR "%s%s", attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的劇毒%s讓你自己 %s%s", attack, vp, punct);
                    sprintf(buf5, "$n的劇毒%s讓$m %s%s", attack, vp, punct);
                } else {
                    sprintf(buf1, "你的%s讓$N%s%s", attack, vp, punct);
                    sprintf(buf2, "$n的%s讓你 %s%s", attack, vp, punct);
                    sprintf(buf3, "$n的%s讓%s$N" NOR "%s%s", attack,
        ...
        if (dt == TYPE_HIT) {
            ...
            sprintf(buf1, "你的%s打得$N%s%s", attack, vp, punct);
            sprintf(buf2, "$n的%s打得你 %s%s", attack, vp, punct);
            sprintf(buf3, "$n的%s打得%s$N" NOR "%s%s", attack,
                    IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
            sprintf(buf4, "你的%s打得你自己 %s%s", attack, vp, punct);
            sprintf(buf5, "$n的%s打得$m %s%s", attack, vp, punct);
        } else {
            if (dt >= 0 && dt < MAX_SKILL) {
                attack = skill_table[dt].noun_damage;
                sprintf(buf1, "你的%s打得$N%s%s", attack, vp, punct);
                ...
                attack = cattack_table[dt - TYPE_HIT];
                if (is_wielding_poisoned(ch, wpn)) {
                    sprintf(buf1, "你的劇毒%s打得$N%s%s", attack, vp, punct);
                    sprintf(buf2, "$n的劇毒%s打得你 %s%s", attack, vp, punct);
                    sprintf(buf3, "$n的劇毒%s打得%s$N" NOR "%s%s", attack,
                            IS_NPC(victim) ? WHT : stat_color1(victim->hit, victim->max_hit), vp, punct);
                    sprintf(buf4, "你的劇毒%s打得你自己 %s%s", attack, vp, punct);
                    sprintf(buf5, "$n的劇毒%s打得$m %s%s", attack, vp, punct);
                } else {
                    sprintf(buf1, "你的%s打得$N%s%s", attack, vp, punct);
                    sprintf(buf2, "$n的%s打得你 %s%s", attack, vp, punct);
                    sprintf(buf3, "$n的%s打得%s$N" NOR "%s%s", attack,
