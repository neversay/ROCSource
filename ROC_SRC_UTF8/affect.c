...
void affect_remove(CHAR_DATA *ch, AFFECT_DATA *paf) {
     ...
     */
    if ((IS_NPC(ch)) && (ch->in_room) && (paf->type == gsn_charm_person) && ((qmark = is_qmark(ch, "roc_mob_charmed")) != NULL) && ((pRoomIndex = get_room_index(qmark->v0)) != NULL)) {
...
void affect_join(CHAR_DATA *ch, AFFECT_DATA *paf) {
    ...
    for (paf_old = ch->affected; paf_old; paf_old = paf_old->next) {
        ...
        if (paf_old->type == paf->type && paf_old->location == paf->location) {
            if (IS_NPC(ch) && paf->location < 6 && paf_old->modifier < 0 && number_percent() < (-paf_old->modifier * 8))
                return;
