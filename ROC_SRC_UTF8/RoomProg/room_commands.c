        ...
        if (is_number(arg)) {
            ...
            for (obj = target->carrying; obj; obj = obj_next) {
                ...
                if (obj->wear_loc == WEAR_NONE)
                    continue;
                if (
                    (IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(target)) || (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(target)) || (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(target))) {
    ...
    if (!str_cmp(type, "exp")) {
        for (gch = victim->in_room->people; gch; gch = gch->next_in_room) {
            ...
            if (is_same_group(victim, gch)) {
                if ((lv < 0 && gch->level + lv < 1) || (lv > 0 && gch->level - lv > -1)) {
                    gain_exp(gch, v);
...
void do_modrvt(ROOM_INDEX_DATA *room, CHAR_DATA *actor, char *argument) {
    ...
    if (arg[0] != 'v' && arg[0] != 't' && arg[0] != 'd' && arg[0] != 'x' // 設定 door 的 size 2022/12/20
    ...
    if (room->people)
        for (vch = room->people; vch; vch = vch->next_in_room) {
            ...
            case 2:
                ...
                break;
            case 3:
                dam = (hmv == 0) ? -vch->hit * num / 100 : (hmv == 1) ? -vch->mana * num / 100
                                                                      : -vch->move * num / 100;
                break;
            case 4:
                dam = (hmv == 0) ? vch->hit * num / 100 : (hmv == 1) ? vch->mana * num / 100
                ...
                break;
            case 5: // 2023/01/15
                dam = (hmv == 0) ? -vch->max_hit * num / 100 : (hmv == 1) ? -vch->max_mana * num / 100
            ...
            case 0:
                ...
                case POS_STUNNED:
                    if (!is_chinese(vch, NULL)) {
                        ...
                        send_to_char("你昏倒了，過一會也許會回復.\n\r", vch);
                        act("$n 昏倒了，過一會也許會回復.", vch, NULL, NULL, TO_ROOM);
                    }
                    break;
                ...
                default:
                    ...
                    if (dam + (vch->max_hit / 4) < 0)
                        send_to_char((is_chinese(vch, NULL)) ? HIY "天哪! 痛死人了!" NOR "\n\r" : HIY "That really did HURT!" NOR "\n\r",
                                     vch);
                    if (vch->hit < vch->max_hit / 4)
                        send_to_char((is_chinese(vch, NULL)) ? HIR "你正在噴血!" NOR "\n\r" : HIR "You sure are BLEEDING!" NOR "\n\r",
                                     vch);
                    break;
                }
                break;
...
CHAR_DATA *room_find_char(ROOM_INDEX_DATA *room, char *name) {
    ...
    if (room->people) {
        for (target = room->people; target; target = target->next_in_room)
            if (is_name(name, target->name) && (IS_NPC(target) || (!IS_NPC(target) && !IS_SET(target->act, PLR_WIZINVIS))))
                break;
        return target;
    }
