...
int rprog_name_to_type(char *name) {
    if (!str_cmp(name, "room_enter"))
        return ROOM_ENTER;
    if (!str_cmp(name, "room_leave"))
        return ROOM_LEAVE;
    if (!str_cmp(name, "room_rand"))
        return ROOM_RAND;
    if (!str_cmp(name, "room_time"))
        return ROOM_TIME;
    if (!str_cmp(name, "room_command"))
        return ROOM_COMMAND;
    if (!str_cmp(name, "room_mask"))
        return ROOM_MASK;
    if (!str_cmp(name, "room_trap"))
        return ROOM_TRAP;
    if (!str_cmp(name, "room_defun"))
        return ROOM_DEFUN;
...
bool rprog_cmd_trigger(ROOM_INDEX_DATA *room, CHAR_DATA *ch, char *txt) {
    ...
    if (!(room->progtypes & ROOM_COMMAND))
        return FALSE;
